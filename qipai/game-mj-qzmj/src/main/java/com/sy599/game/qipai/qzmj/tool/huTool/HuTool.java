...
public class HuTool {
    ...
    public static List<Integer> idsToVals(List<Integer> ids){
        ...
        if(ids==null)
            return list;
        for (Integer id:ids) {
            list.add(QzMj.getMajang((id)).getVal());
        }
        return list;
    }

    public static List<Integer> mjsToVals(List<QzMj> mjs){
    ...
    public static int[]
    getValToInts(List<Integer> vals,int bossNum){
        int [] yus=new int[35];
    ...
    public static int isZu(int size,int bossNum){
        if(size==0)
            return bossNum;
        int yu=size%3;
        if(yu!=0&&bossNum<3-yu)
...
//        }
    ...
    private static List<int[]> getClassify(int[]yus){
        ...
        classfy.add(Arrays.copyOfRange(yus,28,29));
        classfy.add(Arrays.copyOfRange(yus,29,30));
        classfy.add(Arrays.copyOfRange(yus,30,31));
        classfy.add(Arrays.copyOfRange(yus,31,32));
        classfy.add(Arrays.copyOfRange(yus,32,33));
        classfy.add(Arrays.copyOfRange(yus,33,34));
        classfy.add(Arrays.copyOfRange(yus,34,35));
        return classfy;
    }
     ...
     * @return
     */
    public static boolean isHu(List<Integer> ids, List<Integer> chunWang,  List<MjCardDisType> disTypes, boolean baoTing, boolean selfMo){
        if(isXiaoHu(ids,chunWang))
            return true;
        if(isDaHu(ids,chunWang,disTypes,baoTing,selfMo))
            return true;
        return false;
    }
     ...
     * @return
     */
    public static boolean isDaHu(List<Integer> ids,List<Integer> chunWang,List<MjCardDisType> disTypes,boolean baoTing,boolean selfMo){
        //不需要成牌型的大胡
        if(isHu7dui(ids,chunWang))
            return true;
        if(isHu4Wang(ids,chunWang))
            return true;
        if(isHuLanhu(ids,chunWang))
            return true;
        if(isHu7XGW(ids,chunWang))
            return true;
//        if(!baoTing){
//            if(isWangHu(ids,chunWang,zhengWang,selfMo))
//                return true;
//        }
        //需要成牌型的大胡
        if(isHu(ids,chunWang)){
            //碰碰胡
            ...
            if(isQingYiSe(disTypes,ids,chunWang)){
                return true;
            }else if(isHunYiSe(disTypes,ids,chunWang)){ //混一色
                return true;
            }


            if(isZiYiSe(disTypes,ids,chunWang)){
                return true;
            }else if(isDaSanYuan(disTypes,ids,chunWang))
                return true;
        }
        return false;
    }

    public static boolean check7dui(List<Integer> ids,List<Integer> bossVals,List<Integer> huType){
        if(ids.size()!=14)
            return false;
        List<Integer> vals = idsToVals(ids);
        int bossNum = dropBoss(vals, bossVals);
        ...
        if(bossNum>=0){
            huType.add(QzMingTang.MINGTANG_DH_QXD);
            return true;
        }
        return false;
    }
    ...
    public static boolean isHu4Wang(List<Integer> ids,List<Integer> bossVals){
        if(ids.size()!=14)
            return false;
        List<Integer> vals = idsToVals(ids);
        int bossNum = dropBoss(vals, bossVals);
    ...
    public static boolean isHuLanhu(List<Integer> ids,List<Integer> bossVals){
        if(ids.size()!=14)
            return false;
        List<Integer> vals = idsToVals(ids);
        int bossNum = dropBoss(vals, bossVals);
        Set<Integer> fengCards = new HashSet<>();
        Collections.sort(vals);
        int curVal = vals.remove(0);
        for (Integer val : vals){
            int curChu = curVal / 10;
            int valChu = val / 10;
            if(curChu == valChu && val - curVal < 3){
                return false;
            }
            curVal = val;
            if(val > 200){
        ...
        if(fengCards.size() + bossNum < 5){
            return false;
        }
        return true;
    }
    public static boolean isHu7XGW(List<Integer> ids,List<Integer> bossVals){
        if(ids.size()!=14)
            return false;
        List<Integer> vals = idsToVals(ids);
        dropBoss(vals, bossVals);
        Set<Integer> fengCards = new HashSet<>();
        Collections.sort(vals);
        int curVal = vals.remove(0);
        for (Integer val : vals){
     ...
     * @return
     */
    public static boolean isWangHu(List<Integer> ids,List<Integer> chunWang,int zhengWang,boolean selfMo){
        if(ids.size()%3!=2)
            return false;
        List<Integer> vals = idsToVals(ids);
        int chunNum1=getMjNum(MjHelper.toMajiang(ids),chunWang.get(0));
        int chunNum2=0;
        if(chunWang.size()==2){
            chunNum2=getMjNum(MjHelper.toMajiang(ids),chunWang.get(1));
        }
        if(chunNum1>=4||chunNum2>=4)
            return true;
        List<Integer> zWang=new ArrayList<>();
        zWang.add(zhengWang);
        int zhengNum = dropBoss(vals, zWang);
        if(zhengNum>=3&&selfMo)
            return true;
        return false;
    }
    ...
    public static int[] getIntZu(List<Integer> handCards,List<Integer> chunWang){
        ...
        for (Integer id:handCards) {
            int val = QzMj.getMajang(id).getVal();
    ...
    public static boolean isQingYiSe(List<MjCardDisType> disTypes, List<Integer> handIds,List<Integer> chunWang){
        ...
        for (Integer id:handIds) {
            QzMj mj = QzMj.getMajang(id);
            if(chunWang.contains(mj.getVal()))
                continue;
            int chu=QzMj.getMajang(id).getVal()/10;
            if(clas==0){
                clas=chu;
            }else {
                if(chu!=clas)
    ...
    public static boolean isHunYiSe(List<MjCardDisType> disTypes, List<Integer> handIds,List<Integer> chunWang){
        int clas=0;
        boolean isyoufen = false;
        boolean isyouQiu = false;
        for (Integer id:handIds) {
            QzMj mj = QzMj.getMajang(id);
            if(chunWang.contains(mj.getVal()))
                continue;
            if(QzMj.getMajang(id).getVal() > 200){
                isyoufen=  true;
                continue;
            }
            isyouQiu = true;
        ...
        return isyoufen && isyouQiu;
    }

    public static boolean isZiYiSe(List<MjCardDisType> disTypes, List<Integer> handIds,List<Integer> chunWang) {
        for (Integer id : handIds) {
            QzMj mj = QzMj.getMajang(id);
            if(chunWang.contains(mj.getVal()))
                continue;
    ...
    public static boolean isDaSanYuan(List<MjCardDisType> disTypes, List<Integer> handIds,List<Integer> chunWang) {
        int zhong = 0;
        int fa = 0;
        int bai = 0;
        for (Integer id : handIds) {
    ...
    public static List<Integer> getHuType(List<Integer> ids, List<Integer> chunWang, QzMjPlayer player, QzMjTable table, boolean selfMo){
        //先判断起手胡
        List<Integer> huType=getDaHuNew(ids,chunWang,player,table,selfMo);
        if(table.getDisNum()==0){
            huType.add(QzMingTang.MINGTANG_DH_TIANHU);
        }
        if(table.getDisNum()==1 && !selfMo){
            huType.add(QzMingTang.MINGTANG_DH_DIHU);
        }
//        if(huType.size() == 0){
//            huType.add(QzMingTang.MINGTANG_PINGHU);
//        }
        if(player.isZhuayu()){
            huType.add(QzMingTang.MINGTANG_DH_ZY);
        }
//        if(huType.size()>0 && table.getDisNum()==0){
//            huType.add(QzMingTang.MINGTANG_DH_TIANHU);
//        }else if(huType.size()==0&&isXiaoHu(ids,chunWang)){
//            if(table.getDisNum()==0){
//                huType.add(QzMingTang.MINGTANG_DH_TIANHU);
//            }else {
//                huType.add(QzMingTang.MINGTANG_PINGHU);
//            }
//            checkBoss2(huType,MjHelper.toMajiang(ids),chunWang);
//        }
        return huType;
    }
...
//     * @return
//     */
//    public static List<Integer> getDaHu(List<Integer> ids, List<Integer> chunWang, QzMjPlayer player, QzMjTable table){
//        List<Integer> huType;
//        //需要王代替目标牌型的大胡
...
//    private static List<ReplaceLack> getAllJJH(List<MjCardDisType> cardTypes, List<QzMj> handMajiang,List<Integer> chunWang){
...
//    }
    ...
    private static void checkBoss1(List<Integer> huType,List<QzMj> mjs,List<Integer> chunWang){
        int chunNum1=getMjNum(mjs,chunWang.get(0));
         if(chunNum1==4)
            huType.add(QzMingTang.MINGTANG_4W);
        if(huType.size()>0&&chunNum1==0&&!huType.contains(QzMingTang.MINGTANG_0W))
            huType.add(QzMingTang.MINGTANG_0W);
    }

    private static void checkBoss2(List<Integer> huType,List<QzMj> mjs,List<Integer> chunWang){
        int chunNum1=getMjNum(mjs,chunWang.get(0));
        if(huType.size()>0&&chunNum1==0&&!huType.contains(QzMingTang.MINGTANG_0W))
            huType.add(QzMingTang.MINGTANG_0W);
    }

    private static int getMjNum(List<QzMj> mjs,int val){
        int count=0;
        for (QzMj mj:mjs) {
            if(mj.getVal()==val)
                count++;
...
//            return;
//        int val=vals.get(0);
//        List<int[]> paiZus = QzMj.getPaiZu(val);
...
//        Map<Integer, List<int[]>> paiZuMap = QzMj.getPaiZuMap();
...
//        for (MjCardDisType type:cardTypes) {
//            for (Integer id:type.getCardIds()) {
//                int yu=QzMj.getMajang(id).getVal()%10;
//                if(yu!=2&&yu!=5&&yu!=8)
//                    flag=false;
//            }
//        }
...
//    }
     ...
     * @return
     */
    public static List<Integer> getDaHuNew(List<Integer> ids, List<Integer> chunWang, QzMjPlayer player, QzMjTable table ,boolean selfMo){
        List<Integer> huType=new ArrayList<>();

        if(isHu7dui(ids, chunWang)){
            huType.add(QzMingTang.MINGTANG_DH_QXD);
            huType.remove((Integer) QzMingTang.MINGTANG_PINGHU);
            if(isQingYiSe(player.getCardTypes(), ids, chunWang))
                huType.add(QzMingTang.MINGTANG_DH_QYS);
            //混一色
            if(isHunYiSe(player.getCardTypes(),ids,chunWang))
                huType.add(QzMingTang.MINGTANG_DH_HYS);
            if(isZiYiSe(player.getCardTypes(),ids,chunWang)){
                huType.add(QzMingTang.MINGTANG_DH_ZYS);
            }
            //王相关胡
            checkBoss1(huType,MjHelper.toMajiang(ids),chunWang);
            return huType;
        }
        boolean pingHu = isHu(ids, chunWang);
        if(!pingHu){
            if(isHu7XGW(ids,chunWang)){
                huType.add(QzMingTang.MINGTANG_DH_QXGW);
            }else if(isHuLanhu(ids,chunWang)){
                huType.add(QzMingTang.MINGTANG_DH_LP);
            }

        }
        if(pingHu){
            huType.add(QzMingTang.MINGTANG_PINGHU);
            if(isPPH(ids,player.getCardTypes(),chunWang))
                huType.add(QzMingTang.MINGTANG_DH_PPH);
            if(isQingYiSe(player.getCardTypes(), ids, chunWang))
                huType.add(QzMingTang.MINGTANG_DH_QYS);
            //混一色
            if(isHunYiSe(player.getCardTypes(),ids,chunWang))
            ...
            if(isZiYiSe(player.getCardTypes(),ids,chunWang)){
                huType.add(QzMingTang.MINGTANG_DH_ZYS);
            }else if(isDaSanYuan(player.getCardTypes(),ids,chunWang))
                huType.add(QzMingTang.MINGTANG_DH_DSY);
        }
    ...
    private static boolean isPPH(List<Integer> ids,List<MjCardDisType> cardTypes,List<Integer> chunWang){
        for (MjCardDisType type:cardTypes) {
            int action = type.getAction();
            if(action==MjDisAction.action_chi)
                return false;
