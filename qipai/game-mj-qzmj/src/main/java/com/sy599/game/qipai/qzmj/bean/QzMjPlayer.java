...
import com.sy599.game.qipai.qzmj.command.MjCommandProcessor;
...
public class QzMjPlayer extends Player {
    // 座位id
    private int seat;
    // 状态
    ...
    private List<QzMj> handPais;
    private List<Integer> outPais;
    private List<QzMj> peng;
    private List<QzMj> aGang;
    private List<QzMj> mGang;
    private int winCount;
    private int lostCount;
    private int curChiVal;//当前吃的牌
    private List<Integer> guoPengs;//当前回合过碰的牌
    private boolean isZhuayu = false;
    /**
     * 局分
     */
    ...
    private int moNum;//摸排数
    private int chuNum;//出牌数
    ...
    public String toInfoStr() {
        ...
        sb.append(chuNum).append(",");
        sb.append(gangStatus).append(",");
        return sb.toString();
    }
    ...
    public void initPlayInfo(String data) {
        if (!StringUtils.isBlank(data)) {
            ...
            this.chuNum = StringUtil.getIntValue(values, i++);
            this.gangStatus = StringUtil.getIntValue(values, i++);
        }
    }
    ...
    public void dealHandPais(List<QzMj> pais) {
        this.handPais = pais;
        getPlayingTable().changeCards(seat);
    }

    public List<QzMj> getHandMajiang() {
        return handPais;
    }
    ...
    public List<Integer> getHandPaisWhithOutMo() {
        ...
        QzMjTable table = getPlayingTable(QzMjTable.class);
    ...
    public void moMajiang(QzMj majiang) {
     ...
     */
    public void addOutPais(List<QzMj> cards, int action, int disSeat) {
        ...
        if (action == MjDisAction.action_chupai) {
            ...
            if (action == MjDisAction.action_peng) {
                ...
                if (cards.size() != 1) {
                    ...
                    QzMj pengMajiang = cards.get(0);
                    Iterator<QzMj> iterator = peng.iterator();
                    while (iterator.hasNext()) {
                        QzMj majiang = iterator.next();
                ...
                changeAction(QzMjConstants.ACTION_COUNT_INDEX_MINGGANG, 1);
            } else if (action == MjDisAction.action_angang) {
                aGang.addAll(cards);
                changeAction(QzMjConstants.ACTION_COUNT_INDEX_ANGANG, 1);
            }
    ...
    public void qGangUpdateOutPais(QzMj card) {
        Iterator<QzMj> iterator = mGang.iterator();
        while (iterator.hasNext()) {
            QzMj majiang = iterator.next();
     ...
     * @param disCardList
     */
    public void addCardType(int action, List<QzMj> disCardList, int disSeat, int disStatus) {
        if (action != 0) {
            if (action == MjDisAction.action_minggang && disCardList.size() == 1) {
                QzMj majiang = disCardList.get(0);
                for (MjCardDisType disType : cardTypes) {
...
//                            disType.setDisSeat(seat);
    ...
    public void removeOutPais(List<QzMj> cards) {
    ...
    public PlayerInTableRes.Builder buildPlayInTableInfo(boolean isrecover) {
        ...
        res.addExt(0);                       //5
        res.addExt(0);                      //6 fengdong
        res.addExt(baoTing);                       //7
        res.addExt(curChiVal);                  //8
    ...
    public List<QzMj> getGang() {
        List<QzMj> gang = new ArrayList<>();
        gang.addAll(aGang);
        gang.addAll(mGang);
        return gang;
    }
...
//	}
        ...
        if (!StringUtils.isBlank(outPai)) {
            ...
            for (String value : values) {
                ...
                if (i == 0) {
                    ...
                    type.init(value);
                    cardTypes.add(type);
                    List<QzMj> majiangs = MjHelper.toMajiang(type.getCardIds());
     ...
     */
    public List<Integer> checkDisMajiang(QzMj majiang, boolean canChi) {
        List<Integer> list = new ArrayList<>();
        int[] arr = new int[6];
        // 转转麻将别人出牌可以胡
        QzMjTable table=getPlayingTable(QzMjTable.class);
        if (passHu==0 && !isZhuayu() && table.isHuBaoHuDianPao(seat)) {
            // 没有出现漏炮的情况
            List<QzMj> copy = new ArrayList<>(handPais);
            copy.add(majiang);
            boolean hu = HuTool.isHu(MjHelper.toMajiangIds(copy),table.getChunWang(),getCardTypes(),false,false);
            if (hu) {
                arr[QzMjConstants.ACTION_INDEX_HU] = 1;
            }
        }
        //报听不能吃碰
        if(moDa==0){
            int count = MjHelper.getMajiangCount(handPais, majiang.getVal());
            if (count >= 2 && !guoPengs.contains(majiang.getVal())) {
                arr[QzMjConstants.ACTION_INDEX_PENG] = 1;// 可以碰
            }
            if (count == 3&&!passGangValList.contains(majiang.getVal())&&table.getLeftMajiangCount()>2) {
                arr[QzMjConstants.ACTION_INDEX_MINGGANG] = 1;// 可以杠
            }
            if(canChi&&!passMajiangVal.contains(majiang.getVal())){
                List<QzMj> chis = MjTool.checkChi(handPais, majiang,table.getChunWang());
                if(chis.size()>0)
                    arr[QzMjConstants.ACTION_INDEX_CHI] = 1;// 可以吃
            }
        }else{
            int count = MjHelper.getMajiangCount(handPais, majiang.getVal());
            if (count == 3&&!passGangValList.contains(majiang.getVal())&&table.getLeftMajiangCount()>2) {
                List<Integer> copyIds = removeGangId(handPais, majiang.getVal());
                if(TingTool.getTing(copyIds,table.getChunWang(),getCardTypes(),false).size() >= 0){
                    arr[QzMjConstants.ACTION_INDEX_MINGGANG] = 1;// 可以杠
                }
            }
        }
    ...
    public List<Integer> checkMo() {
        ...
        QzMjTable table=getPlayingTable(QzMjTable.class);
        int[] arr = checkGang();
        if (HuTool.isHu(MjHelper.toMajiangIds(handPais),table.getChunWang(),getCardTypes(),false,true)) {
            arr[QzMjConstants.ACTION_INDEX_HU] = 1;
        }
        for (int val : arr) {
            list.add(val);
        }
        checkBaoTing(list,table.getChunWang());
        moNum ++;
        if (list.contains(1)) {
            return list;
        } else {
        ...
        if (isAlreadyMoMajiang()&&table.getLeftMajiangCount()>2) {
            ...
            for (QzMj handMajiang : handPais) {
                ...
                if (pengMap.containsKey(handMajiang.getVal())&&!table.getChunWang().contains(handMajiang.getVal())) {
                    ...
                    if(moDa == 0 || TingTool.getTing(copyIds,table.getChunWang(),getCardTypes(),false).size()>0){
                        arr[QzMjConstants.ACTION_INDEX_MINGGANG] = 1;// 可以杠
                        break;
                    }
                }
            ...
            if(handMap.containsValue(4)){
                for (Map.Entry<Integer,Integer> entry:handMap.entrySet()) {
                    if(entry.getValue()==4&&!table.getChunWang().contains(entry.getKey())){
                        ...
                        if(moDa == 0 || TingTool.getTing(copyIds,table.getChunWang(),getCardTypes(),false).size()>0) {
                            arr[QzMjConstants.ACTION_INDEX_ANGANG] = 1;// 可以暗杠
                        }
                    }
                }
            }
    ...
    public List<Integer> removeGangId(List<QzMj> mjs,int removeVal){
    ...
    public List<Integer> checkBaoTing(List<Integer> actList,List<Integer> chunWang){
        if(moNum == 0 && handPais.size() == 14){
            List<QzMj> cards = new ArrayList<>(getHandMajiang());
            Map<Integer, List<Integer>> daTing = TingTool.getDaTing(MjHelper.toMajiangIds(cards),chunWang,getCardTypes());
            if(daTing != null && daTing.size() >0){
                actList.set(QzMjConstants.ACTION_INDEX_BAOTING,1);
            }
        }
        if(moNum == 0 && handPais.size() == 13){
            List<Integer> ting = TingTool.getTing(getHandPais(), chunWang, getCardTypes(),true);
            if(ting.size()>0)
                actList.set(QzMjConstants.ACTION_INDEX_BAOTING,1);
        }
        if(moNum == 0 && cardTypes.size() == 1){
            List<QzMj> cards = new ArrayList<>(getHandMajiang());
            Map<Integer, List<Integer>> daTing = TingTool.getDaTing(MjHelper.toMajiangIds(cards),chunWang,getCardTypes());
            if(daTing != null && daTing.size() >0){
                actList.set(QzMjConstants.ACTION_INDEX_BAOTING,1);
    ...
    public List<QzMj> getPeng() {
        return peng;
    }
    public boolean isZhuayu(){
        return isZhuayu;
    }
    public void setZhuayu(boolean zy){
        this.isZhuayu = zy;
    }
    public int getCurChiVal(){
        return curChiVal;
    }
    public void setCurChiVal(int curChiVal){
        this.curChiVal = curChiVal;
    }
    public void addGouPeng(int guoPengVal){
        this.guoPengs.add(guoPengVal);
    }
    public void cleanGuoPeng(){
        guoPengs.clear();
    }
    public void cleanCurChiVal(){
        curChiVal = 0;
    }
    public List<QzMj> getaGang() {
        return aGang;
    }

    public List<QzMj> getmGang() {
        return mGang;
    }
     ...
     * @return
     */
    public boolean isPassGang(QzMj majiang) {
    ...
    public boolean checkAutoPlay(int autoType, boolean forceSend) {
        ...
        if (!auto && table.getIsAutoPlay() >= 1) {
            ...
            } else if (timeOut >= QzMjConstants.AUTO_CHECK_TIMEOUT) {
                if (sendAutoTime == 0) {
                    sendAutoTime = now;
                    setLastCheckTime(now - QzMjConstants.AUTO_CHECK_TIMEOUT * 1000);
        ...
        if (auto) {
            if (getAutoPlayTime() == 0L) {
                ...
                if (autoType == 1) {
                    ...
                    if (timeOut >= QzMjConstants.AUTO_HU_TIME) {
                        setAutoPlayTime(0);
                        return true;
                    }
                } else {
                    if (timeOut >= QzMjConstants.AUTO_PLAY_TIME) {
                        setAutoPlayTime(0);
                        return true;
                    }
                }
    ...
    public void setGangStatus(int gangStatus) {
    ...
    public void setAutoPlay(boolean autoPlay, boolean isSelf) {
        ...
        if (this.autoPlay != autoPlay || autoPlaySelf != isSelf) {
            ...
            table.addPlayLog(table.getDisCardRound() + "_" +getSeat() + "_" + QzMjConstants.action_tuoguan + "_" +(autoPlay?1:0)+ getExtraPlayLog());
        	}
    ...
    public int getGangFen(){
        ...
        int fen = 0;
        for (MjCardDisType type : cardTypes){
            if(type.getAction() == MjDisAction.action_minggang){
                fen += 1;
            }else if(type.getAction() == MjDisAction.action_angang){
                fen += 2;
            }
        }
        return fen;
    ...
    public int getGangNum(){
        ...
        int gangNum = 0;
        for (MjCardDisType type : cardTypes){
            if(type.getAction() == MjDisAction.action_minggang || type.getAction() == MjDisAction.action_angang){
                gangNum ++ ;
            }
        }
        return gangNum;
...
//		getPlayingTable().changeExtend();
    ...
    public int getXingNum(List<Integer> xingVals){
        if(xingVals== null || xingVals.size() <= 0){
            return 0;
        }
        int count = 0;
        for (MjCardDisType type : cardTypes){
            for (Integer mjId : type.getCardIds()){
                QzMj mj = QzMj.getMajang(mjId);
                if(mj!= null && xingVals.contains(mj.getVal())){
                    count ++;
                }
            }
        }
        List<QzMj> handCards = getHandMajiang();
        if(handCards != null && handCards.size() > 0){
            for (QzMj mj : handCards){
                if(mj!= null && xingVals.contains(mj.getVal())){
                    count ++;
                }
            }
