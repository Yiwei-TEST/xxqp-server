...
public class MjIndexArr {
    ...
    public void addMajiangIndex(int count, List<QzMj> majiangList, int val) {
        if (a[count] == null) {
            a[count] = new MjIndex();
        }
     ...
     * @return
     */
    public Map<Integer, List<QzMj>> getMajiangMap(int size) {
        Map<Integer, List<QzMj>> map = new HashMap<>();
    ...
    public Map<Integer, List<QzMj>> getJiang(boolean need258) {
        Map<Integer, List<QzMj>> map = new HashMap<>();
        for (int i = 0; i < a.length; i++) {
            if (2 <= i + 1) {
                MjIndex majiangIndex = a[i];
                if (majiangIndex != null) {
                    if (need258) {
                        for (Entry<Integer, List<QzMj>> entry : majiangIndex.getMajiangValMap().entrySet()) {
