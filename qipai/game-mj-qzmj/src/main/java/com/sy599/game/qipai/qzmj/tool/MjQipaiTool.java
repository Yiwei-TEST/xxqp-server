...
import com.sy599.game.qipai.qzmj.rule.MjIndexArr;
...
public class MjQipaiTool {
     ...
     * @return String
     */
    public static List<QzMj> explodeMajiang(String str, String delimiter) {
        ...
        for (String val : strArray) {
            ...
            if (val.startsWith("mj")) {
                majiang = QzMj.valueOf(QzMj.class, val);
            } else {
                Integer intVal = (Integer.valueOf(val));
                if (intVal == 0) {
                    continue;
                }
                majiang = QzMj.getMajang(intVal);
            }
            list.add(majiang);
        }
        return list;
     ...
     * @param list
     */
    public static void getMax(MjIndexArr card_index, List<QzMj> list) {
        Map<Integer, List<QzMj>> majiangMap = new HashMap<Integer, List<QzMj>>();
        for (int i = 0; i < list.size(); i++) {
            QzMj majiang = list.get(i);
            List<QzMj> count = null;
        ...
        for (int majiangVal : majiangMap.keySet()) {
            List<QzMj> majiangList = majiangMap.get(majiangVal);
     ...
     * @return
     */
    public static List<QzMj> getVal(List<QzMj> copy, int val) {
