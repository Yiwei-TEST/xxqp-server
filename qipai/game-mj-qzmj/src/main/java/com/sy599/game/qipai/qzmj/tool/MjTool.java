 ...
 */
public class MjTool {
    public static synchronized List<List<QzMj>> fapai(List<Integer> copy, int playerCount) {
        List<List<QzMj>> list = new ArrayList<>();
        Collections.shuffle(copy);
        List<QzMj> allMjs = new ArrayList<>();
        for (int id : copy) {
            allMjs.add(QzMj.getMajang(id));
        }
    ...
    public static synchronized List<List<QzMj>> fapai(List<Integer> copy, int playerCount, List<List<Integer>> t) {
        List<List<QzMj>> list = new ArrayList<>();
        ...
        for (int i = 0; i < playerCount; i++) {
            if(buShouPai||list.get(i).size()==0){
                List<QzMj> ahmjs = list.get(i);
                if(i==0){
                    for (int j = ahmjs.size(); j < 14; j++) {
                        ahmjs.add(QzMj.getMajang(copy.get(j)));
                        copy.remove(j);
                    }
                }else {
        ...
        if(list.size()>playerCount&&buMoPai){
            ...
            List<QzMj> l=new ArrayList<>();
            for (int i = 0; i < copy.size(); i++) {
                l.add(QzMj.getMajang(copy.get(i)));
            }
            list.add(l);
        }
    ...
    // 拆牌
    public static boolean chaipai(MjHuLack lack, List<QzMj> hasPais, boolean isNeedJiang258) {
        if (hasPais.isEmpty()) {
            return true;
    ...
    public static void sortMin(List<QzMj> hasPais) {
        Collections.sort(hasPais, new Comparator<QzMj>() {

            @Override
            public int compare(QzMj o1, QzMj o2) {
     ...
     * @return
     */
    public static boolean chaishun(MjHuLack lack, List<QzMj> hasPais, boolean needJiang258) {
        if (hasPais.isEmpty()) {
            return true;
        }
        sortMin(hasPais);
        QzMj minMajiang = hasPais.get(0);
        int minVal = minMajiang.getVal();
        List<QzMj> minList = MjQipaiTool.getVal(hasPais, minVal);
        if (minList.size() >= 3) {
            // 先拆坎子
        ...
        List<QzMj> num1 = MjQipaiTool.getVal(hasPais, pai1);
        List<QzMj> num2 = MjQipaiTool.getVal(hasPais, pai2);
        List<QzMj> num3 = MjQipaiTool.getVal(hasPais, pai3);

        // 找到一句话的麻将
        List<QzMj> hasMajiangList = new ArrayList<>();
        if (!num1.isEmpty()) {
        ...
        if (lackNum > 0) {
            ...
            if (lackNum >= 2) {
                // 补坎子
                List<QzMj> count = MjQipaiTool.getVal(hasPais, hasMajiangList.get(0).getVal());
                if (count.size() >= 3) {
            ...
            // 可以一句话
            if (lack.getHongzhongNum() > 0) {
                List<QzMj> count1 = MjQipaiTool.getVal(hasPais, hasMajiangList.get(0).getVal());
                List<QzMj> count2 = MjQipaiTool.getVal(hasPais, hasMajiangList.get(1).getVal());
                List<QzMj> count3 = MjQipaiTool.getVal(hasPais, hasMajiangList.get(2).getVal());
                if (count1.size() >= 2 && (count2.size() == 1 || count3.size() == 1)) {
                    List<QzMj> copy = new ArrayList<>(hasPais);
    ...
    public static void removeAllPai(List<QzMj> hasPais, List<QzMj> remPai) {
        for (QzMj majiang : remPai) {
            hasPais.remove(majiang);
        }
    }

    public static boolean isCanAsJiang(QzMj majiang, boolean isNeed258) {
        if (isNeed258) {
    ...
    public static List<QzMj> checkChi(List<QzMj> majiangs, QzMj dismajiang) {
     ...
     * @return
     */
    public static List<QzMj> checkChi(List<QzMj> majiangs, QzMj dismajiang, List<Integer> wangValList) {
        ...
        return new ArrayList<QzMj>();
    }

    public static List<QzMj> findMajiangByVals(List<QzMj> majiangs, List<Integer> vals) {
        List<QzMj> result = new ArrayList<>();
        for (int val : vals) {
     ...
     * @return
     */
    public static List<QzMj> dropHongzhong(List<QzMj> copy) {
        List<QzMj> hongzhong = new ArrayList<>();
        Iterator<QzMj> iterator = copy.iterator();
    ...
    public static boolean checkWang(Object majiangs, List<Integer> wangValList) {
        if (majiangs instanceof List) {
            ...
            for (Object majiang : list) {
                int val = 0;
                if (majiang instanceof QzMj) {
                    val = ((QzMj) majiang).getVal();
                } else {
                    val = (int) majiang;
                }
    ...
    private static List<QzMj> find(List<Integer> copy, List<Integer> valList) {
