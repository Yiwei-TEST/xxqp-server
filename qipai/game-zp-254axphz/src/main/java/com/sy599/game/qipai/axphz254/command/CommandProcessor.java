package com.sy599.game.qipai.axphz254.command;
...
import com.sy599.game.qipai.axphz254.command.com.ComCommand;
import com.sy599.game.qipai.axphz254.command.play.PlayCommand;
...
public class CommandProcessor extends AbstractBaseCommandProcessor {
    ...
    public void process(AxPaohuziPlayer player, MessageUnit message) {
        int code = 0;
        try {
    ...
    public void process(Player player, MessageUnit message) {
        ...
        int code = 0;
        try {
            AxPaohuziPlayer pdkPlayer = player.getPlayer(processor);
            BaseCommand action = ObjectUtil.newInstance(commandMap.get(message.getMsgType()));
            action.setPlayer(player);
