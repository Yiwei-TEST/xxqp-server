package com.sy599.game.qipai.axphz254.tool;
...
import com.sy599.game.qipai.axphz254.bean.PaohuziHandCard;
import com.sy599.game.qipai.axphz254.bean.PaohuziHuBean;
...
//        System.out.println("玩家总牌数:"+j+" "+allCardStatics);
    ...
    public static synchronized List<List<PaohzCard>> fapai(List<Integer> copy, List<List<Integer>> t,int playerCount) {
        ...
        if (GameServerConfig.isDebug() && t != null && !t.isEmpty()) {
            ...
            for (List<Integer> zp : t) {
                list.add(find(copy2, zp));
                testcount += zp.size();
            }
            if (list.size() == 3) {
                list.add(toPhzCards(copy2));
            }
        }else {

            List<Integer> copy2 = new ArrayList<>(copy);
            int fapaiCount = 19 * 3 + 1 - testcount;
            if (pai.size() >= 20) {
                list.add(pai);
                pai = new ArrayList<>();
            }
    ...
    public static boolean allReFaPai(List<List<PaohzCard>> list,int playerCount){
        ...
        if(!isTianHu(list,playerCount))
            return false;
        
        return true;
    }
    
    public static boolean isTianHu(List<List<PaohzCard>> list,int playerCount){
    	for (int i = 0; i < playerCount; i++) {
            List<PaohzCard> cards = list.get(i);
            
          //红牌数量
            int redCardCount = Optional.ofNullable(PaohuziTool.findRedPhzs(cards)).orElse(Collections.emptyList()).size();
            if(redCardCount==0){
            	return false;
            }
    ...
    public static synchronized boolean checkShuangLong(List<List<PaohzCard>> list,int playerCount){
        for (int i = 0; i < playerCount; i++) {
            ...
            for (int j = 0; j < nums.length; j++) {
                ...
                if((j==0||j==9||j==10||j==19)&&nums[j]>=2){
                	sibian++;
                }
                
                if((j==1||j==6||j==9||j==11||j==16||j==19)&&nums[j]>=2){
                	redCount++;
                }
            }
            if (count>=2||count2>7||redCount>5||sibian>2||count2<3)
                return false;
        }
        return true;
    }
     ...
     * @return
     */
    public static synchronized boolean checkWuKan(List<List<PaohzCard>> list,int playerCount){
        for (int i = 0; i < playerCount; i++) {
            List<PaohzCard> cards = list.get(i);
            int []nums=new int[20];
            for (PaohzCard card:cards) {
...
//            //检测顺子or对子
    ...
    public static List<PaohzCard> getHuCardVals(List<Integer> phzIds, PaohzCard huCard) {
        List<PaohzCard> cards = new ArrayList<>();
        for (int id : phzIds) {
        	PaohzCard card = 	PaohzCard.getPaohzCard(id);
        	if(card.getVal()%100==huCard.getVal()%100){
        		cards.add(card);
        	}
        }
        return cards;
...
public static boolean  getTingZps(AxPaohuziPlayer player) {
	List<Integer> copy = new ArrayList<>(PaohuziConstant.cardList);
	for(Integer id: copy){
		PaohzCard tingC = PaohzCard.getPaohzCard(id);
		PaohuziHuLack lack = player.checkTingHu(tingC, true);
		if(lack.isHu()){
			return true;
		}
...
//        }
    ...
    public static List<PaohzCard> checkChiHu(List<PaohzCard> handCards, int kanVal) {
        int disVal = kanVal;
...
//    }
	...
	public static List<PaohzCard>  getTingZps(List<PaohzCard> handPais,AxPaohuziPlayer player) {
		AxPaohuziTable table = player.getPlayingTable(AxPaohuziTable.class);
		List<PaohzCard>  huCards = new ArrayList<PaohzCard> ();
		if(table==null){
			return huCards;
		}
		...
		for(Integer id : copy) {
			PaohzCard card =  PaohzCard.getPaohzCard(id);
			if(!checkHuCard(huCards, card)||handPais.contains(card)) {
				continue;
			}
			 // 检查胡
            PaohuziHuLack lack = player.checkHu(card, false);
	...
	private static boolean checkHuCard(List<PaohzCard>  huCards,PaohzCard card){
		if(huCards.size() ==0) {
			return true;
		}
		for(PaohzCard huC : huCards) {
...
//                if (disCard != null && val == disCard.getVal()) {
//                    if (!isPaoHu) {
                      int  action = PaohzDisAction.action_kan;
                        if (val > 100) {
                            // 坎大的6分
                            huxi = 6;
                        } else {
...
//                handCardBean.getOperateCards().removeAll(cards);
        ...
        return checkHuAfterKan(handCardBean, disCard, needDui, gameModel, lack, arr);
    }
	...
	private static PaohuziHuLack checkHuAfterKan(PaohuziHandCard handCardBean, PaohzCard disCard, boolean needDui,
			GameModel gameModel, PaohuziHuLack lack, PaohzCardIndexArr arr) {
		//四张牌型包含的数量
        ...
        if (lack.isNeedDui()) {
            ...
            if (duiZi != null && duiZi.getPaohzValMap().size() > 0) {
                for (Integer val : duiZi.getPaohzValMap().keySet()) {
                    ...
                    int huxi= 0;
                    if(duiZiCardList.get(0).isRed()){
                    	huxi =2;
                    }
                    
                    newLack.addPhzHuCards(0, duiZiIdList, huxi);
            ...
            if (sanZhang != null && sanZhang.getPaohzValMap().size() > 0) {
                for (Integer val : sanZhang.getPaohzValMap().keySet()) {
                    ...
                    int act = 0;
                    if(duiZiIdList.size()==2){
                    	  if(duiZiCardList.get(0).isRed()){
                          	huxi =2;
                          }
                    }else{
                    	act = PaohzDisAction.action_kan;
                    	 if(duiZiCardList.get(0).isRed()){
                           	huxi =4;
                          }else{
                        	  huxi =3;
                    ...
                    PaohuziHuLack newLack = lack.clone();
                    newLack.setNeedDui(false);
                    newLack.addPhzHuCards(act, duiZiIdList, huxi);
                    if (handPaisCopy.size() == 0) {
                        newLack.setHu(true);
                        huList.add(newLack);
                    } else {
            ...
            for (int[] paiZu : paiZus) {
                ...
                if (disCard != null && disCard.getVal() == val && isSameCard(rmList)) {
                    ...
                    newLack.addPhzHuCards(PaohzDisAction.action_peng, toPhzCardIds(rmList), disCard.isRed() ? 3 : 2);
                } else {
                	
                	if(isSameCard(rmList)){
                		int huxi = 3;
                		 if(rmList.get(0).isRed()){
                            	huxi =4;
                           }
