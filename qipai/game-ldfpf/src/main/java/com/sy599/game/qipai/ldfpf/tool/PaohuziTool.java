package com.sy599.game.qipai.ldfpf.tool;

import com.alibaba.fastjson.JSON;
import com.sy599.game.GameServerConfig;

import com.sy599.game.qipai.ldfpf.been.*;
...
public class PaohuziTool {
    public static List<Integer> remove(List<Integer> cardList, int [] cardNum){
        List<Integer> list=new ArrayList<>();
        for (int i = 0; i < cardList.size(); i++) {
            boolean flag=false;
            for (int j = 0; j <cardNum.length ; j++) {
                if (cardList.get(i)==cardNum[j]){
                    flag=true;
                    break;
                }
            }
    ...
    public static List<List<PaohzCard>> zuopai(List<Integer> cards, List<List<Integer>> t,int playerCount){
...
//            list.add(l);
    ...
    public static synchronized List<List<PaohzCard>> fapaiControl(List<Integer> copy, List<List<Integer>> t,int fifteenCards,int playerCount,int fapaiNum) {
        List<List<PaohzCard>> list = fapai(copy, t, fifteenCards, playerCount);
        fapaiNum++;
        if(fapaiNum>=10)
            return list;
        for (int j = 0; j < playerCount; j++) {
            ...
            if(num>=8||num<=4){
                ...
                if(rand.nextInt(10)<5)
                    return fapaiControl(copy,t,fifteenCards,playerCount,fifteenCards);
            }
        }
        return list;
    }
    ...
    public static synchronized List<List<PaohzCard>> fapai(List<Integer> copy, List<List<Integer>> t,int fifteenCards,int playerCount) {
        ...
        if(allReFaPai(list,playerCount)){
            ...
            return fapai(copy,null,0,playerCount);
        }
    }
     ...
     * @param list
     * @param playerCount
     * @return true 可以发牌，false 不能发牌
     */
    public static synchronized boolean checkDiHu(List<List<PaohzCard>> list,int playerCount){
        List<PaohzCard> banker = list.get(0);
        for (int i = 1; i < playerCount; i++) {
            List<PaohzCard> cards = list.get(i);
            for (int j = 0; j < banker.size(); j++) {
                List<PaohzCard> copy=new ArrayList<>(cards);
                copy.add(banker.get(j));
                List<PaohuziHuLack> hulist = isHuNew1(PaohuziTool.getPaohuziHandCardBean(copy), banker.get(j), false, false, true);
    ...
    public static synchronized boolean checkTianHu(List<List<PaohzCard>> list){
        List<PaohzCard> banker = list.get(0);
        List<PaohuziHuLack> hulist = isHuNew1(PaohuziTool.getPaohuziHandCardBean(banker), null, false, false, true);
        for (PaohuziHuLack hu:hulist){
            //跑胡不可能为飘胡
            hu.setHuxi(hu.calcHuxi());
            if (hu.getHuxi() >= 15||hu.getHuxi()==0){
    ...
    private static List<PaohzCard> find(List<Integer> zp){
        List<PaohzCard> cards=new ArrayList<>();
        for (int i = 0; i < zp.size(); i++) {
            cards.add(PaohzCard.getPaohzCard(zp.get(i)));
        }
        return cards;
    }
     ...
     * 是否胡牌
     */
    public static PaohuziHuLack isHu(PaohuziHandCard handCardBean, PaohzCard disCard, boolean isSelfMo, int outCardHuxi, boolean needDui, boolean isPaoHu) {
...
//				lack.addPhzHuCards(PaohzDisAction.action_zai, toPhzCardIds(cards), huxi);
    ...
    public static PaohuziHuLack isHuNew(PaohuziHandCard handCardBean, PaohzCard disCard, boolean isSelfMo, int outCardHuxi, boolean needDui, boolean isPaoHu,boolean isPiaoHu) {
        ...
        if (handPais.size() == 0) {
            ...
            if (huList.size() > 0) {
                return getMaxHuxi(huList,outCardHuxi,isPiaoHu);
            }
        }
        return lack;
    }
    ...
    public static PaohuziHuLack getMaxHuxi(List<PaohuziHuLack> huList,int outCardHuxi,boolean isPiaoHu) {
        ...
        for (PaohuziHuLack hu : huList) {
            ...
            hu.setHuxi(huxi);
            int allHuxi=outCardHuxi+huxi;
            if (allHuxi > maxHuxi||(isPiaoHu&&allHuxi==0)) {
                maxHuxi = allHuxi;
                maxHu = hu;
            }
        }
    ...
    //此处一块匾只考虑手牌的影响，只有手牌才能有多种组合组成不同牌型
    private  static boolean isYiKuaiBian(PaohuziHuLack huLack){
        List<PaohzCard> allCards = new ArrayList<>();
        for (CardTypeHuxi type : huLack.getPhzHuCards()) {
            allCards.addAll(PaohuziTool.toPhzCards(type.getCardIds()));
        }
        //判定红黑堂
        List<PaohzCard> redCardList = findRedPhzs(allCards);
        int count=0;
        switch (redCardList.size()){
            case 2://只可能为手牌剩下的一对
                for (CardTypeHuxi type : huLack.getPhzHuCards()){
    ...
    public static String getFapaiIds(List<List<PaohzCard>> list){
        StringBuilder s=new StringBuilder();
        for (int i = 0; i < list.size(); i++) {
            List<PaohzCard> cards=list.get(i);
            for (int j = 0; j < cards.size(); j++) {
                s.append(cards.get(j).getId());
                if(j==cards.size()-1&&i<list.size()-1){
                    s.append("_");
                }else {
                    s.append(",");
                }
