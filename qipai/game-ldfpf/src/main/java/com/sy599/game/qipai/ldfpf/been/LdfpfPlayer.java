...
import com.sy599.game.qipai.ldfpf.command.PaohuziCommandProcessor;
import com.sy599.game.qipai.ldfpf.constant.EnumHelper;
...
public class LdfpfPlayer extends Player {
    // 座位id
    private int seat;
    // 状态
    private player_state state;// 1进入 2已准备 3正在玩 4已结束
    private int isEntryTable;
    private List<Integer> originalHandPaiIds;
    ...
    //是否已经收到打鸟信息
    private boolean alreadyDaniao=false;
    //是否打鸟 0未打鸟，其他打鸟：如果打鸟，则代表打鸟分
    private int isDaniao=0;
    //鸟分 -1 未打鸟，0，不打鸟，1，翻倍打鸟，其他，打鸟分
    private int niaoFen=-1;
    //是否弃胡
    private int lockHand=0;
    //大结算时的分数<userId,与该userId之间的输赢分数>
    private int lastPoint;
    //首局洗牌的选择  -1 未选, 0不洗, 1洗
    private int firstXipai=-1;
    public LdfpfPlayer() {
        handPais = new ArrayList<>();
        originalHandPaiIds = new ArrayList<>();
    ...
    public int getLockHand() {
        return lockHand;
    }

    public void setLockHand(int lockHand) {
        this.lockHand = lockHand;
    }
    ...
    public void changeLastPoint(int num){
        this.lastPoint+=num;
    }

    public void setLastPoint(int num){
        lastPoint=num;
    }
...
//            }
    ...
    public PaohuziHuLack checkPaoHu(PaohzCard card, boolean isSelfMo, boolean canDiHu) {
        ...
        if (card != null) {
            LdfpfTable table = getPlayingTable(LdfpfTable.class);
...
//            return null;
        ...
        boolean isQishouTi=getPlayingTable(LdfpfTable.class).getFirstCard()&&isTiContian(card);
    ...
    public PaohuziHuLack checkHuNew(PaohzCard card, boolean isSelfMo) {
        ...
        if (list!=null&&!list.isEmpty()){
            for(PaohuziHuLack hu:list){
                ...
                if (allHuxi >= table.getFloorValue()||((table.isPiaoHu()&&allHuxi==0))){
    ...
    public PaohuziCheckCardBean  checkCard(PaohzCard card, boolean isSelfMo, boolean isPassHu, boolean isBegin, boolean isFirstCard, boolean isPass) {
        ...
        if (card != null) {
            ...
            if (isCanChiPeng && !check.isTi() && !check.isPao() && isCheckChi && !isPass) {
                ...
                LdfpfPlayer frontPlayer = (LdfpfPlayer) table.getPlayerBySeat(table.calcFrontSeat(seat));
    ...
    public List<Integer> toShowOutPais(List<PaohzCard> pias) {
        if (pias == null) {
            return Collections.emptyList();
        }

        List<Integer> ids = new ArrayList<>(pias.size());
        for (PaohzCard phz : pias) {
            if (originalHandPaiIds.contains(phz.getId())) {     //属于手牌的*-1
                ids.add(phz.getId() * -1);
            } else {
                ids.add(phz.getId());
            }
    ...
    public String buildHandPaiStr() {
        ...
        sb.append(StringUtil.implode(this.originalHandPaiIds)).append(";");
        return sb.toString();
    }
    ...
    public String toInfoStr() {
        ...
        sb.append(alreadyDaniao?1:0).append(",");
        sb.append(isDaniao).append(",");
        sb.append(zaiTiHu?1:0).append(",");
        sb.append(siShou?1:0).append(",");
        sb.append(lockHand).append(",");
     ...
     * 单局详情
     */
    public ClosingPhzPlayerInfoRes.Builder bulidOneClosingPlayerInfoRes(boolean isOver, int winLossPoint,int allHuxi) {
        ClosingPhzPlayerInfoRes.Builder res = ClosingPhzPlayerInfoRes.newBuilder();
        res.setUserId(userId + "");
        res.addAllCards(getHandPais());
        res.setName(name);
        res.setPoint(point);
        res.setTotalPoint(winLossPoint);
        res.setSeat(seat);
        res.setBopiPoint(0);
        res.setAllHuxi(allHuxi);
     ...
     * @return
     */
    public ClosingPhzPlayerInfoRes.Builder bulidTotalClosingPlayerInfoRes( boolean isOver, int winLossPoint,int allHuxi) {
        ClosingPhzPlayerInfoRes.Builder res = bulidOneClosingPlayerInfoRes( isOver, winLossPoint,allHuxi);
...
//        res.addAllOutedIds(toShowOutPais(this.outPais));

        if (seat==table.getShuXingSeat()){
            LdfpfPlayer paohuziPlayer = (LdfpfPlayer)table.getSeatMap().get(table.getLastWinSeat());
        ...
        if (state == player_state.play && table.getDaNiaoType() > 0) {
            res.addExt(isAlreadyDaniao() ? 1 : 0);//8
        } else {
            res.addExt(-1);//8
        }
        res.addExt(table.getDaNiaoType());//9
        res.addExt(getIsDaniao());//10打鸟分
        res.addExt(getLockHand());//11是否锁住手牌
        res.addExt(getNiaoFen());//12打鸟分
        int isGm = 0;
        if (table.isGroupRoom()) {
            String groupId = table.loadGroupId();
            GroupUser groupUser = this.getGroupUser();
            if(groupUser == null || !groupId.equals(groupUser.getGroupId() + "")) {
                groupUser = this.loadGroupUser(groupId);
            }
            if (groupUser != null && groupUser.getIsSpy() == 2) {
                isGm = 1;
            }
        }
        res.addExt(isGm);//13是否 spy2
        res.addExt(firstXipai);//14首局洗牌的选择
        res.setName(name);
        res.setSeat(seat);
        res.setSex(sex);
        ...
        StringBuilder sb = new StringBuilder("");
        for (Integer id : originalHandPaiIds) {
            sb.append(id);
            sb.append(',');
        }
        if (sb.length() > 0) {
        ...
        res.addStrExts(sb.toString());      //原始手牌
    ...
    public void initPais(String handPai, String outPai) {
        ...
        if (!StringUtils.isBlank(handPai)) {
            ...
            String originalHandPaiIdStr = StringUtil.getValue(values, i++);
            ...
            if(!StringUtil.isBlank(originalHandPaiIdStr)) {
                String[] ids = originalHandPaiIdStr.split(",");
                originalHandPaiIds.clear();
                for (String id : ids) {
                    originalHandPaiIds.add(Integer.valueOf(id));
                }
            }
        }
    }
    ...
    public void dealOriginalHandPaiIds(List<PaohzCard> list) {
        this.originalHandPaiIds = PaohuziTool.toPhzCardIds(list);
    }
    ...
    public boolean isAlreadyDaniao() {
        return alreadyDaniao;
    }

    public void setAlreadyDaniao(boolean alreadyDaniao) {
        this.alreadyDaniao = alreadyDaniao;
        changeTableInfo();
    }
    ...
    public List<PhzHuCards> buildPhzCards(LdfpfTable table,long lookUid) {
        List<PhzHuCards> list = new ArrayList<>();
        for (CardTypeHuxi type : cardTypes) {
            if (type.getAction() == PaohzDisAction.action_zai) {
                // 不是本人并且是栽
    ...
    public void setIsDaniao(int isDaniao) {
        this.isDaniao = isDaniao;
    }

    public int getIsDaniao() {
        return isDaniao;
    }
    ...
    public int getNiaoFenNonnegative() {
        if(niaoFen>=0)
            return niaoFen;
        return 0;
    }
