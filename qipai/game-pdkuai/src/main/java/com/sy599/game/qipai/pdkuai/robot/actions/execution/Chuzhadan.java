...
/** ExecutionAction class created from MMPM action Chuzhadan. */
public class Chuzhadan extends jbt.execution.task.leaf.action.ExecutionAction {

	/**
	 * Constructor. Constructs an instance of Chuzhadan that is able to run a
	 * com.sy599.game.qipai.pdkuai.robot.actions.Chuzhadan.
	 */
	public Chuzhadan(
			com.sy599.game.qipai.pdkuai.robot.actions.Chuzhadan modelTask,
...
//		LogUtil.printDebug(this.getClass().getCanonicalName() + " spawned");
	...
	protected jbt.execution.core.ExecutionTask.Status internalTick() {
		...
		List<List<Integer>> santiaos = allPaixing.get(CardTypeTool.px_zhadan);
		if(santiaos.isEmpty()){
		...
		List<Integer> pais = CardTypeTool.getPaisByPaiXing(CardTypeTool.px_zhadan, Arrays.asList(allSantiao.get(allSantiao.size()-1)), handCards);
		if(pais.isEmpty()){
