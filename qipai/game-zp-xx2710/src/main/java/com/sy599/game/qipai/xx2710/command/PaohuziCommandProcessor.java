package com.sy599.game.qipai.xx2710.command;
...
import com.sy599.game.qipai.xx2710.command.com.PaohuziComCommand;
import com.sy599.game.qipai.xx2710.command.play.PlayCommand;
...
public class PaohuziCommandProcessor extends AbstractBaseCommandProcessor {
	...
	public void process(Xx2710Player player, MessageUnit message) {
		int code = 0;
		try {
	...
	public void process(Player player, MessageUnit message) {
		...
		int code = 0;
		try {
			Xx2710Player pdkPlayer = player.getPlayer(processor);
			BaseCommand action = ObjectUtil.newInstance(commandMap.get(message.getMsgType()));
			action.setPlayer(player);
