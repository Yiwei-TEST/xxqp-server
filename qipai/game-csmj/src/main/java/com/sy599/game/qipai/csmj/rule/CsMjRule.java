...
public class CsMjRule {
	 ...
	 *
	 */
	public static void checkXiaoHu3(CsMjiangHu hu, List<CsMj> majiangIds, boolean begin, CsMjTable table) {
		int playerNum = 4;
		if (table != null) {
			playerNum = table.getMaxPlayerCount();
		}
		CsMjAction actionData = new CsMjAction();
		CsMjIndexArr card_index = new CsMjIndexArr();
		CsMjQipaiTool.getMax(card_index, majiangIds);
		if (begin) {
			boolean findjiang = false;
			List<Integer> seList = new ArrayList<>(Arrays.asList(1, 2, 3));
			Map<Integer, List<CsMj>> huaseListMap = new HashMap<>();
			for (CsMj majiang : majiangIds) {
				...
				if (huaseListMap.containsKey(huase)) {
					List<CsMj> list = huaseListMap.get(huase);
					list.add(majiang);
			...
			List<CsMj> jiangMjs = new ArrayList<>();
			...
			if (jiangMjs.size() == 1 && jiangMjs.get(0).getVal() % 10 == 5) {
				...
				for (List<CsMj> list : huaseListMap.values()) {
			...
			CsMjIndex index3 = card_index.getMajiangIndex(3);
			...
			if (jinTongYuNuMjVals.size() == 2) {
				...
				hu.setShowMajiangs(CsMjQipaiTool.findMajiangByValsAndCount(majiangIds, vals, 2));
			}
			...
			if (!jiejieGaoMjVals.isEmpty()) {
				...
				hu.setShowMajiangs(CsMjQipaiTool.findMajiangByVals(majiangIds, vals));
			}
	 ...
	 *
	 */
	public static void checkXiaoHu2(CsMjiangHu hu, List<CsMj> majiangIds, boolean begin, CsMjTable table) {
		int playerNum = 4;
		if (table != null) {
			playerNum = table.getMaxPlayerCount();
		}
		...
		if (begin) {
			...
			if (!seList.isEmpty()) {
				...
				hu.setQueyise(true);
				hu.setShowMajiangs(majiangIds);
				hu.addXiaoHu(CsMjAction.QUEYISE, new HashMap<Integer, List<CsMj>>());
			}
			// 板板胡 起牌后玩家手上没有一张258将牌
			if (!findjiang) {
				actionData.addBanBanHu();
				hu.setBanbanhu(true);
				hu.setShowMajiangs(majiangIds);
				hu.addXiaoHu(CsMjAction.BANBANHU, new HashMap<Integer, List<CsMj>>());
			}
			// 一枝花：满足以下任意一种情况：起牌后，玩家手牌中只有一张将牌，且这张将牌数字为“5”，即可胡牌（等同小胡自摸）。起牌后，玩家手牌中某花色只有一张，且这张将牌数字为“5”，即可胡牌（等同小胡自摸）。
			if (yizhihua) {
				actionData.addYiZhiHua();
				hu.setYizhihua(true);
				hu.addXiaoHu(CsMjAction.YIZHIHUA, new HashMap<Integer, List<CsMj>>());
				hu.setShowMajiangs(majiangIds);
			}
			...
			if (index3 != null) {
				actionData.addDaSiXi();
				hu.addXiaoHu(CsMjAction.DASIXI, index3.getMajiangValMap());
				hu.setDasixi(true);
				hu.setShowMajiangs(index3.getMajiangs());
			...
			if (count3 > 1) {
				...
				if (index2 != null) {
					...
					Map<Integer, List<CsMj>> map2 = index2.getMajiangValMap();
					for (Entry<Integer, List<CsMj>> entry : map2.entrySet()) { 
//						if(map2.size()<2){
						if(table.checkXiaoHuCards(CsMjAction.ZHONGTULIULIUSHUN,entry.getKey())){
							continue;
						}
...
//						}
				...
				if(index3!=null){
					Map<Integer, List<CsMj>> map3 = index3.getMajiangValMap();
					for (Entry<Integer, List<CsMj>> entry : map3.entrySet()) { 
//						if(map2.size()<2){
						if(table.checkXiaoHuCards(CsMjAction.ZHONGTULIULIUSHUN,entry.getKey())){
							continue;
						}
...
//						}
				...
				if(map.size()>1){
					int count = 0;
					Map<Integer, List<CsMj>> map2 = new HashMap<>();
					int key = 0;
					for (Entry<Integer, List<CsMj>> entry : map.entrySet()) { 
						count++;
						if (count % 2 == 1) {
							key = entry.getKey();
							if (count3 == 3 && count == 3) {
							...
							List<CsMj> list = map2.get(key);
							if (list != null) {
								list.addAll(entry.getValue());
							}
						}
					}
					hu.addXiaoHu(CsMjAction.LIULIUSHUN, map2);
				}
			...
			if (jinTongYuNuMjVals.size() == 2) {
				...
				List<CsMj> list = CsMjQipaiTool.findMajiangByValsAndCount(majiangIds, vals, 2);
				Map<Integer, List<CsMj>> map2 = new HashMap<>();
				map2.put(list.get(0).getVal(), list);
				hu.addXiaoHu(CsMjAction.JINGTONGYUNU, map2);

				// 删除掉
				//majiangIds.removeAll(list);
				card_index = new CsMjIndexArr();
				CsMjQipaiTool.getMax(card_index, majiangIds);
				// hu.setShowMajiangs();
			}
			...
			if (!jiejieGaoMjVals.isEmpty()) {
				...
				Map<Integer, List<CsMj>> map2 = new HashMap<>();
				map2.put(list.get(0).getVal(), list);
				hu.addXiaoHu(CsMjAction.JIEJIEGAO, map2);
				...
				if (!jiejieGaoMjVals.isEmpty()) {
					...
					List<CsMj> list2 = CsMjQipaiTool.findMajiangByValsAndCount(majiangIds, vals2, 2);
			...
			if (!sanTongMjVals.isEmpty()) {
				...
				Map<Integer, List<CsMj>> map2 = new HashMap<>();
				map2.put(list.get(0).getVal(), list);
				hu.addXiaoHu(CsMjAction.SANTONG, map2);
			...
			if (index3 != null) {
				...
				hu.setZhongtusixi(true);
				hu.setShowMajiangs(index3.getMajiangs());
				hu.addXiaoHu(CsMjAction.ZHONGTUSIXI, index3.getMajiangValMap());
				//card_index = removeMj(majiangIds, index3);
			}
...
//						}
				...
				if(map.size()>1){
					...
					hu.addXiaoHu(CsMjAction.ZHONGTULIULIUSHUN, map2);
				}
	...
	private static CsMjIndexArr removeMj(List<CsMj> majiangIds, CsMjIndex index3) {
		CsMjIndexArr card_index;
		List<CsMj> list = new ArrayList<CsMj>();
		// list.addAll(index3.getMajiangValMap().values());
		for (List<CsMj> mjs : index3.getMajiangValMap().values()) {
			list.addAll(mjs);
		}
	...
	public static Set<Integer> getJinTongYuNv(CsMjIndexArr card_index) {
		Set<Integer> jinTongYuNuMjVals = new HashSet<>();
		for (int i = 1; i <= 3; i++) {
			CsMjIndex jinTongYuNuIndex = card_index.getMajiangIndex(i);// 对子数量
	...
	public static Set<Integer> getJieJieGao(CsMjIndexArr card_index) {
		Set<Integer> jiejieGaoMjVals = new HashSet<>();
		Set<Integer> rset = new HashSet<>();
		Map<Integer, Set<Integer>> huaseMap = new HashMap<>();
		for (int i = 1; i <= 3; i++) {
			CsMjIndex jiejieGaoIndex = card_index.getMajiangIndex(i);// 对子数量
	...
	public static Set<Integer> getSantong(CsMjIndexArr card_index) {
		Set<Integer> sanTongMjVals = new HashSet<>();
		Map<Integer, Set<Integer>> valMap = new HashMap<>();
		for (int i = 1; i <= 3; i++) {
			CsMjIndex sanTongIndex = card_index.getMajiangIndex(i);// 对子数量
	 ...
	 * @return
	 */
	public static int[] checkDahu(CsMjiangHu hu, List<CsMj> majiangIds, List<CsMj> gang, List<CsMj> peng,
			List<CsMj> chi, List<CsMj> buzhang) {
		List<CsMj> allMajiangs = new ArrayList<>();
		...
		CsMjIndexArr all_card_index = new CsMjIndexArr();
		CsMjQipaiTool.getMax(all_card_index, allMajiangs);
		...
		int hongzhongNum = CsMjTool.dropHongzhong(copy).size();
		...
		if (card_index.getDuiziNum() == 6 && hongzhongNum > 0) {
			...
			// 是否有豪华7小对
			CsMjIndex index = card_index.getMajiangIndex(3);
			if (index != null) {
				// 有4个一样的牌
	...
	private static boolean isqingyiseHu(List<CsMj> allMajiangs, List<CsMj> majiangIds) {
		boolean qingyise = checkqingyise(allMajiangs);
		if (qingyise) {
			boolean hu = CsMjTool.isPingHu(majiangIds, false);
			return hu;
		}
	...
	public static boolean checkqingyise(List<CsMj> allMajiangs) {
		boolean qingyise = false;
		int se = 0;
		for (CsMj mjiang : allMajiangs) {
	...
	private static boolean isJiangJiangHu(List<CsMj> majiangIds) {
		boolean jiangjianghu = true;
		for (CsMj mjiang : majiangIds) {
			if (mjiang.isHongzhong()) {
				continue;
			}
			if (!mjiang.isJiang()) {
	...
	private static boolean isPengPengHu(List<CsMj> majiangIds, CsMjIndexArr card_index) {
		CsMjIndex index4 = card_index.getMajiangIndex(3);
		CsMjIndex index3 = card_index.getMajiangIndex(2);
		CsMjIndex index2 = card_index.getMajiangIndex(1);
		CsMjIndex index1 = card_index.getMajiangIndex(0);
		...
		if (sameCount == 4 && index2 != null && index2.getLength() == 1) {
			return true;
		} else if (majiangIds.contains(CsMj.getMajang(201)) && index4 != null && index4.getLength() == 2
				&& index1 != null && index1.getLength() == 1 && index2 == null) {
			return true;
		} else if (majiangIds.contains(CsMj.getMajang(201)) && sameCount >= 3 && index2 == null && index1 != null
				&& index1.getLength() == 2) {
			return true;
		} else if (majiangIds.contains(CsMj.getMajang(201)) && sameCount >= 2 && index2 != null
