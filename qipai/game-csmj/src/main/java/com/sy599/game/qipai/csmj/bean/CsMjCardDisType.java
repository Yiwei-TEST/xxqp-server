...
//	}
	...
	public boolean isHasCard(CsMj card) {
		return cardIds != null && cardIds.contains(card.getId());
	}

	public boolean isHasCardVal(CsMj card) {
		if (cardIds != null) {
			List<CsMj> majiangs = CsMjHelper.toMajiang(cardIds);
			List<Integer> vals = CsMjHelper.toMajiangVals(majiangs);
	...
	public int removeCardVal(int val) {
		if (cardIds != null) {
			...
			while (iterator.hasNext()) {
				...
				CsMj majiang = CsMj.getMajang(id);
