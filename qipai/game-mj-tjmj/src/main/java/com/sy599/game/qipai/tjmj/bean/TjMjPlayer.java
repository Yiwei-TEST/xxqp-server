...
import com.sy599.game.qipai.tjmj.command.MjCommandProcessor;
...
import com.sy599.game.qipai.tjmj.tool.MjEnumHelper;
...
@Data
public class TjMjPlayer extends Player {
    ...
    public List<Mj> showXiaoHuMajiangs(int xiaohu, boolean connect) {
        TjMjTable table = getPlayingTable(TjMjTable.class);
        MjiangHu hu = new MjiangHu();
        List<Mj> copy2 = new ArrayList<>(getHandMajiang());
        MjRule.checkXiaoHu2(hu, copy2, true, table, this);

        HashMap<Integer, Map<Integer, List<Mj>>> map = hu.getXiaohuMap();
        Map<Integer, List<Mj>> cardMap = map.get(xiaohu);
        if (cardMap == null) {
            if (xiaohu == MjAction.ZHONGTUSIXI) {
                xiaohu = 10;
            } else if (xiaohu == MjAction.ZHONGTULIULIUSHUN) {
                xiaohu = 9;
            }
        ...
        if (xiaohu == 6) {// 缺一色
            ...
            return handPais;
        } else if (xiaohu == 10 || xiaohu == MjAction.ZHONGTUSIXI) {// 大四喜|| xiaohu==

            return getShowCards(cardMap, connect);
        } else if (xiaohu == 9 || xiaohu == MjAction.ZHONGTULIULIUSHUN) {// 六六顺
            return getShowCards(cardMap, connect);
        } else if (xiaohu == MjAction.SANTONG) {// 三同
            return getShowCards(cardMap, connect);
        } else if (xiaohu == MjAction.JINGTONGYUNU) {// 金童玉女
            return getShowCards(cardMap, connect);
        } else if (xiaohu == MjAction.JIEJIEGAO) {// 节节高
            return getShowCards(cardMap, connect);
        } else {
...
//		}
    ...
    public List<Mj> getCanChiMajiangs(Mj disMajiang) {
        return MjTool.checkChi(handPais, disMajiang, getPlayingTable(TjMjTable.class));
    }

//    public boolean isTingPai(int val, boolean needRemoveThisVal) {
//		return isTingPai(val, false, needRemoveThisVal, true);
//	}

    public boolean isTingPai(int val, boolean needRemoveThisVal, boolean noYingZhuangNoHu) {
        return isTingPai(val, false, needRemoveThisVal, noYingZhuangNoHu);
    }
     ...
     * @return
     */
    public boolean isTingPai(int val, boolean onlySevenDuiNoHu, boolean needRemoveThisVal, boolean noYingZhuangNoHu) {
        ...
        if (copy.size() % 3 != 2) {
            if (getPlayingTable(TjMjTable.class).getKingCard() != null) {
				if (((TjMjTable) table).getKingCard2() != null &&
						((TjMjTable) table).getPlayerMap().entrySet().stream().map(kv -> ((TjMjPlayer) kv.getValue()).getOutPais().stream().filter(v -> v == ((TjMjTable) table).getKingCard().getVal()).count()).count() >= 4) {
					copy.add(Mj.getMajang(getPlayingTable(TjMjTable.class).getKingCard2().getId()));
				} else {
					copy.add(Mj.getMajang(getPlayingTable(TjMjTable.class).getKingCard().getId()));
				}
            }else {
        ...
        MjiangHu huBean = MjTool.isHu(copy, gangList, copyPeng, chi, bzCopy, false, jiang258, table, handPais.get(0), this, false, 0, noYingZhuangNoHu);
        ...
        //仅地胡胡牌, 也不能听
		if ((huBean.getDahuCountAll() == 1 && (huBean.isDihu() || huBean.isDidihu()))) {
        	return false;
		}
    ...
    public List<Integer> checkDisCard(Mj majiang) {
		return checkDisCard(majiang, true, false, getPlayingTable(TjMjTable.class).isKingCard(majiang), true);
    }
     ...
     *@param haveKingIce 有没有冻结的王牌,意味着有王牌不能变了
     *@param noYingZhuangNoHu 非硬庄不能胡牌,默认true
     *@return
     *@author Guang.OuYang
     *@date 2019/10/28
     */
    public List<Integer> checkDisCard(Mj majiang, boolean need258, boolean gangHouMo, boolean haveKingIce, boolean noYingZhuangNoHu) {
        List<Integer> list = new ArrayList<>();
		...
		//王牌没有明杠暗杠
        boolean canGang = !table.isKingCard(majiang);
        if ((!table.moHu() || table.getDisCardSeat() == seat)/* && passMajiangVal != majiang.getVal()*/) {
			...
			MjiangHu hu = MjTool.isHu(copy, getGang(), peng, chi, bzCopy, false, jiang258, table, majiang, this, haveKingIce, -1, noYingZhuangNoHu);
            ...
            if (hu.isHu()) {
                ...
                //天胡特殊操作, 这里做限制用
                if (hu.isTianhu() || hu.isTiantianhu()) {
                    actData.addTianHu();
                }
...
//                }
        ...
        if (table.getDisCardSeat() != seat) {
            ...
            if (count == 3) {
                boolean isTing = isTingPai(majiang.getVal(), true, false);
                if (isTing) {
                    if (canGang)
                        actData.addMingGang();
            ...
            if (gangList.isEmpty()) {
                ...
                if (table.calcNextSeat(table.getDisCardSeat()) == seat) {
                    if (!(table.getMaxPlayerCount() == 2 && table.getGameModel().getSpecialPlay().isBuChi())) {
                        List<Mj> chi = MjTool.checkChi(handPais, majiang, getPlayingTable(TjMjTable.class));
                        if (!chi.isEmpty()) {
                            actData.addChi();
                        }
                    }
    ...
    public int getDahuScore() {
        return MjiangHu.calcMenZiScore(getPlayingTable(TjMjTable.class), getDahu(), getPlayingTable(TjMjTable.class).getGameModel().getSpecialPlay().selfMoEqGunHu());
    }
	...
	public MjiangHu checkHu(Mj disMajiang, boolean isbegin) {
		return checkHu2(disMajiang, isbegin, true);
	}

	public MjiangHu checkHu2(Mj disMajiang, boolean isbegin, boolean noYingZhuangNoHu) {
		return checkHu(disMajiang, isbegin, true, noYingZhuangNoHu);
	}

	public MjiangHu checkHu(Mj disMajiang, boolean isbegin, boolean haveKingIce) {
		return checkHu(disMajiang, isbegin, haveKingIce, true);
	}
	public MjiangHu checkHu(Mj disMajiang, boolean isbegin, boolean haveKingIce, boolean noYingZhuangNoHu) {
        List<Mj> copy = new ArrayList<>(handPais);
        if (disMajiang != null) {
            copy.add(disMajiang);
        }
		...
		//默认胡牌需要检测是否有不能用的王, 因为可能是点炮
        MjiangHu hu = MjTool.isHu(copy, getGang(), peng, chi, bzCopy, isbegin, jiang258, table, disMajiang, this, haveKingIce, -1, noYingZhuangNoHu);
        if (hu.isHu()) {
            if (table.getGameModel().getSpecialPlay().isMenqing() && !isChiPengGang()) {
                hu.setMenqing(true);
                hu.initDahuList();
...
//        try {
//            LogUtil.msgLog.info("tjmj_room_checkhu:{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{}",
//                    getPlayingTable(TjMjTable.class).getId(),
//                    getPlayingTable(TjMjTable.class).getPlayBureau(),
//                    disMajiang,
//                    this.getSeat(),
//                    this.getName(),
//                    copy,
...
//                    isbegin,
//                    jiang258,
//                    table.getDisCardRound(),
//                    getPlayingTable(TjMjTable.class).getActionSeatMap(),
//                    getPlayingTable(TjMjTable.class).getGangSeatMap(),
//                    getPlayingTable(TjMjTable.class).getHuConfirmMap(),
//                    hu,
//                    hu != null ? hu.buildDahuList() : null,
...
//        }
    ...
    public List<Integer> checkMoCard(Mj majiang, boolean isBegin) {
        ...
        if (noNeedMoCard() || majiang != null || isBegin) {
            ...
            List<Mj> copy = new ArrayList<>(handPais);
            MjiangHu hu = MjTool.isHu(copy, getGang(), peng, chi, bzCopy, isBegin, jiang258, table, majiang, this, false, 0);
            temp = hu;
//            canGang = hu.isHu() && !((!table.getGameModel().getSpecialPlay().isXiaoDuiGang() && (hu.isXiaodui() || hu.isHao7xiaodui() || hu.isShuang7xiaodui())) && hu.getDahuCount() == 1);
            ...
            //自摸控制
            if (hu.isHu() && getPlayingTable(TjMjTable.class).getGameModel().getSpecialPlay().isSelfMoHu()) {
                //二人麻将
                if (table.getMaxPlayerCount() == 2) {
                    if (table.getGameModel().getSpecialPlay().isOnlyDaHu()) {
                        if (table.getGameModel().getSpecialPlay().isMenqing() && !isChiPengGang()) {
...
//                for (int index : MjAction.xiaoHuPriority) {
//
//                    Map<Integer, List<Mj>> xiaoHuCardMap = xiaohuMap.get(index);
...
//            }
        ...
        if (!getPlayingTable(TjMjTable.class).checkSignTingInfo()) {
...
//        try{
//            LogUtil.msgLog.info("tjmj_room_selfMo:{},round:{},leftCard:{},play:{}->seat:{},opera:{}({},{}),{},dahuList:{},isHu:{},hand:{},stack:{}",
//                    getPlayingTableId(),
//                    getPlayingTable().getDisCardRound(),
//                    getPlayingTable(TjMjTable.class).getLeftMajiangs().size(),
//                    getUserId(),
//                    getSeat(),
//                    majiang,
//                    majiang!=null?majiang.getId():null,
//                    majiang!=null?majiang.getVal():null,
//                    list,
//                    temp != null ? temp.buildDahuList() : "null",
//                    temp != null ? temp.isHu() : "null",
//                    getHandMajiang(),
//                    ExceptionUtils.getStackTrace(new Throwable()).trim().replace("\r\n","/").substring(0, 480));
//        }catch(Exception e){
//        }
    ...
    private void checkGang(MjAction actData, TjMjTable table, boolean canXiaohu, boolean canGang) {
        if (noNeedMoCard() && !canXiaohu) {
            List<Mj> gangList = getGang();
            Map<Integer, Integer> pengMap = MjHelper.toMajiangValMap(peng);
            ...
            for (Mj handMajiang : handPais) {
                if (pengMap.containsKey(handMajiang.getVal())) {
                    // 有碰过
                    boolean isTing = isTingPai(handMajiang.getVal(), true,false);

                    canGang = !table.isKingCard(handMajiang);
            ...
            if (handMap.containsValue(4)) {
                for (Entry<Integer, Integer> entry : handMap.entrySet()) {
                    if (entry.getValue() == 4) {
                        boolean isTing = isTingPai(entry.getKey(), true,false);

                        canGang = !table.isKingCard(Mj.getMajangByVal(entry.getKey()));
                        if (isTing) {
                            if (canGang)
