...
public class PaohuziTool {
    ...
    public static synchronized List<List<PaohzCard>> fapaiControl(List<Integer> copy, List<List<Integer>> t,int cardNum,int playerCount,int bossNum) {
        List<Integer> copy1=new ArrayList<>(copy);
        List<List<PaohzCard>> list = fapai(copy1, t, cardNum, playerCount, bossNum);
        for (int j = 0; j < playerCount; j++) {
            ...
            if(num>=4)
                return fapaiControl(copy, new ArrayList<>(),cardNum,playerCount,bossNum);
        }
        return list;
    }
    
    public static synchronized List<List<PaohzCard>> fapai(List<Integer> copy, List<List<Integer>> t,int cardNum,int playerCount,int bossNum) {
        copy = copy.subList(0, copy.size() - (4-bossNum));
        List<List<PaohzCard>> list = new ArrayList<>();
     ...
     * @return
     */
    public static List<PaohuziHuLack> getAllHu(List<Integer> handCards, PaohzCard disCard, ZhzPlayer player,ZhzTable table){
    ...
    public static List<PaohuziHuLack> getAllPingHu(List<Integer> allCarcds,ZhzPlayer player,ZhzTable table){
        ...
        List<PaohuziHuLack> huList=new ArrayList<>();
        splitCardsGetAll(huList,new PaohuziHuLack(),toPhzCards(normal),toPhzCards(boss),boss.size());
        filtratePingHu(huList, player,table);
        return huList;
    }

    public static void splitCardsGetAll(List<PaohuziHuLack> huList,PaohuziHuLack lack,List<PaohzCard> normal,List<PaohzCard> boss,int bossNum){
        if((normal.size()+boss.size())%3==2)
            boss.add(PaohzCard.getPaohzCard(85));
        if(normal==null||normal.size()==0){
            addAllReplaceBoss(lack,huList,boss,bossNum);
            return;
        }
        ...
        for (int[] paiZu : paiZus) {
            ...
            if (normalCopy.size()==0&&bossCopy.size()==3){
                ...
                addAllReplaceBoss(newLack,huList,bossCopy,bossNum);
            }else if (normalCopy.size() +bossCopy.size()== 0) {
                //牌已组合完毕，需要删除最初加入的王牌,同时检测是否胡牌
                deleteBoss85All(newLack,huList,bossNum);
            } else {
                splitCardsGetAll(huList, newLack, normalCopy, bossCopy,bossNum);
            }
        }
    }

    private static void addAllReplaceBoss(PaohuziHuLack lack,List<PaohuziHuLack> huList,List<PaohzCard> boss,int bossNum) {
        ...
        for (Map.Entry<Integer,List<int[]>> entry:paiZuMap.entrySet()) {
            ...
            for (int[] aGroup:aCardGroup) {
                ...
                deleteBoss85All(clone,huList,bossNum);
            }
        }
    }
    ...
    private static void deleteBoss85All(PaohuziHuLack lack,List<PaohuziHuLack> huList,int bossNum) {
     ...
     * @return
     */
    public static boolean isHu(List<Integer> handCards, PaohzCard disCard, ZhzPlayer player,ZhzTable table,boolean canBbh,boolean isCheckTing){
        List<Integer> allCards=new ArrayList<>();
        allCards.addAll(handCards);
        if(disCard!=null&&!allCards.contains(disCard.getId()))
            allCards.add(disCard.getId());
        ...
        return isPingHu(allCards,player,table);
    }

    public static boolean isPingHu(List<Integer> allCarcds,ZhzPlayer player,ZhzTable table){
        if(allCarcds.size()%3!=2)
            return false;
        List<Integer> normal=new ArrayList<>();
        List<Integer> boss=new ArrayList<>();
        ...
        return splitCards(new PaohuziHuLack(),toPhzCards(normal),toPhzCards(boss),boss.size(),player,table);
    }

    public static boolean splitCards(PaohuziHuLack lack,List<PaohzCard> normal,List<PaohzCard> boss,int bossNum,ZhzPlayer player,ZhzTable table){
        if((normal.size()+boss.size())%3==2)
            boss.add(PaohzCard.getPaohzCard(85));
        if(normal.size()==0)
            return true;
        ...
        for (int[] paiZu : paiZus) {
            ...
            if (normalCopy.size() +bossCopy.size()== 0) {
                if(deleteBoss85(newLack,bossNum,player,table))
                    return true;
            } else {
                if(splitCards(newLack, normalCopy, bossCopy,bossNum,player,table))
                    return true;
            }
        }
        return false;
    ...
    private static boolean deleteBoss85(PaohuziHuLack lack,int bossNum,ZhzPlayer player,ZhzTable table) {
        List<CardTypeHuxi> huTypes = lack.getPhzHuCards();
        List<PaohuziHuLack> huList= new ArrayList<>();
        if(bossNum==0){
            for (int i = 0; i < huTypes.size(); i++) {
    ...
    private static List<PaohuziHuLack> filtratePingHu(List<PaohuziHuLack> huList,ZhzPlayer player,ZhzTable table){
    ...
    public static Map<Integer,List<Integer>> checkDisTing(List<Integer> handCards, ZhzPlayer player,ZhzTable table){
    ...
    public static List<Integer> checkTing(List<Integer> handCards, ZhzPlayer player,ZhzTable table){
    ...
    public static List<Integer> addResidueCards(ZhzTable table,List<Integer> handCards){
        List<Integer> residue =new ArrayList<>(PaohuziConstant.cardList);
        residue = residue.subList(0, residue.size() - (4-table.getBossNum()));
