...
//		return bl;
	...
	public static List<List<String>> getTsLiandui(List<String> sjp, List<String> zjp) {
		...
		String[] zjpary = new String[zjp2.size()];
		zjp2.toArray(zjpary);
		zjpary = HsthSfNew.sortAry(zjpary, "asc");
		if(isLiandui(sjp)) {
			HsthSfNew.combinationDz1(zjpary, 2, list, 2);
			int slen = sjp.size();
			int num = slen/2;
			if(slen <= zjp.size()) {
				HsthSfNew.combinationLd(list, num, ldlist, num);
			}
		}
		...
		for (int i = 0; i < ldlist.size(); i++) {
			...
			pary = HsthSfNew.sortAry(pary, "asc");
