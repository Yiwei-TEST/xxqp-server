...
//	}
	...
	public static int getCpfen(List<String> pai) {
		...
		int fen = 0;
		int[] ary = HsthSf.paiToShortAry(pai);
	...
	public static int getScoreCards(List<Integer> pai2) {
		List<String> pai = HsthSfNew.intCardToStringCard(pai2);
		if(null==pai || pai.size()==0){
			return 0;
		}
		int fen = 0;
	...
	public static int getXiqian(List<String> pai) {
		if(HsthSf.is8zha(pai)) {
			return 4*4;
		} else if(HsthSf.is7zha(pai)) {
			return 2*4;
		} else if(HsthSf.is6zha(pai)) {
			return 1*4;
		} else if(HsthSf.is4wang(pai)) {
			return 1*4;
		} else {
			return 0;
		}
