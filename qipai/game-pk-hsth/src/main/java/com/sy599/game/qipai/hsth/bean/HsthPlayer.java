...
import com.sy599.game.qipai.hsth.command.HsthCommandProcessor;
...
public class HsthPlayer extends Player {
...
//	全局总拿分
	...
	protected void loadFromDB0(RegInfo info) {
		if (UserDatasDao.getInstance().exists()){
			String val = UserDatasDao.getInstance().selectUserDataValue(String.valueOf(userId), HsthTable.GAME_CODE,"all","currentLs");
			if (CommonUtil.isPureNumber(val)){
				currentLs = Integer.parseInt(val);
			}else{
				UserDatasDao.getInstance().saveUserDatas(String.valueOf(userId), HsthTable.GAME_CODE,"all","currentLs","0");
			}

			val = UserDatasDao.getInstance().selectUserDataValue(String.valueOf(userId), HsthTable.GAME_CODE,"all","maxLs");
			if (CommonUtil.isPureNumber(val)){
				maxLs = Integer.parseInt(val);
			}else{
				UserDatasDao.getInstance().saveUserDatas(String.valueOf(userId), HsthTable.GAME_CODE,"all","maxLs","0");
			}
		}
	}
	...
	public void setAutoPlay(boolean autoPlay,BaseTable table) {
		if (this.autoPlay != autoPlay && !isRobot()){
			...
			HsthTable table2 = getPlayingTable(HsthTable.class);
        	...
        	table2.addPlayLog(table2.addSandhPlayLog(getSeat(), HsthConstants.action_tuoguan, val, false, 0, null, 0));
		}
		...
		if(addLog) {
			StringBuilder sb = new StringBuilder("Hsth");
			if (table != null) {
	...
	public void dealHandPais(List<Integer> pais, HsthTable table) {
		this.table = table;
		this.handPais = pais;
		...
		Collections.sort(pais, new Comparator<Integer>() {
			...
			public int compare(Integer o1, Integer o2) {
				return HsthUtil.loadCardValue(o2) - HsthUtil.loadCardValue(o1);
			}
		});
	...
	public void addOutPais(List<Integer> cards , HsthTable table) {
		this.table = table;
	...
	public void initPlayInfo(String data) {
		if (!StringUtils.isBlank(data)) {
			...
			xifen=  StringUtil.getIntValue(values, i++);
		}
	}
	...
	public PlayerInTableRes.Builder buildPlayInTableInfo(long recUserId, boolean isrecover) {
		...
		HsthTable table = getPlayingTable(HsthTable.class);
		if (table == null) {
		...
		int totalScore = HsthUtil.getCpfen(HsthSfNew.intCardToStringCard(chiFenCards));
		...
		extList.add(0);//9 托管倒计时
		extList.add(autoPlay ? 1 : 0);//10 是否托管状态
		extList.add(xifen);//11
...
//		}
	...
	public static final List<Integer> wanfaList = Arrays.asList(GameUtil.play_type_pk_hsth);
	...
	public static void loadWanfaPlayers(Class<? extends Player> cls){
		for (Integer integer:wanfaList){
			PlayerManager.wanfaPlayerTypesPut(integer,cls, HsthCommandProcessor.getInstance());
		}
	}
	...
	public void setXifen(int xifen) {
		this.xifen = xifen;
	}
	public void changeXifen(int xifen) {
		this.xifen += xifen;
		changeTableInfo();
	}
