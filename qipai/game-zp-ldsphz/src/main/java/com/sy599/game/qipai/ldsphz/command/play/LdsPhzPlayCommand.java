package com.sy599.game.qipai.ldsphz.command.play;
...
public class LdsPhzPlayCommand extends BaseCommand {
	@Override
	public void execute(Player player0, MessageUnit message) throws Exception {
		LdsPhzTable table = player0.getPlayingTable(LdsPhzTable.class);
		if (table == null) {
			return;
		}
		LdsPhzPlayer player=(LdsPhzPlayer)player0;
...
//		}
		...
		if (action == LdsPhzConstants.action_pao||action == LdsPhzConstants.action_mo){
			return;
		}
			...
			synchronized (table){
				...
				if (actionMap != null && (list=actionMap.get(player.getSeat()))!=null) {
					...
					if (list.get(7).intValue() == 1 || list.get(8).intValue() == 1 || list.get(9).intValue() == 1) {
						...
						LogUtil.monitorLog.info("action pass wang diao or chuang or zha:userId="+player.getUserId()+",tableId="+table.getId()+",card="+nextCard+",cardVal="+ LdsPhzCardUtils.loadCardVal(nextCard)+",wang action="+list.get(7)+"_"+list.get(8)+"_"+list.get(9));
					}else{
					...
					if (list.get(0).intValue() == 1) {
						LogUtil.monitorLog.info("action pass hu:userId="+player.getUserId()+",tableId="+table.getId()+",card="+table.getHuCard()+",cardVal="+ LdsPhzCardUtils.loadCardVal(table.getHuCard()));
					}
				}
			}
			if (!cards.isEmpty()) {
