package com.sy599.game.qipai.hsphz.command;
...
import com.sy599.game.qipai.hsphz.command.com.ComCommand;
import com.sy599.game.qipai.hsphz.command.play.PlayCommand;
...
public class CommandProcessor extends AbstractBaseCommandProcessor {
    ...
    public void process(HsphzPlayer player, MessageUnit message) {
        int code = 0;
        try {
    ...
    public void process(Player player, MessageUnit message) {
        ...
        int code = 0;
        try {
            HsphzPlayer pdkPlayer = player.getPlayer(processor);
            BaseCommand action = ObjectUtil.newInstance(commandMap.get(message.getMsgType()));
            action.setPlayer(player);
