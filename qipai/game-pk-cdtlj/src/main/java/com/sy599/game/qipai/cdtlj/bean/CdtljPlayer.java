...
import com.sy599.game.qipai.cdtlj.command.CdtljCommandProcessor;
...
public class CdtljPlayer extends Player {
	...
	/**叫分阶段叫的分 -1：还没叫 0不叫 */
	private int jiaoZhu=-1;
	private long auto_giveUpQzTime =0;
	...
	protected void loadFromDB0(RegInfo info) {
		if (UserDatasDao.getInstance().exists()){
			String val = UserDatasDao.getInstance().selectUserDataValue(String.valueOf(userId),CdtljTable.GAME_CODE,"all","currentLs");
			if (CommonUtil.isPureNumber(val)){
				currentLs = Integer.parseInt(val);
			}else{
				UserDatasDao.getInstance().saveUserDatas(String.valueOf(userId),CdtljTable.GAME_CODE,"all","currentLs","0");
			}

			val = UserDatasDao.getInstance().selectUserDataValue(String.valueOf(userId),CdtljTable.GAME_CODE,"all","maxLs");
			if (CommonUtil.isPureNumber(val)){
				maxLs = Integer.parseInt(val);
			}else{
				UserDatasDao.getInstance().saveUserDatas(String.valueOf(userId),CdtljTable.GAME_CODE,"all","maxLs","0");
			}
		}
	}
	...
	public void setAutoPlay(boolean autoPlay,BaseTable table) {
		if (this.autoPlay != autoPlay && !isRobot()){
			...
			CdtljTable table2 = getPlayingTable(CdtljTable.class);
			...
			if(null == table2){
				...
				table2.addPlayLog(table2.addSandhPlayLog(getSeat(), CdtljConstants.action_tuoguan, val, false, 0, null, 0,""));
			}
			// table.addPlayLog(getSeat(), CdtljConstants.action_tuoguan + "",(autoPlay?1:0) + "");
		}
	...
	public void dealHandPais(List<Integer> pais,CdtljTable table) {
		this.table = table;
		this.handPais = pais;
	...
	public void addOutPais(List<Integer> cards ,CdtljTable table) {
		this.table = table;
	...
	public void initPlayInfo(String data) {
		if (!StringUtils.isBlank(data)) {
			...
			this.jiaoZhu = StringUtil.getIntValue(values, i++);
	...
	public PlayerInTableRes.Builder buildPlayInTableInfo(long recUserId, boolean isrecover) {
		...
		CdtljTable table = getPlayingTable(CdtljTable.class);
		...
		res.setName(name);
		//
		int t= table.getNewSeat(getUserId());
		if(t>0){
			res.setSeat(t);
			setSeat(t);
		}else{
		...
		extList.add(jiaoZhu);//1
		extList.add(getBaofu());//2
		extList.add(isAutoPlay() && !isRobot() ? 1 : 0);//3
		extList.add(getLiushou());//4
		extList.add(getTouXiang());//5
		extList.add(getPiaofen());//6
		if(table.getTableStatus()==CdtljConstants.TABLE_STATUS_SELECT_ONE_DIPAI){
			extList.add(0);
		}else{
			if(table.isIs_1v3()==1){
				//独战
				extList.add(0);
			}else if(table.isIs_1v3()==2){
...
//		extList.add(0);//7
	...
	public ClosingPlayerInfoRes.Builder bulidOneClosingPlayerInfoRes() {
		...
		res.setSex(sex);
		int team =table.getTeamSeat().contains(getSeat())?1:2;
		res.addExt(team+"");//0
	...
	public long getAuto_giveUpQzTime() {
		return auto_giveUpQzTime;
	}

	public void setAuto_giveUpQzTime(long auto_giveUpQzTime) {
		this.auto_giveUpQzTime = auto_giveUpQzTime;
	}

	public int getJiaoZhu() {
		return jiaoZhu;
	}

	public void setJiaoZhu(int jiaofen) {
		this.jiaoZhu = jiaofen;
		changeTableInfo();
	}
	...
	public static void loadWanfaPlayers(Class<? extends Player> cls){
		for (Integer integer:wanfaList){
			PlayerManager.wanfaPlayerTypesPut(integer,cls, CdtljCommandProcessor.getInstance());
		}
	}
