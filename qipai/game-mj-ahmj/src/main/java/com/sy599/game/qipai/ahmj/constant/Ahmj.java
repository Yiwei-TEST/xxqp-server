...
public enum Ahmj {
	...
	public static Ahmj getMajang(int id) {
		return Ahmj.valueOf(Ahmj.class, "mj" + id);
	}

	public static Ahmj getMajangByVal(int val) {
		List<Integer> majiangList = AhmjConstants.getMajiangList();
		for (Integer id:majiangList) {
	...
	public static String idToMajiang(int id) {
		Ahmj mj = Ahmj.getMajang(id);
		return valueToMajiang(mj.val);
	}

	public static List<Integer> getIdsByMjs(List<Ahmj> mjs){
		List<Integer> ids=new ArrayList<>();
		for (int i = 0; i < mjs.size(); i++) {
			ids.add(mjs.get(i).getId());
		}
		return ids;
	}
