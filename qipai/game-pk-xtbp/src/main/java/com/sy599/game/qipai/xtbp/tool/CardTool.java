package com.sy599.game.qipai.xtbp.tool;
...
public final class CardTool {
	 ...
	 * @return
	 */
	public static List<List<Integer>> fapai(int playerCount, List<List<Integer>> zps, boolean chouliu) {
		List<List<Integer>> list = new ArrayList<>();
		List<Integer> copy;
		copy = new ArrayList<>(XtbpConstants.cardList);
...
//	}
	 ...
	 * @return
	 */
	public static int checkCardValue(List<Integer> hands,List<Integer> list ,int zhuColor,int disColor,boolean isFirst) {
		
		//一轮首次出牌
		if(isFirst) {
			CardType ct = getCardType(list,zhuColor);
			//甩牌
	...
	public static CardType getCardType(List<Integer> list, int zhuColor) {
		CardType ct;
		if(list.size()==1) {
			...
			if(list.size()==2){
				...
				boolean isShunzi =  CardUtils.isTuoLaji2(list,zhuColor);
				if(isShunzi || ct.getType()== CardType.TUOLAJI){
