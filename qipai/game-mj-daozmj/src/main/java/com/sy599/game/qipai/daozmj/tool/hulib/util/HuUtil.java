package com.sy599.game.qipai.daozmj.tool.hulib.util;
...
//        TableMgr.getInstance().load();
    ...
    public static boolean isCanHu(List<DzMj> cards, int gui_num) {
    ...
    public static List<DzMj> getLackPaiList(List<DzMj> cards, int gui_num) {
        int[] cardsArr = toCardArray(cards);
        boolean any = false;
        if (gui_num > 0) {
    ...
    public static int[] toCardArray(List<DzMj> cards) {
        int[] cardsArr = new int[34];
        Arrays.fill(cardsArr, 0);

        for (int i = 0; i < cards.size(); i++) {
            DzMj card = cards.get(i);
            try {
    ...
    private static DzMj index2Majiang(int index) {
        int type = index / 9 + 1;
        ...
        return DzMj.getMajiangByVal(val);
    }

    private static List<DzMj> toRawCards(List<Integer> list) {
        List<DzMj> retList = new ArrayList<>();
        for (int index : list) {
    ...
    public static int getMjIndex(DzMj card) {
