...
public class DaozMjHelper {
	 ...
	 * @return
	 */
	public static int getCountByVal(List<DzMj> copy, int val) {
		int count = 0;
	...
	public static List<DzMj> find(List<Integer> copy, List<Integer> valList) {
		...
		if (!valList.isEmpty()) {
			for (int zpId : valList) {
				...
				while (iterator.hasNext()) {
					...
					DzMj majiang = DzMj.getMajang(card);
