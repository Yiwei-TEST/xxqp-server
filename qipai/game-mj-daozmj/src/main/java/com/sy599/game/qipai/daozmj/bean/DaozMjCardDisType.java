...
public class DaozMjCardDisType {
	private int action;
	...
	public boolean isHasCard(DzMj card) {
	...
	public boolean isHasCardVal(int val) {
		if (cardIds != null) {
			List<DzMj> majiangs = DaozMjQipaiTool.toMajiang(cardIds);
			List<Integer> vals = DaozMjQipaiTool.toMajiangVals(majiangs);
			return vals.contains(val);
		}
		return false;

	}
	public boolean isHasCardVal(DzMj card) {
		if (cardIds != null) {
			List<DzMj> majiangs = DaozMjQipaiTool.toMajiang(cardIds);
			List<Integer> vals = DaozMjQipaiTool.toMajiangVals(majiangs);
	...
	public int removeCardVal(int val) {
		if (cardIds != null) {
			...
			while (iterator.hasNext()) {
				...
				DzMj majiang = DzMj.getMajang(id);
