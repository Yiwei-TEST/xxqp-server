...
public class YyWhzCardTypeHuxi {
    private int action;
    ...
    public YyWhzCardTypeHuxi() {
    }

    public YyWhzCardTypeHuxi(int action, List<Integer> cardIds, int hux, boolean selfMo) {
        this.action = action;
    ...
    public boolean isHasCard(YyWhzCard card) {
    ...
    public Builder buildMsg(boolean hideCards) {
        ...
        if (hideCards) {
            msg.addAllCards(YyWhzTool.toGhzCardZeroIds(cardIds));
        } else {
            if (cardIds == null)
