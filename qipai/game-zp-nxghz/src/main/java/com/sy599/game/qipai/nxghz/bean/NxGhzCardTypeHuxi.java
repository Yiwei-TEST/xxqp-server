...
public class NxGhzCardTypeHuxi {
    private int action;
    ...
    public NxGhzCardTypeHuxi() {
    }

    public NxGhzCardTypeHuxi(int action, List<Integer> cardIds, int hux, boolean selfMo) {
        this.action = action;
    ...
    public boolean isHasCard(NxGhzCard card) {
    ...
    public Builder buildMsg(boolean hideCards) {
        ...
        if (hideCards) {
            msg.addAllCards(NxGhzTool.toGhzCardZeroIds(cardIds));
        } else {
            if (cardIds == null)
