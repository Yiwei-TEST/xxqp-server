...
public class MjIndexArr {
    ...
    public void addMajiangIndex(int count, List<TcdpMj> majiangList, int val) {
        if (a[count] == null) {
            a[count] = new MjIndex();
        }
     ...
     * @return
     */
    public Map<Integer, List<TcdpMj>> getMajiangMap(int size) {
        Map<Integer, List<TcdpMj>> map = new HashMap<>();
    ...
    public Map<Integer, List<TcdpMj>> getJiang(boolean need258) {
        Map<Integer, List<TcdpMj>> map = new HashMap<>();
        for (int i = 0; i < a.length; i++) {
            if (2 <= i + 1) {
                MjIndex majiangIndex = a[i];
                if (majiangIndex != null) {
                    if (need258) {
                        for (Entry<Integer, List<TcdpMj>> entry : majiangIndex.getMajiangValMap().entrySet()) {
    ...
    public List<TcdpMj> getKeziList() {
