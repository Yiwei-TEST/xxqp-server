...
import com.sy599.game.qipai.zhengzmj.command.ZhengzMjCommandProcessor;
...
public class ZhengzMjPlayer extends Player {
    // 座位id
    private int seat;
    // 状态
    ...
    private List<ZhengzMj> handPais;
    private List<ZhengzMj> outPais;
    private List<ZhengzMj> peng;
    private List<ZhengzMj> aGang;
	private List<ZhengzMj> chi;
    private List<ZhengzMj> mGang;
     ...
     * 出牌对应操作信息
     */
    private List<ZhengzMjCardDisType> cardTypes;
    //胡牌类型
    ...
    public ZhengzMjPlayer() {
        handPais = new ArrayList<ZhengzMj>();
        outPais = new ArrayList<ZhengzMj>();
        peng = new ArrayList<>();
        chi = new ArrayList<>();
        aGang = new ArrayList<>();
        mGang = new ArrayList<>();
...
//        waiPiaoPoint = -1;
    ...
    public void dealHandPais(List<ZhengzMj> pais) {
        this.handPais = pais;
        getPlayingTable().changeCards(seat);
    }

    public List<ZhengzMj> getHandMajiang() {
        return handPais;
    }


    public List<Integer> getHandPais() {
        return ZhengzMjHelper.toMajiangIds(handPais);
    }

    public List<Integer> getOutPais() {
        return ZhengzMjHelper.toMajiangIds(outPais);
    }

    public List<ZhengzMj> getOutMajing() {
        return outPais;
    }

    public void moMajiang(ZhengzMj majiang) {
        setPassMajiangVal(0);
     ...
     */
    public void addOutPais(List<ZhengzMj> cards, int action, int disSeat) {
        handPais.removeAll(cards);
        if (action == ZhengzMjDisAction.action_chupai) {
            ...
            if (action == ZhengzMjDisAction.action_peng) {
                ...
                if (cards.size() != 1) {
                    ...
                    ZhengzMj pengMajiang = cards.get(0);
                    Iterator<ZhengzMj> iterator = peng.iterator();
                ...
                changeAction(ZhengzMjConstants.ACTION_COUNT_INDEX_MINGGANG, 1);
            } else if (action == ZhengzMjDisAction.action_angang) {
                //myExtend.setMjFengshen(FirstmythConstants.firstmyth_index12, 1);
                aGang.addAll(cards);
                changeAction(ZhengzMjConstants.ACTION_COUNT_INDEX_ANGANG, 1);
            }
    ...
    public void qGangUpdateOutPais(ZhengzMj card) {
        Iterator<ZhengzMj> iterator = mGang.iterator();
        while (iterator.hasNext()) {
            ZhengzMj majiang = iterator.next();
        ...
        for (ZhengzMjCardDisType disType : cardTypes) {
            if (disType.getAction() == ZhengzMjDisAction.action_minggang) {
                if (disType.isHasCardVal(card)) {
                    disType.setAction(ZhengzMjDisAction.action_peng);
     ...
     * @param disCardList
     */
    public void addCardType(int action, List<ZhengzMj> disCardList, int disSeat, int disStatus) {
        if (action != 0) {
            if (action == ZhengzMjDisAction.action_minggang && disCardList.size() == 1) {
                ZhengzMj majiang = disCardList.get(0);
                for (ZhengzMjCardDisType disType : cardTypes) {
                    if (disType.getAction() == ZhengzMjDisAction.action_peng) {
                        if (disType.isHasCardVal(majiang)) {
                            disType.setAction(ZhengzMjDisAction.action_minggang);
                            disType.addCardId(majiang.getId());
                            disType.setDisSeat(disType.getDisSeat());
                            break;
                        }
                    }
                }
            } else {
                ZhengzMjCardDisType type = new ZhengzMjCardDisType();
                type.setAction(action);
                type.setCardIds(ZhengzMjQipaiTool.toMajiangIds(disCardList));
    ...
    public void removeOutPais(List<ZhengzMj> cards, int action) {
...
//        sb.append(waiPiaoPoint).append(",");
    ...
    public void initPlayInfo(String data) {
        if (!StringUtils.isBlank(data)) {
            ...
            this. passHuV = StringUtil.getIntValue(values, i++);
...
//           this.waiPiaoPoint  = StringUtil.getIntValue(values, i++);
    ...
    public List<Integer> getMoldIds() {
        ...
        if (!peng.isEmpty()) {
            list.addAll(ZhengzMjHelper.toMajiangIds(peng));
        }
        if (!mGang.isEmpty()) {
            list.addAll(ZhengzMjHelper.toMajiangIds(mGang));
        }
        if (!aGang.isEmpty()) {
            list.addAll(ZhengzMjHelper.toMajiangIds(aGang));
        }
        if (!chi.isEmpty()) {
			list.addAll(ZhengzMjHelper.toMajiangIds(chi));
		}
        return list;
    }
    ...
    public List<PhzHuCards> buildDisCards(long lookUid, boolean hide) {
        ...
        for (ZhengzMjCardDisType type : cardTypes) {
            if (hide && lookUid != this.userId && type.getAction() == ZhengzMjDisAction.action_angang) {
                // 不是本人并且是栽
    ...
    public PlayerInTableRes.Builder buildPlayInTableInfo(boolean isrecover) {
        ...
        res.addAllAngangIds(ZhengzMjHelper.toMajiangIds(aGang));
        res.addAllMoldCards(buildDisCards(userId));
        List<ZhengzMj> gangList = getGang();
        // 是否杠过
        res.addExt(gangList.isEmpty() ? 0 : 1); //0
        ZhengzMjTable table = getPlayingTable(ZhengzMjTable.class);
        // 现在是否自己摸的牌
    ...
    public List<ZhengzMj> getGang() {
        List<ZhengzMj> gang = new ArrayList<>();
        gang.addAll(aGang);
        gang.addAll(mGang);
        return gang;
    }
...
//        }
    ...
    public void initPais(String handPai, String outPai) {
        if (!StringUtils.isBlank(handPai)) {
            ...
            this.handPais = ZhengzMjHelper.toMajiang(list);
        }
        if (!StringUtils.isBlank(outPai)) {
            ...
            for (String value : values) {
                ...
                if (i == 0) {
                    ...
                    this.outPais = ZhengzMjHelper.toMajiang(list);
                } else {
                    ZhengzMjCardDisType type = new ZhengzMjCardDisType();
                    type.init(value);
                    cardTypes.add(type);
                    List<ZhengzMj> majiangs = ZhengzMjHelper.toMajiang(type.getCardIds());
                    if (type.getAction() == ZhengzMjDisAction.action_angang) {
                        aGang.addAll(majiangs);
                    } else if (type.getAction() == ZhengzMjDisAction.action_minggang) {
                        mGang.addAll(majiangs);
                    } else if (type.getAction() == ZhengzMjDisAction.action_peng) {
                        peng.addAll(majiangs);
                    }else if (type.getAction() == ZhengzMjDisAction.action_chi) {
                        chi.addAll(majiangs);
                    }
    ...
    public List<ZhengzMjCardDisType> getCardTypesByAction(int action) {
    	List<ZhengzMjCardDisType> types = new ArrayList<>();
    	  for (ZhengzMjCardDisType type : cardTypes) {
    ...
    public String buildOutPaiStr() {
        ...
        for (ZhengzMjCardDisType huxi : cardTypes) {
            sb.append(huxi.toStr()).append(";");
        }
        return sb.toString();
    }
	public boolean isChiPengGang(){
		if(chi.size() >0 || mGang.size() >0 || peng.size()>0 || mGang.size() >0) {
			return true;
		}
     ...
     */
    public List<Integer> checkDisMajiang(ZhengzMj majiang, boolean canCheckHu) {
        List<Integer> list = new ArrayList<>();
        int[] arr = new int[6];
        // 转转麻将别人出牌可以胡
        ZhengzMjTable table = getPlayingTable(ZhengzMjTable.class);
        if (canCheckHu && passMajiangVal != majiang.getVal()&&passHuV==0) {
            // 没有出现漏炮的情况
            List<ZhengzMj> copy = new ArrayList<>(handPais);
            copy.add(majiang);
          
            boolean hu = ZhengzMjTool.disIsHu(copy, table,majiang.getVal());
            if (hu) {
            	//if(getWangzState()!=1)
                arr[ZhengzMjConstants.ACTION_INDEX_HU] = 1;
            }
            
           
        }
        int count = ZhengzMjHelper.getMajiangCount(handPais, majiang.getVal());
        if (count >= 2&&majiang.getVal()!=table.getWangMjVal()&&table.getHunPaiNum()!=2) {
            // 除了红中外必须要3张以上的牌
            if (passMajiangVal != majiang.getVal()) {
                arr[ZhengzMjConstants.ACTION_INDEX_PENG] = 1;// 可以碰
            }
        }
        if (count == 3&&majiang.getVal()!=table.getWangMjVal()) {
            arr[ZhengzMjConstants.ACTION_INDEX_MINGGANG] = 1;// 可以杠
        }
        
        
        if (table.calcNextSeat(table.getDisCardSeat()) == seat&&table.getKeChi()==1) {
        	  List<ZhengzMj> chiMj = ZhengzMjTool.checkChi(handPais, majiang,table.getWangMjVal());
        	  if (!chiMj.isEmpty()&&majiang.getVal()!=table.getWangMjVal()) {
        		  arr[ZhengzMjConstants.ACTION_INDEX_CHI] = 1;
        	  }
		}
...
//    }
    ...
    public ZhengzMj getLastMoMajiang() {
        if (handPais.isEmpty()) {
            return null;
     ...
     */
    public List<Integer> checkMo(ZhengzMj majiang) {
        List<Integer> list = new ArrayList<>();
        int[] arr = new int[6];
        ZhengzMjTable table = getPlayingTable(ZhengzMjTable.class);
//        if(hongZhongCount >=4 &&table.isSiBaHZ()) {
//        	  arr[ZhengzMjConstants.ACTION_INDEX_ZIMO] = 1;
//        }
        if (ZhengzMjTool.isHu(handPais, table)) {
            arr[ZhengzMjConstants.ACTION_INDEX_ZIMO] = 1;
        }
        ...
        if (isAlreadyMoMajiang()) {
            // if (majiang != null) {
            Map<Integer, Integer> pengMap = ZhengzMjHelper.toMajiangValMap(peng);
            for (ZhengzMj handMajiang : handPais) {
            ...
            List<ZhengzMj> copy = new ArrayList<>(handPais);
           // ZhengzMjTool.dropWangBa(copy,table.getWangMjVal());//道州麻将需要做下判断
            Map<Integer, Integer> handMap = ZhengzMjHelper.toMajiangValMap(copy);
            if (handMap.containsValue(4)) {
                arr[ZhengzMjConstants.ACTION_INDEX_ANGANG] = 1;// 可以暗杠
            }
        }
        for (int val : arr) {
            list.add(val);
    ...
    public List<ZhengzMj> getPeng() {
        return peng;
    }

    public List<ZhengzMj> getaGang() {
        return aGang;
    }

    public List<ZhengzMj> getmGang() {
        return mGang;
    }
    ...
    public int getPassHuV() {
		return passHuV;
	}

	public void setPassHuV(int passHuV) {
		this.passHuV = passHuV;
	}
     ...
     * @return
     */
    public boolean isPassGang(ZhengzMj majiang) {
    ...
    public static final List<Integer> wanfaList = Arrays.asList(
            GameUtil.game_type_zhengzmj);

    public static void loadWanfaPlayers(Class<? extends Player> cls) {
        for (Integer integer : wanfaList) {
            PlayerManager.wanfaPlayerTypesPut(integer, cls, ZhengzMjCommandProcessor.getInstance());
        }
    }
    ...
    public boolean checkAutoPlay(int autoType, boolean forceSend) {
        ...
        if (!auto && table.getIsAutoPlay() > 0) {
            //检查玩家是否进入系统托管状态
        	 if (!checkAutoPlay && table.getTableStatus()!= ZhengzMjConstants.TABLE_STATUS_PIAO) {
        ...
        if (auto) {
            if (getAutoPlayTime() == 0L) {
                ...
                if (autoType == 1) {
                    if (timeOut >= ZhengzMjConstants.AUTO_READY_TIME) {
                        setAutoPlayTime(0);
                        return true;
                    }
                } else if (autoType == 2) {
                    if (timeOut >= ZhengzMjConstants.AUTO_HU_TIME) {
                        setAutoPlayTime(0);
                        return true;
                    }
                } else {
                    if (timeOut >= ZhengzMjConstants.AUTO_PLAY_TIME) {
                        setAutoPlayTime(0);
                        return true;
                    }
                }
    ...
    public void setAutoPlay(boolean autoPlay, boolean isSelf) {
        ...
        if (this.autoPlay != autoPlay || autoPlaySelf != isSelf) {
            ...
            if(!getHandPais().isEmpty()){
            	table.addPlayLog(table.getDisCardRound() + "_" +getSeat() + "_" + ZhengzMjDisAction.action_tuoguan + "_" +(autoPlay?1:0)+ getExtraPlayLog());
            }
        }
