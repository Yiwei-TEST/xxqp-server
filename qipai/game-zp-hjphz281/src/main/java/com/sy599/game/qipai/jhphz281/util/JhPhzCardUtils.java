...
public final class JhPhzCardUtils {
	...
	public static boolean canTi(JhPhzHandCards handCards, int id) {
		int val = loadCardVal(id);
		return handCards.KAN.containsKey(val);
	}

	public static boolean canPao(JhPhzHandCards handCards, int id) {
		int val = loadCardVal(id);
	...
	public static JhPhzHuXiEnums loadHuXiEnum(List<Integer> list) {
		sort(list);
		if (list.size() == 3) {
				...
				if (val2 >= WANGPAI_VAL) {
					...
					if (val1 == val2) {
						...
						if (sameType(list.get(0), list.get(1))) {
							if (val1 == 1 || val1 == 101) {
								return JhPhzHuXiEnums.CHI123;
							} else if (redCard(list.get(0)) && redCard(list.get(1))) {
								return JhPhzHuXiEnums.CHI2710;
							} else if ((val1 == 2 && val2 == 3) || (val1 == 102 && val2 == 103)) {
								return JhPhzHuXiEnums.CHI123;
							} else {
								return JhPhzHuXiEnums.CHI;
							}
						} else {
							return JhPhzHuXiEnums.JIAO;
						}
					}
				}
		...
		if (handCards.CHI_COMMON.size() > 0) {
			for (List<Integer> temp : handCards.CHI_COMMON) {
				...
				if (map.containsKey(1)) {
					list.add(new JhPhzCardMessage(JhPhzHuXiEnums.CHI123, temp));
				} else if (map.containsKey(101)) {
					list.add(new JhPhzCardMessage(JhPhzHuXiEnums.CHI123, temp));
				} else if (map.containsKey(2) && map.containsKey(7) && map.containsKey(10)) {
					list.add(new JhPhzCardMessage(JhPhzHuXiEnums.CHI2710, temp));
				} else if (map.containsKey(102) && map.containsKey(107) && map.containsKey(110)) {
					list.add(new JhPhzCardMessage(JhPhzHuXiEnums.CHI2710, temp));
				} else {
					list.add(new JhPhzCardMessage(JhPhzHuXiEnums.CHI, temp));
				}
			}
		}
		return list;
	...
	public static List<JhPhzChiCard> canChi(List<Integer> srcList, int id) {
		List<JhPhzChiCard> result = new ArrayList<>();
		int val = loadCardVal(id);
		sort(srcList);
		// 绞牌
		List<Integer> similarVals = loadIdsByVal(srcList, val <= 10 ? val + 100 : val - 100);
		...
		case 0: {
			if (similarVals.size() == 2) {
				result.add(new JhPhzChiCard().buildChi(similarVals));
			}
			...
			if (id1 > 0 && id2 > 0) {
				result.add(new JhPhzChiCard().buildChi(asList(id1, id2)));
			}
			id1 = id2;
		...
		case 1:
			...
			if (yjhList.size() > 0) {
				for (List<Integer> temp : yjhList) {
					...
					List<JhPhzChiCard> list1 = canChi(list0, id);
					for (JhPhzChiCard chiCard : list1) {
			...
			if (similarVals.size() > 0) {
				result.add(new JhPhzChiCard().buildChi(asList(sameVals.get(0), similarVals.get(0))));
			}
		...
		if (result.size() > 0 && sameVals.size() > 0) {
			List<JhPhzChiCard> result0 = new ArrayList<>();
			for (JhPhzChiCard chiCard : result) {
				JhPhzChiCard pre = null;
				List<Integer> chi = chiCard.getChi();
				sort(chi);
				int val1 = loadCardVal(chi.get(0));
				int val2 = loadCardVal(chi.get(1));
				for (JhPhzChiCard chiCard1 : result0) {
	...
	public static int countHuxi0(List<Integer> list) {
		...
		for (Integer tmp : list) {
			int val = JhPhzCardUtils.loadCardVal(tmp);
			Integer count = map.get(val);
		...
		case 3:
			...
			if (wangCount == null) {
				...
				if (size == 3) {
					if (map.containsKey(1)) {
						...
						total = (JhPhzCardUtils.smallCard(list.get(0)) ? JhPhzHuXiEnums.CHI.getSmall()
								: JhPhzHuXiEnums.CHI.getBig());
					}
				} else if (size == 2) {
					total = (JhPhzCardUtils.smallCard(list.get(1)) ? JhPhzHuXiEnums.JIAO.getSmall()
							: JhPhzHuXiEnums.JIAO.getBig());
				} else {
					total = (JhPhzCardUtils.smallCard(list.get(0)) ? JhPhzHuXiEnums.KAN.getSmall()
							: JhPhzHuXiEnums.KAN.getBig());
				}
			} else {
				...
				if (val1 == 201) {
					total = JhPhzHuXiEnums.KAN.getBig();
				} else if (val2 == 201) {
					total = (JhPhzCardUtils.smallCard(list.get(0)) ? JhPhzHuXiEnums.KAN.getSmall()
							: JhPhzHuXiEnums.KAN.getBig());
				} else if (val3 == 201) {
					if (val1 == val2) {
						...
						if (val1 == 1 || val1 == 101 || val1 == 2 || val1 == 102) {
							total = JhPhzCardUtils.smallCard(list.get(0)) ? JhPhzHuXiEnums.CHI123.getSmall()
									: JhPhzHuXiEnums.CHI123.getBig();
						} else {
							total = JhPhzCardUtils.smallCard(list.get(0)) ? JhPhzHuXiEnums.CHI.getSmall()
									: JhPhzHuXiEnums.CHI.getBig();
						}
					} else if (redCard(list.get(0)) && redCard(list.get(1)) && sameType(list.get(0), list.get(1))) {
						total = JhPhzCardUtils.smallCard(list.get(0)) ? JhPhzHuXiEnums.CHI2710.getSmall()
								: JhPhzHuXiEnums.CHI2710.getBig();
					} else {
						total = JhPhzHuXiEnums.JIAO.getBig();
					}
				}
			...
			break;
		case 2:
			total = JhPhzCardUtils.smallCard(list.get(0)) ? JhPhzHuXiEnums.DUI.getSmall()
					: JhPhzHuXiEnums.DUI.getBig();
			break;
		case 4:
			total = JhPhzCardUtils.smallCard(list.get(0)) ? JhPhzHuXiEnums.TI.getSmall()
					: JhPhzHuXiEnums.TI.getBig();
			break;
		}
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 100000; i++) {
			int count1=0;
			List<List<Integer>> list = fapaiControl(JhPhzCardUtils.loadCards(3), 2, 14,0);
			for (int j = 0; j < 2; j++) {
				List<Integer> cards = list.get(j);
				int num=0;
				int nums[]=new int[21];
