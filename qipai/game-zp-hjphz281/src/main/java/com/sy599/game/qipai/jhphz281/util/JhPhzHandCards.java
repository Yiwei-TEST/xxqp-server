...
public class JhPhzHandCards implements Serializable {
    ...
    public void init(List<Integer> list) {
        ...
        SRC.addAll(tempList);
        JhPhzCardUtils.sort(tempList);
        int pre = tempList.get(0);

        if (JhPhzCardUtils.wangCard(pre)) {
            WANGS.add(pre);
        }
        ...
        for (int i = 1; i < size; i++) {
            ...
            if (JhPhzCardUtils.wangCard(cur)) {
                WANGS.add(cur);
            } else if (JhPhzCardUtils.commonCard(cur) && JhPhzCardUtils.sameCard(pre, cur)) {
                count++;
                if (count == 3) {
                    KAN.put(JhPhzCardUtils.loadCardVal(cur), new ArrayList<>(tempList.subList(i - 2, i + 1)));
                } else if (count == 4) {
                    TI.put(JhPhzCardUtils.loadCardVal(cur), new ArrayList<>(tempList.subList(i - 3, i + 1)));
                    KAN.remove(JhPhzCardUtils.loadCardVal(cur));
                }
            } else {
                count = 1;
            }
    ...
    public JhPhzHandCards(List<Integer> list) {
        init(list);
    }
    ...
    public int loadPPChuxi() {
        ...
        if (PAO.size() > 0) {
            for (Map.Entry<Integer, List<Integer>> kv : PAO.entrySet()) {
                total += (kv.getKey() > 100 ? JhPhzHuXiEnums.PAO.getBig() : JhPhzHuXiEnums.PAO.getSmall());
            }
        }
        if (PENG.size() > 0) {
            for (Map.Entry<Integer, List<Integer>> kv : PENG.entrySet()) {
                total += (kv.getKey() > 100 ? JhPhzHuXiEnums.PENG.getBig() : JhPhzHuXiEnums.PENG.getSmall());
            }
        }
        if (CHI_COMMON.size() > 0) {
            for (List<Integer> temp : CHI_COMMON) {
                ...
                for (Integer tmp : temp) {
                    map.put(JhPhzCardUtils.loadCardVal(tmp), 1);
                }

                if (map.containsKey(1)) {
                    total += (JhPhzHuXiEnums.CHI123.getSmall());
                } else if (map.containsKey(101)) {
                    total += (JhPhzHuXiEnums.CHI123.getBig());
                } else if (map.containsKey(2) && map.containsKey(7) && map.containsKey(10)) {
                    total += (JhPhzHuXiEnums.CHI2710.getSmall());
                } else if (map.containsKey(102) && map.containsKey(107) && map.containsKey(110)) {
                    total += (JhPhzHuXiEnums.CHI2710.getBig());
                } else {
                    total += (JhPhzCardUtils.smallCard(temp.get(0)) ? JhPhzHuXiEnums.CHI.getSmall() : JhPhzHuXiEnums.CHI.getBig());
                }
            }
        }
        ...
        if (WEI_CHOU.size() > 0) {
            for (Map.Entry<Integer, List<Integer>> kv : WEI_CHOU.entrySet()) {
                total += (kv.getKey().intValue() > 100 ? JhPhzHuXiEnums.WEI_CHOU.getBig() : JhPhzHuXiEnums.WEI_CHOU.getSmall());
            }
        }
        return total;
    }
    ...
    public int loadTotalHuxiOutIns() {
        ...
        if (WEI_CHOU.size() > 0) {
            for (Map.Entry<Integer, List<Integer>> kv : WEI_CHOU.entrySet()) {
                total += (kv.getKey() > 100 ? JhPhzHuXiEnums.WEI_CHOU.getBig() : JhPhzHuXiEnums.WEI_CHOU.getSmall());
            }
        }
        ...
        if (CHI_JIAO.size() > 0) {
            for (List<Integer> temp : CHI_JIAO) {
                ...
                total += (count == 2 ? JhPhzHuXiEnums.JIAO.getBig() : JhPhzHuXiEnums.JIAO.getSmall());
            }
        }
    ...
    public int loadRedCardCount(boolean all) {
        ...
        if (TI.size() > 0) {
            for (Map.Entry<Integer, List<Integer>> kv : TI.entrySet()) {
                if (JhPhzCardUtils.redCard(kv.getValue().get(0))) {
                    count += 4;
                }
            }
        }
        ...
        if (CHI_JIAO.size() > 0) {
            for (List<Integer> temp : CHI_JIAO) {
                if (JhPhzCardUtils.redCard(temp.get(0))) {
                    count += 3;
                }
            }
        }
    ...
    public String toString() {
        ...
        if (TI.size() > 0) {
            strBuilder.append("{");
            for (Map.Entry<Integer, List<Integer>> kv : TI.entrySet()) {
                strBuilder.append(",").append(kv.getKey()).append("=").append(kv.getValue()).append(" huxi ").append(kv.getKey() > 100 ? JhPhzHuXiEnums.TI.getBig() : JhPhzHuXiEnums.TI.getSmall());
            }
            strBuilder.append("}");
        } else {
            strBuilder.append("{}");
        ...
        if (PAO.size() > 0) {
            strBuilder.append("{");
            for (Map.Entry<Integer, List<Integer>> kv : PAO.entrySet()) {
                strBuilder.append(",").append(kv.getKey()).append("=").append(kv.getValue()).append(" huxi ").append(kv.getKey() > 100 ? JhPhzHuXiEnums.PAO.getBig() : JhPhzHuXiEnums.PAO.getSmall());
            }
            strBuilder.append("}");
        } else {
            strBuilder.append("{}");
        ...
        if (KAN.size() > 0) {
            strBuilder.append("{");
            for (Map.Entry<Integer, List<Integer>> kv : KAN.entrySet()) {
                strBuilder.append(",").append(kv.getKey()).append("=").append(kv.getValue()).append(" huxi ").append(kv.getKey() > 100 ? JhPhzHuXiEnums.KAN.getBig() : JhPhzHuXiEnums.KAN.getSmall());
            }
            strBuilder.append("}");
        } else {
            strBuilder.append("{}");
        ...
        if (WEI.size() > 0) {
            strBuilder.append("{");
            for (Map.Entry<Integer, List<Integer>> kv : WEI.entrySet()) {
                strBuilder.append(",").append(kv.getKey()).append("=").append(kv.getValue()).append(" huxi ").append(kv.getKey() > 100 ? JhPhzHuXiEnums.WEI.getBig() : JhPhzHuXiEnums.WEI.getSmall());
            }
            strBuilder.append("}");
        } else {
            strBuilder.append("{}");
        ...
        if (WEI_CHOU.size() > 0) {
            strBuilder.append("{");
            for (Map.Entry<Integer, List<Integer>> kv : WEI_CHOU.entrySet()) {
                strBuilder.append(",").append(kv.getKey()).append("=").append(kv.getValue()).append(" huxi ").append(kv.getKey() > 100 ? JhPhzHuXiEnums.WEI_CHOU.getBig() : JhPhzHuXiEnums.WEI_CHOU.getSmall());
            }
            strBuilder.append("}");
        } else {
            strBuilder.append("{}");
        ...
        if (PENG.size() > 0) {
            strBuilder.append("{");
            for (Map.Entry<Integer, List<Integer>> kv : PENG.entrySet()) {
                strBuilder.append(",").append(kv.getKey()).append("=").append(kv.getValue()).append(" huxi ").append(kv.getKey() > 100 ? JhPhzHuXiEnums.PENG.getBig() : JhPhzHuXiEnums.PENG.getSmall());
            }
            strBuilder.append("}");
        } else {
            strBuilder.append("{}");
        ...
        if (CHI_JIAO.size() > 0) {
            ...
            for (List<Integer> temp : CHI_JIAO) {
                ...
                strBuilder.append(",").append(temp).append(" huxi ").append(count == 2 ? JhPhzHuXiEnums.JIAO.getBig() : JhPhzHuXiEnums.JIAO.getSmall());
            }
            strBuilder.append("}");
        } else {
            strBuilder.append("{}");
        ...
        if (CHI_COMMON.size() > 0) {
            ...
            for (List<Integer> temp : CHI_COMMON) {
                ...
                if (map.containsKey(1)) {
                    strBuilder.append(",").append(temp).append(" huxi ").append(JhPhzHuXiEnums.CHI123.getSmall());
                } else if (map.containsKey(101)) {
                    strBuilder.append(",").append(temp).append(" huxi ").append(JhPhzHuXiEnums.CHI123.getBig());
                } else if (map.containsKey(2) && map.containsKey(7) && map.containsKey(10)) {
                    strBuilder.append(",").append(temp).append(" huxi ").append(JhPhzHuXiEnums.CHI2710.getSmall());
                } else if (map.containsKey(102) && map.containsKey(107) && map.containsKey(110)) {
                    strBuilder.append(",").append(temp).append(" huxi ").append(JhPhzHuXiEnums.CHI2710.getBig());
                } else {
                    strBuilder.append(",").append(temp).append(" huxi ").append(JhPhzCardUtils.smallCard(temp.get(0)) ? JhPhzHuXiEnums.CHI.getSmall() : JhPhzHuXiEnums.CHI.getBig());
                }
            }
            strBuilder.append("}");
        } else {
    ...
    public JhPhzHandCards copy() {
        JhPhzHandCards handCards = new JhPhzHandCards();
        for (Map.Entry<Integer, List<Integer>> kv : TI.entrySet()) {
