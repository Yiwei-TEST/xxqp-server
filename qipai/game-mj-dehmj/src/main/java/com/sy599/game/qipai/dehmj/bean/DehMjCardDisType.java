...
//	}
	...
	public boolean isHasCard(DehMj card) {
		return cardIds != null && cardIds.contains(card.getId());
	}

	public boolean isHasCardVal(DehMj card) {
		if (cardIds != null) {
			List<DehMj> majiangs = DehMjHelper.toMajiang(cardIds);
			List<Integer> vals = DehMjHelper.toMajiangVals(majiangs);
	...
	public int removeCardVal(int val) {
		if (cardIds != null) {
			...
			while (iterator.hasNext()) {
				...
				DehMj majiang = DehMj.getMajang(id);
