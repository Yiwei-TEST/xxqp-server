package com.sy599.game.qipai.dehmj.command;
...
import com.sy599.game.qipai.dehmj.command.com.DehMjComCommand;
import com.sy599.game.qipai.dehmj.command.play.DehMjPlayCommand;
...
public class DehMjCommandProcessor extends AbstractBaseCommandProcessor {
    private static DehMjCommandProcessor processor = new DehMjCommandProcessor();
    ...
    static {
        commandMap.put(WebSocketMsgType.cs_com, DehMjComCommand.class);
        commandMap.put(WebSocketMsgType.cs_play, DehMjPlayCommand.class);
    ...
    public static DehMjCommandProcessor getInstance() {
        return processor;
    }
    ...
    public void process(Player player, MessageUnit message) {
        int code = 0;
        try {
            if (player == null || !(player instanceof DehMjPlayer)) {
                return;
            }
            DehMjPlayer csMajiangPlayer = player.getPlayer(processor);
