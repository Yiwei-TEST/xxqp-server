package com.sy599.game.qipai.hzmj.tool.hulib.util;
...
public class HuUtil {
    ...
    public static boolean isCanHu(List<HzMj> cards, int gui_num) {
    ...
    public static List<HzMj> getLackPaiList(List<HzMj> cards, int gui_num) {
        int[] cardsArr = toCardArray(cards);
        boolean any = false;
        if (gui_num > 0) {
    ...
    public static int[] toCardArray(List<HzMj> cards) {
        int[] cardsArr = new int[34];
        Arrays.fill(cardsArr, 0);

        for (int i = 0; i < cards.size(); i++) {
            HzMj card = cards.get(i);
    ...
    private static HzMj index2Majiang(int index) {
        int type = index / 9 + 1;
        ...
        return HzMj.getMajiangByVal(val);
    }

    private static List<HzMj> toRawCards(List<Integer> list) {
        List<HzMj> retList = new ArrayList<>();
        for (int index : list) {
    ...
    public static int getMjIndex(HzMj card) {
