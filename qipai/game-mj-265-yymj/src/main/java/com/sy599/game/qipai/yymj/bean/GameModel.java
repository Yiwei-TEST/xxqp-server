...
public class GameModel {
     ...
     */
     ...
     private boolean need258;
     /**杠后自动出牌*/
     private boolean gangAfterAutoPlay;

     //黄庄未听牌扣分
     private int huangZhuangMisFen;
     //门清将将胡 可接炮
     private boolean mQJJHuInvite;
     //门清7对
     private boolean mQSevenDui;
    ...
    @JSONField(serialize = false)
    public int incSignTingFlag(){
        return signTing += 1;
    }

    @JSONField(serialize = false)
    public int decSignTingFlag(){
        return signTing -= 1;
    }
    ...
    public Integer[] birdSeat(int seat) {
		...
		//对家：3、7
		//上家：4、8
		//2人模式13579中鸟其他不中；3人模式下147中自己，258中下家，369中上家；
		if (getGameMaxHuman() == 2) {
			return Arrays.asList(new Integer[] { 1, 3, 5, 7, 9 }, null).get(seat - 1);
		}
		else if (getGameMaxHuman() == 3) {
			return Arrays.asList(new Integer[] { 1, 4, 7 }, new Integer[] { 2, 5, 8 }, new Integer[] { 3, 6, 9 }).get(seat - 1);
		}
		else {
			return Arrays.asList(new Integer[] { 1, 5, 9 }, new Integer[] { 2, 6 }, new Integer[] { 3, 7 }, new Integer[] { 4, 8 }).get(seat - 1);
		}
	}

    /**
     *@description 3、扎鸟分 只有自摸才能扎鸟，以胡牌的人为标准，摸到1,5,9，算全中，其他三家出分结果X2，如果摸到2、6，下家出分X2，其他两家不变，摸到3、7，对家出分x2；摸到4、8，上家出分x2。
     *@param seat 座位号
     *@param bird 当前抓出来的鸟
     *@return 中几个鸟
     *@author Guang.OuYang
    ...
    public int birdIsThisSeat(int seat , int realSeat, int winnerRealSeat, Integer[] bird, List<Integer> birdSeat, List<Integer> birdId, MjiangHu.BigMap<Integer, Integer, Integer> seatBirdBigMap) {
        Integer[] birdCombo = birdSeat(seat/*seat*/);
		int birdNum = 0;
        if(birdCombo != null){
...
//						seatBirdBigMap.get(birdMj.getId()).setE(winnerRealSeat);
						seatBirdBigMap.get(birdMj.getId()).setE(realSeat);
    ...
    public static class SpecialPlay {
        ...
        //一字撬有喜
        private boolean oneCharPry;
        private int oneCharPryAdditonScore;
