...
import com.sy599.game.msg.serverPacket.PlayCardResMsg.MoMajiangRes.Builder;
...
import com.sy599.game.qipai.yymj.rule.MjRobotAI;
...
public class YyMjTable extends BaseTable {
     ...
     * 当前房间所有玩家信息map
     */
    private Map<Long, YyMjPlayer> playerMap = new ConcurrentHashMap<Long, YyMjPlayer>();
    /**
     * 座位对应的玩家信息MAP
     */
    private Map<Integer, YyMjPlayer> seatMap = new ConcurrentHashMap<Integer, YyMjPlayer>();
    /**
     * 胡确认信息
     */
	...
	public void sendOverSpecialEffect(Integer seatNum) {
		YyMjPlayer player = seatMap.get(seatNum);
		try {
			PlayMajiangRes.Builder builder = PlayMajiangRes.newBuilder();
			List<Integer> showMjLists = player.getHandPais();
			buildPlayRes(builder, player, MjDisAction.action_hu, !CollectionUtils.isEmpty(showMjLists) ? showMjLists.stream().map(v -> Mj.getMajang(v)).collect(Collectors.toList()) : Collections.emptyList());

			builder.setZimo(player.noNeedMoCard() ? 1 : 0);

			builder.addAllHuArray(player.getDahu());

			builder.setFromSeat(0);
			// 胡
			for (YyMjPlayer seat : seatMap.values()) {
				// 推送消息
    ...
    public void calcOver() {
		if(state!= table_state.play)return;
...
//        }
        ...
        //算鸟
        calcBird(zhuaNiao, winSeatList, seatBirdMap, seatBirdBigMap, seatBirds, birdsIds, true);

        //算杠
        calcGangScore(seatBirdBigMap);

        // 算胡的
        if (winSeatList.size() != 0) {
            // 先判断是自摸还是放炮
            YyMjPlayer winPlayer = null;
...
//            calcSpecialMenZi(lastWinSeat);
            ...
            //连庄
            int playSeat = 0;
            int needSubReappointment = 0;
            //连庄加分硬加分
            if (gameModel.isReappointment()) {
                // 没有流局
                if (!flow) {
                    //若胡出抢杠胡，转庄到点炮的人
                    if (winSeatList.size() > 1 || qiangGangHu) {
                        //  一炮多响设置放炮的人为庄家
                        //一炮多响，离点炮近的玩家优先胡牌
                        playSeat = disCardSeat;
//                        seatMap.get(disCardSeat).changePoint(reappointment);
                    } else {
                        //胡牌为庄
                        playSeat = winSeatList.get(0);
//                        seatMap.get(winSeatList.get(0)).changePoint(reappointment);
                    }

                } else {
                    playSeat = getLastWinSeat();
//                    seatMap.get(getLastWinSeat()).changePoint(reappointment);
                }

                if (playSeat > 0 && playSeat == lastWinSeat && playBureau != 1) {
                    reappointment += 1;
//                    seatMap.get(playSeat).setDahu(seatMap.get(playSeat).getDahu());
                    seatMap.get(playSeat).setDahu(new ArrayList<>(seatMap.get(playSeat).getDahu()));
                    seatMap.get(playSeat).getDahu().add(32);
                    needSubReappointment = reappointment;
                    LogUtil.printDebug("{},连庄:+{},{}", seatMap.get(playSeat).getName(), needSubReappointment, seatMap.get(playSeat).getPoint());
                } else if (playSeat != lastWinSeat) {
                    reappointment = 0;
                }
            }

            //吃杠杠爆全包
            boolean eatGangBurstAllInCharge = gameModel.isEatGangBurstAllInCharge() && winPlayer != null && winPlayer.getDahu().contains(27);
            qiangGangHu = gameModel.getSpecialPlay().isRobGangHu() && winPlayer != null && winPlayer.getDahu().contains(9);
//
            //点炮玩家
            YyMjPlayer eatGangPlay = null;
            if (eatGangBurstAllInCharge) {
                eatGangPlay = seatMap.get(disCardSeat);
            }
            ...
            if (selfMo) {
                ...
                int winScore = MjiangHu.calcMenZiScore(this, winPlayer.getDahu()) + needSubReappointment;

                LogUtil.printDebug("计算门子得分:{}, {}", winScore, MjTool.dahuListToString(winPlayer.getDahu()));
//
//                List<Integer> basicMenZi = MjiangHu.calcDaHuBasic(winPlayer.getDahu());
//
//                int basicScore = MjiangHu.calcMenZiScore(this, basicMenZi) + needSubReappointment;
//
//                LogUtil.printDebug("算鸟基础分:{}, {}", basicScore, MjTool.dahuListToString(basicMenZi));
//
...
//
                int winnerBirdBasicScore = calcBirdPoint(winScore, winnerBirdNum, true);
//
//                //计算鸟加成
                winScore = /*winScore + */winnerBirdBasicScore;

                int totalWinScore = 0;

                LogUtil.printDebug("算鸟后得分:{}, 鸟:{}, {}", totalWinScore, winnerBirdNum, winScore);

                Iterator<YyMjPlayer> iterator = seatMap.values().iterator();
                while (iterator.hasNext()) {
                    YyMjPlayer player = iterator.next();
                    //十倍不计分,仅仅扣除杠与马跟杠的分数
                    if (player.getSeat() != winPlayer.getSeat()) {
                        //十倍牌型不扣分
                        boolean dontAddWinScore = player.getDahu().contains(28);
                        //十倍不计分只记杠和马分
                        if (!dontAddWinScore) {
                            //输家的鸟
                            int loseBirdNum = seatBirdMap.get(player.getSeat());/*calcBirdNum(seatBirdBigMap, winPlayer.getSeat(), player.getSeat(), false, false, false)*/;
                            //算入输家的鸟
                            int lossScoreBasic = calcBirdPoint(winScore, loseBirdNum, true);
                            //十倍不计分只记杠和马分
                            int lossScore = lossScoreBasic /*+ (dontAddWinScore ? winScore : winScore)*/;

                            LogUtil.printDebug(player.getName() + ",鸟:{}, 分:{} , 鸟后lossScore:{}, {}, 十倍不计分:{}", loseBirdNum, winScore, lossScore, lossScoreBasic, dontAddWinScore);

                            LogUtil.printDebug("封顶:{}->{} 杠+分:{}", lossScore, getGameModel().topFenCalc(Math.abs(lossScore)), player.getGangPoint());

                            lossScore = Math.abs(lossScore);//getGameModel().topFenCalc(Math.abs(lossScore));
                            //点杠玩家全包,分数记录
                            if (!eatGangBurstAllInCharge || eatGangPlay == null) {
                                player.changePoint(-lossScore);
                            }
                ...
                if (eatGangBurstAllInCharge && eatGangPlay != null) {
                    eatGangPlay.changePoint(-totalWinScore);
                    LogUtil.printDebug("杠爆全包:{},{},-{}", eatGangPlay.getSeat(), eatGangPlay.getName(), totalWinScore);
                }

                LogUtil.printDebug("{} 总赢分:{},杠+分:{}", winPlayer.getName(), totalWinScore, winPlayer.getGangPoint());
                ...
                //大进大出点炮玩家需要出各家分
                //点炮玩家
                YyMjPlayer losePlayer = seatMap.get(disCardSeat);
                losePlayer.changeAction(0, 1);
                losePlayer.getMyExtend().setMjFengshen(FirstmythConstants.firstmyth_index10, winSeatList.size());
                boolean dontSubScore = losePlayer.getDahu().contains(28);
//                losePlayer.setDahu(Collections.emptyList());

                //一炮多响找到点炮的位置计算
                boolean oneGunMultiRing = winSeatList.size() > 1;
                int loseBirdNum = 0;
                if (oneGunMultiRing) {
                    loseBirdNum = calcBirdNum(seatBirdBigMap, losePlayer.getSeat(), losePlayer.getSeat(), true, true, false);
                    LogUtil.printDebug("一炮多响, 点炮玩家鸟:{}", loseBirdNum);
                }

                LogUtil.printDebug("点炮玩家:{}, 十倍不扣分:{}", losePlayer.getName(), dontSubScore);
                for (int winnerSeat : winSeatList) {
                    winPlayer = seatMap.get(winnerSeat);

                    int addScore = playSeat == winPlayer.getSeat() ? needSubReappointment : 0;

//                    boolean gangUpGun = false;
                    YyMjPlayer calcScorePlay;

                    //不是杠上炮,按赢家接炮算
                    calcScorePlay = winPlayer;

                    //计算门子得分,自摸没有门子算平胡, 平胡+1
                    int winScore = MjiangHu.calcMenZiScore(this, calcScorePlay.getDahu()) + addScore;
...
//                    List<Integer> basicMenZi = MjiangHu.calcDaHuBasic(winPlayer.getDahu());
//
//                    int basicScore = MjiangHu.calcMenZiScore(this, basicMenZi) + addScore;
//
//                    LogUtil.printDebug("算鸟基础分:{}, {}", basicScore, MjTool.dahuListToString(basicMenZi));
                    ...
                    int birdNum = 0;
                    //算入鸟
                    //一炮多响的情况，由放炮的人拿马，如果中对应的马，要赔更多，点炮所输分数加倍（不包含杠牌的分数），中一个马加一倍，两个马加两倍
//                    if (oneGunMultiRing) {
//                        birdNum = loseBirdNum + calcBirdNum(seatBirdBigMap, winnerSeat, losePlayer.getSeat(), true, false, true);
//                    } else {
                        for (Iterator<Integer> iterator = calcScoreSeat.iterator(); iterator.hasNext(); ) {
                            Integer seat = iterator.next();
//                            if (seat == winnerSeat) {
                                birdNum += seatBirdMap.get(seat);//calcBirdNum(seatBirdBigMap, seat, seat, true, true, false);
//                            } else {
//                                birdNum += calcBirdNum(seatBirdBigMap, winnerSeat, seat, false, false, false);
//                            }
                        }
//                    }
                    //所有鸟,基础分
                    int basicBirdScore = calcBirdPoint(winScore, birdNum, true);
                    //计算鸟加成,没有鸟则算入基础分,有鸟算鸟
                    lossTotalSubScore += (/*winScore +*/ basicBirdScore);
                    LogUtil.printDebug("算鸟后得分:{}, 鸟:{}", lossTotalSubScore, birdNum);

                    LogUtil.printDebug("总赢分:{}", lossTotalSubScore);

                    LogUtil.printDebug("封顶:{}->{},赢家杠+分:{},输家杠+分{}", lossTotalSubScore, getGameModel().topFenCalc(Math.abs(lossTotalSubScore)), winPlayer.getGangPoint(), losePlayer.getGangPoint());

                    lossTotalSubScore = Math.abs(lossTotalSubScore);//getGameModel().topFenCalc(Math.abs(lossTotalSubScore));
...
//                        }
        ...
        //流局罚分
		if (flow && gameModel.getHuangZhuangMisFen() > 0) {
			//流局时，未听牌玩家输给听牌玩家相应分数，可选项，可选1分、2分或4分。
			List<Integer> signTingSeats = seatMap.values().stream().filter(v -> !CollectionUtils.isEmpty(MjTool.getTingMjs(new ArrayList<>(v.getHandMajiang()), v.getGang(), v.getPeng(), v.getChi(),
					v.getBuzhang(), true, gameModel.getSpecialPlay().isOnlyDaHu(), 0, YyMjTable.this, v))).map(v -> v.getSeat()).collect(Collectors.toList());

			if(!CollectionUtils.isEmpty(signTingSeats)){
				//总罚分,单个玩家出的罚分
				int totalFaFen = 0;

				Iterator<YyMjPlayer> iterator1 = seatMap.values().iterator();
				while (iterator1.hasNext()) {
					YyMjPlayer next = iterator1.next();
					if(!signTingSeats.contains(next.getSeat())){
						//罚分
						totalFaFen += (gameModel.getGameMaxHuman() - 1) * gameModel.getHuangZhuangMisFen();
						next.setFaScore(-((gameModel.getGameMaxHuman() - 1) * gameModel.getHuangZhuangMisFen()));
						//扣分
						next.changePoint(next.getFaScore());

						//罚分门子
						if (CollectionUtils.isEmpty(next.getDahu())) {
							next.setDahu(new ArrayList<>());
						}
						next.getDahu().add(19);
						LogUtil.printDebug("罚分:{},{}",next.getName(), totalFaFen);
					}else{
						LogUtil.printDebug("不罚分:{},{}",next.getName(), signTingSeats);
					}
				}

				int totalFaFenFianl = totalFaFen / Math.max(signTingSeats.size(), 1);
				signTingSeats.stream().forEach(v -> {
					seatMap.get(v).setFaScore(totalFaFenFianl);
					seatMap.get(v).changePoint(totalFaFenFianl);
				});
			}

		}

		//一字撬有喜 有喜，一字撬时勾选每人加2分，不管是否胡牌。
		if(gameModel.getSpecialPlay().getOneCharPryAdditonScore() > 0){
			//喜分,单吊增加喜分
			int sum = seatMap.values().stream().filter(v -> v.getHandMajiang().size() == 1).mapToInt(v1 -> 1).sum();

			if(sum > 0){
				Iterator<YyMjPlayer> iterator1 = seatMap.values().iterator();
				while (iterator1.hasNext()) {
					YyMjPlayer v = iterator1.next();
					//喜分门子
					if (CollectionUtils.isEmpty(v.getDahu())) {
						v.setDahu(new ArrayList<>());
					}
					v.getDahu().add(18);

					if (v.getHandMajiang().size() == 1) {
						v.setXiScore(gameModel.getSpecialPlay().getOneCharPryAdditonScore() * (gameModel.getGameMaxHuman() - sum));
						v.changePoint(v.getXiScore());
					} else {
						v.setXiScore(-gameModel.getSpecialPlay().getOneCharPryAdditonScore() * sum);
						v.changePoint(v.getXiScore());
					}
					LogUtil.printDebug("玩家门子:{}, {}", v.getName(), MjTool.dahuListToString(v.getDahu()));
				}
			}
...
//            setLastWinSeat(moLastMajiangSeat);
        ...
        for (YyMjPlayer player : seatMap.values()) {
...
//        int upWinnerSeat = lastWinSeat;
        ...
        Iterator<YyMjPlayer> iterator1 = seatMap.values().iterator();
        while (iterator1.hasNext()) {
            YyMjPlayer player = iterator1.next();
...
//                    player.setDahu(finalHu.buildDahuList());
     ...
     * @date 2019/11/21
     */
    public void calcGangScore(MjiangHu.BigMap<Integer, Integer, Integer> seatBirdBigMap) {
        //清空杠分
        Iterator<YyMjPlayer> iterator4 = seatMap.values().iterator();
        while (iterator4.hasNext()) {
            YyMjPlayer next = iterator4.next();
            next.setGangPoint(0);
        ...
        Iterator<YyMjPlayer> iterator = seatMap.values().iterator();
        while (iterator.hasNext()) {
            YyMjPlayer curtPlay = iterator.next();
            //需要单独-3杠分的玩家
...
//                Mj mj = Mj.ge tMajang(next1.getKey());
               ...
               if (singleSubSeat < 1000) { //放杠
                   ...
                   //放杠3分
                   int basic = gameModel.getGameMaxHuman() == 2 ? 2 : 3;
                   ...
                   needSubScoreSeatBasicBird.merge(singleSubSeat, birdBasic, (oV, nV) -> oV + nV);
                    ...
                    if (action == MjDisAction.action_minggang) {    //明杠
                    	//明杠1分
						int basic = gameModel.getGameMaxHuman() == 2 ? 2 : 1;
                    	...
                    	//暗杠2分,2人模式4分
                        int basic = gameModel.getGameMaxHuman() == 2 ? 4 : 2;
                        int birdScore = (gameModel.isBirdAndGang() ? basic * birdNum : 0);
            ...
            if (haveMingAnGang) {
                //处理暗杠和明杠
                Iterator<YyMjPlayer> iterator2 = seatMap.values().iterator();
                while (iterator2.hasNext()) {
                    YyMjPlayer chaosMjPlayer = iterator2.next();
            ...
            if (haveSingleGang) {
                ...
                while (iterator3.hasNext()) {
                    ...
                    YyMjPlayer chaosMjPlayer = seatMap.get(fangGangSeat.getKey());
    ...
    public void calcBird(boolean zhuaNiao, List<Integer> winSeatList, Map<Integer, Integer> seatBirdMap, MjiangHu.BigMap<Integer, Integer, Integer> seatBirdBigMap, List<Integer> seatBirds, List<Integer> birdsIds, boolean onlyWinnerAwardHorse) {
        if (zhuaNiao) {
            // 先砸鸟
            Integer[] birdMjIds = zhuaNiao(lastMajiang);
            ...
            while (iterator.hasNext()) {
                ...
                for (int i = 1; i <= seatMap.keySet().size(); i++) {
                        seatBirdMap.put(nextSeat, gameModel.birdIsThisSeat(finalWinnerSeat, nextSeat, startSeat, allBirds.toArray(new Integer[0])/*buyHorse*/, seatBirds, birdsIds, seatBirdBigMap));
                        if (LogUtil.msgLog.isDebugEnabled())
                            LogUtil.printDebug("中鸟:::seat:{}->{},转换位置:{},num:{}, {}", nextSeat, seatMap.get(nextSeat).getName(), i, seatBirdMap.get(startSeat), allBirds.stream().map(v -> Mj.getMajang(v)).collect(Collectors.toList()));
						finalWinnerSeat++;
     ...
     * @date 2019/10/24
     */
    public void calcOverAfterClearInfo(YyMjPlayer player) {
...
//        }
    ...
    private int calcXiaoHuPoint(int seat) {
        ...
        YyMjPlayer player = seatMap.get(seat);
    ...
    public void saveLog(boolean over, long winId, Object resObject) {
        ...
        for (YyMjPlayer player : playerMap.values()) {
    ...
    private Integer[] zhuaNiao(Mj lastMaj) {
        // 先砸鸟
		//海底最后一张
        int realBirdNum = leftMajiangs.size() > gameModel.getSpecialPlay().getBirdNum() ? gameModel.getSpecialPlay().getBirdNum() : 1;
    ...
    protected void sendDealMsg(long userId) {
        ...
        if (checkSignTingInfo()) {
        	//所有人不能出牌
			btRoomMsg(WebSocketMsgType.req_com_sj_changeUnderCards);
			return;
        }

        // 没有操作的话通知庄家出牌
		btRoomMsg(WebSocketMsgType.req_com_kt_ask_dismajiang);
		// isBegin = false;
    }

    //\消息广播
	public void btRoomMsg(int req_com_sj_changeUnderCards) {
		ComRes.Builder com = SendMsgUtil.buildComRes(req_com_sj_changeUnderCards);//推送不能出牌
		seatMap.values().forEach(v -> v.writeSocket(com.clone().build()));
	}
     ...
     * @date 2019/10/21
     */
    private void sendDealInfo(YyMjPlayer tablePlayer) {
        //买马
    ...
    public void moMajiang(YyMjPlayer player, boolean isBuzhang) {
     ...
     * @param player
     */
    public void moMajiang(YyMjPlayer player, boolean isBuzhang, boolean gangHouMo) {
    	//报听没有选择完毕, 不摸牌
		...
		if (gameModel.getSpecialPlay().isHaiDiLaoYue() && getLeftMajiangCount() == 1) {
			moLastMajiang(player, 2, gangHouMo);
			return;
		}
		...
		//闲家在首轮检测, 摸到第一张牌之后是否能报听, 可以置换一次听牌
		if (player.isFirstDisCard() && player.isSignTing()) {
			int count = player.getHandMajiang().size();
			LinkedList<Mj> newHandCards = new LinkedList<>(player.getHandMajiang());
			newHandCards.addLast(nowMoCard);
			Mj up = null;
			while (count-- > 0) {
				if (up != null) {
					newHandCards.addLast(up);
				}

				up = newHandCards.removeFirst();

				List<Mj> huCards = MjTool.getTingMjs(newHandCards, player.getGang(), player.getPeng(), player.getChi(),
						player.getBuzhang(), true, gameModel.getSpecialPlay().isOnlyDaHu(), gameModel.getSpecialPlay().isQuanQiuRenJiang() ? 1 : 0, this, player);
				if (!CollectionUtils.isEmpty(huCards)) {
					player.setSignCanAutoDisCard(false);
					break;
				}
			}
		}
...
//        }
    ...
    public void calcMoLastSeats(int firstSeat) {
        for (int i = 0; i < getMaxPlayerCount(); i++) {
            YyMjPlayer player = seatMap.get(firstSeat);
...
//        sendMoLast(player, 1);
    ...
    public void buildPlayRes(PlayMajiangRes.Builder builder, Player player, int action, List<Mj> majiangs) {
     ...
     */
    public void checkBegin(YyMjPlayer player) {
    }
     ...
     * @date 2019/10/16
     */
    public MjiangHu checkGangUpFlowerOrGun(List<Integer> huMjIds, Map<Integer, MjiangHu> huMap, YyMjPlayer player, MjiangHu huBean, List<Mj> huMjs) {
        // 有大胡
...
//                    temp.setDianPao(false);
                ...
                if (gameModel.getSpecialPlay().isGangUpGun()) {
                    // 出掉杠牌
                    YyMjPlayer mPlayer = seatMap.get(moMajiangSeat);
    ...
    public MjiangHu checkGangUpFlowerOrGun(YyMjPlayer player, List<Mj> majiangs, List<Mj> huMjs) {
...
//        MjiangHu finalHu = player.checkHu(Mj.getMajang(mjId), disCardRound == 0);
     ...
     * @param majiangs
     */
    private void hu(YyMjPlayer player, List<Mj> majiangs, int action) {
...
//            }
            // 抢杠胡
            YyMjPlayer moGangPlayer = seatMap.get(nowDisCardSeat);
...
//                huBean.setTianhu(true);
        ...
        try {
            LogUtil.msgLog.info("yymj_room_hu:{},{},{},{},{},{},{},{},{},{},{},{}",
                    getId(),
                    getPlayBureau(),
                    disMajiang,
                    player.getSeat(),
            ...
            LogUtil.errorLog.error("yymj_room_hu error:{}", e);
        }
    ...
    private void checkRemoveMj(YyMjPlayer player, int action) {
        Mj mjB = null;
    ...
    private void removeGangMj(YyMjPlayer player, int mjId) {
     ...
     * @return
     */
    private YyMjPlayer getPlayerByHasMajiang(Mj majiang) {
        for (YyMjPlayer player : seatMap.values()) {
    ...
    private boolean isCalcOver() {
        ...
        if (!over) {
            YyMjPlayer disCSMajiangPlayer = seatMap.get(disCardSeat);
            for (int huseat : huActionList) {
                if (huConfirmMap.containsKey(huseat)) {
                    if (disCardRound == 0) {
                        // 天胡
                ...
                YyMjPlayer seatPlayer = seatMap.get(huseat);
     ...
     * @param action
     */
    private void chiPengGang(YyMjPlayer player, List<Mj> majiangs, int action) {
        if (action == MjDisAction.action_minggang && player.isPassGang(majiangs.get(0))) {  //過杠了不能再杠
            player.writeErrMsg("已经过杠的牌不能杠");
            return;
        }
        PlayMajiangRes.Builder builder = PlayMajiangRes.newBuilder();
    ...
    private void chiPengGang(PlayMajiangRes.Builder builder, YyMjPlayer player, List<Mj> majiangs, int action,
                             boolean moMj) {
        setIsBegin(false);
        processHideMj(player);
        ...
        for (YyMjPlayer seatPlayer : seatMap.values()) {
            // 推送消息
...
//                gangMoMajiang(player, majiangs.get(0), action);
     ...
     * 杠后摸两张牌
     */
    private void gangMoMajiang(YyMjPlayer player, Mj gangMajiang, int action) {
        if (state != table_state.play) {
            return;
        }
        if (player.isRobot()) {
...
//        }
    ...
    private void checkMoOutCard(List<Mj> list, YyMjPlayer player, int action) {
    ...
    private void gangNoticePlayer(YyMjPlayer player, Mj gangMajiang, List<Mj> moList) {
        // 发送摸牌消息res
    ...
    private boolean checkQGangHu(YyMjPlayer player, List<Mj> majiangs, int action) {
        PlayMajiangRes.Builder builder = PlayMajiangRes.newBuilder();
        ...
        if (!huListMap.isEmpty()) {
            ...
            for (Entry<Integer, List<Integer>> entry : huListMap.entrySet()) {
                PlayMajiangRes.Builder copy = builder.clone();
                YyMjPlayer seatPlayer = seatMap.get(entry.getKey());
    ...
    public void checkSendGangRes(Player player) {
        if (isHasGangAction()) {
            ...
            YyMjPlayer disPlayer = seatMap.get(disCardSeat);
     ...
     * @param action
     */
    private void disCard(YyMjPlayer player, List<Mj> majiangs, int action) {
        if (!gameModel.signTingAllOver()) {
        ...
        if(player.isSignTing()){
			ArrayList<Mj> mjs = new ArrayList<>(player.getHandMajiang());
			mjs.removeAll(majiangs);
			if(CollectionUtils.isEmpty(MjTool.getTingMjs(mjs, player.getGang(), player.getPeng(), player.getChi(),
					player.getBuzhang(), true, gameModel.getSpecialPlay().isOnlyDaHu(), gameModel.getSpecialPlay().isQuanQiuRenJiang() ? 1 : 0, this, player))){
				LogUtil.e("报听后不允许出其他牌!");
				return;
			}

        	if(!player.isSignCanAutoDisCard())
...
//		sendDisMajiangAction(builder);
        ...
        //是否要推送给玩家当前的操作列表
        boolean pushCard = false;
        //每个玩家首次出牌后都需要进行是否可以报听校验
		//庄家首次出牌
		//闲家首次即使选择过掉报听, 后续如可以继续报听也能选择报听
		if (player.isFirstDisCard()/*isBegin*/ && player.getHandMajiang().size() == 13 && !player.isSignTing()) {
			if (gameModel.getSignTing() == -1) {
				gameModel.setSignTing(0);
				player.setSignTing(0);
			} else {
...
//        else {
//			//第一轮包括闲家有权利在报听状态下,更改听牌牌型,更改后没有听牌则取消报听,能听则自动出牌报听
//			int firstCount = seatMap.values().stream().filter(v -> v.isFirstDisCard()).mapToInt(v -> 1).sum();
//
//			if (firstCount > 0) {
//				seatMap.values().stream().filter(v -> v.isSignTing()).forEach(v -> {
//					List<Mj> huCards = MjTool.getTingMjs(v.getHandMajiang(), player.getGang(), player.getPeng(), player.getChi(),
//							player.getBuzhang(), true, gameModel.getSpecialPlay().isOnlyDaHu(), gameModel.getSpecialPlay().isQuanQiuRenJiang() ? 1 : 0, this, player);
//					if (CollectionUtils.isEmpty(huCards)) {
//						//取消报听状态
//						v.setSignTing(3);
...
//						objects.addAll(Arrays.asList(0));
//						broadMsgToAll(ComMsg.ComRes.newBuilder().addAllParams(objects).setCode(WebSocketMsgType.req_code_tjmj_baoting_show).build());
//					}
//				});
//			}
//		}
     ...
     * @date 2019/11/11
     */
    private boolean checkDisCardAfter(YyMjPlayer otherSeat, List<Mj> majiangs, boolean next, YyMjPlayer selfSeat, List<Integer> actionList) {
        if (actionList.contains(1)) {
...
//            List<Integer> pActions = selfSeat.checkHu(majiangs.get(0), isBegin).buildDahuList();
			//报听仅可以接炮一次
            if (actionList.get(MjAction.QIANG_GANG_HU) != 1/*&& MjiangHu.calcMenZiScore(this, pActions) <= selfSeat.getTakeCardFlag()*/)
                if (gameModel.getSpecialPlay().isPassHuLimit() && (actionList.get(MjAction.HU) == 1 || /*actionList.get(MjAction.QIANG_GANG_HU) == 1 ||*/ actionList.get(MjAction.PENG) == 1 || actionList.get(MjAction.CHI) == 1)
                        && ((selfSeat.isSignTing() && selfSeat.getSignTingPao() == 1) || selfSeat.getTakeCardFlag() > 0)) {
                    actionList.set(MjAction.HU, 0);
            ...
            //平胡不可接炮
			//报听变为大门子
            if (actionList.get(MjAction.HU) == 1 && actionList.get(MjAction.PING_HU) == 1 && !selfSeat.isSignTing()/* && (!gameModel.isIgnite() || !gameModel.canIgnite(MjiangHu.calcMenZiScore(this, MjiangHu.calcDaHuBasicBase(pActions))))*/) {
//                //111
                actionList.set(MjAction.HU, 0);
     ...
     * @date 2019/10/21
     */
    public void sendDisMajiangAction(PlayMajiangRes.Builder builder) {
        // 如果有人可以胡 优先胡
        // 把胡的找出来
        buildPlayRes1(builder);
        List<Integer> huList = getHuSeatByActionMap();
    ...
    private void err(YyMjPlayer player, int action, String errMsg) {
     ...
     * @param action   出牌指令
     */
    public synchronized void playCommand(YyMjPlayer player, List<Mj> majiangs, int action) {
        ...
        if (player.isSignTing() && !player.isFirstDisCard() && gameModel.getSpecialPlay().signTingNotToDoActionDis(action) && !majiangs.isEmpty()/*&& nowMoCard != null  && majiangs.get(0).getId() != nowMoCard.getId()*/) {
			err(player, action, "报听后不能进行的操作" + majiangs + "action:" + action);
            return;
        }
     ...
     * @param action
     */
    public synchronized void moLastMajiang(YyMjPlayer player, int action, boolean gangHouMo) {
        ...
        if (action == MjDisAction.action_passmo) {
            ...
            moRes.setSeat(0/*player.getSeat()*/);
...
//            List<Integer> selfActList = player.checkMoCard(majiang, isBegin);
            List<Integer> selfActList = player.checkDisCard(majiang, gameModel.isNeed258(), gangHouMo, false);
...
//				}
            ...
            //没有海底炮
            // 自己能胡
			ComRes.Builder builder = ComRes.newBuilder();
            ...
            if (MjAction.hasHu(selfActList)) {
            	...
            	//强行自摸
				selfActList.set(0,0);
				selfActList.set(17,1);
				player.moMajiang(majiang);
                // 优先自己胡
				builder.setCode(50);		//海底亮牌效果
				builder.addParams(lastMajiang.getId());
				builder.addParams((int)player.getSeat());
				builder.addParams(1);
				seatMap.values().forEach(v -> v.writeSocket(builder.clone().build()));
				lastCanHu(gangHouMo, majiang, moRes, player, selfActList);
				return;
				...
				while (iterator.hasNext()) {
					YyMjPlayer next = iterator.next();
					if(next.getSeat() != player.getSeat()) {
						List<Integer> integers = next.checkDisCard(majiang, gameModel.isNeed258(), gangHouMo, false);
						if (MjAction.hasHu(integers)) {
							setMoLastMajiangSeat(next.getSeat());
							setMoMajiangSeat(next.getSeat());
							//强行自摸
							integers.set(0,0);
							integers.set(17,1);
							next.moMajiang(majiang);
							builder.addParams((int) next.getSeat());
							builder.addParams(1);
							seatMap.values().forEach(v -> v.writeSocket(builder.clone().build()));
						}
						if (lastCanHu(gangHouMo, majiang, moRes, next, integers)) {
							return;
						} else {
							builder.addParams((int) next.getSeat());
							builder.addParams(0);
				...
				seatMap.values().forEach(v -> {
					try{
						ComRes build = builder.clone().build();
//						System.out.println(build+"....................");
	...
	private boolean lastCanHu(boolean gangHouMo, Mj majiang, Builder moRes, YyMjPlayer next, List<Integer> integers) {
		integers = MjAction.keepHu(integers);
		if (integers != null && !integers.isEmpty()) {
			if (MjAction.hasHu(integers)) {
				addActionSeat(next.getSeat(), integers);

				//杠后摸得牌, 这里需要验证杠上花
				if (gangHouMo) {
					addGangActionSeat(nowMoCard.getId(), next.getSeat(), integers);
				}
			}
		}
		...
		buildPlayRes(builder, next, getDisEventAction(), getNowDisCardIds());
		builder.addAllSelfAct(integers);

		next.writeSocket(builder.build());

		//自动胡
		if(MjAction.hasHu(integers)){
			playCommand(next, Arrays.asList(majiang), MjDisAction.action_hu);
			return true;
		}
		return false;
	}

	private void chuLastPai(YyMjPlayer player) {
    ...
    private void passMoHu(YyMjPlayer player, List<Mj> majiangs, int action) {
        if (!moGangHuList.contains(player.getSeat())) {
            return;
        }
		...
		List<Integer> actionLists = removeActionSeat(player.getSeat());
        ...
        //自摸不算过胡,点炮算过胡
		if (!CollectionUtils.isEmpty(actionLists) && actionLists.get(MjAction.HU) == 1) {
        	player.setPassMajiangVal(nowDisCardIds.get(0).getVal());
		}

        YyMjPlayer moGangPlayer = seatMap.get(nowGangSeat);
        if (moGangHuList.isEmpty()) {
...
//                }
     ...
     * @param action
     */
    private void guo(YyMjPlayer player, List<Mj> majiangs, int action) {
        if (!actionSeatMap.containsKey(player.getSeat())) {
            return;
        }
...
//            player.addPassGangVal(CollectionUtils.isEmpty(majiangs) ? player.getHandMajiang().get(player.getHandMajiang().size() - 1).getVal() : majiangs.get(0).getVal());
            ...
            //检测是摸的杠还是打出来的杠，优先检测打出来的
            //打出来的杠，必须在手上有3张，才能明杠
            if(!CollectionUtils.isEmpty(getNowDisCardIds()) && player.getHandMajiang().stream().filter(v->v.getVal() == getNowDisCardIds().get(0).getVal()).count() == 3){
                player.addPassGangVal(getNowDisCardIds().get(0).getVal());
            } else if (getNowMoCard() != null && player.getPeng().stream().anyMatch(v->v.getVal() == getNowMoCard().getVal())) {
                //摸出来的必须peng了才能明杠
                player.addPassGangVal(getNowMoCard().getVal());
            }
        ...
        if (!actionSeatMap.isEmpty()) {
            ...
            for (int seat : actionSeatMap.keySet()) {
                ...
                YyMjPlayer seatPlayer = seatMap.get(seat);
                seatPlayer.writeSocket(copy.build());
            }
        }
...
//        if (isBegin && xiaoHu == -1 && player.getSeat() == lastWinSeat) {
     ...
     *@date 2019/12/20
     */
    private void calcGangScore(YyMjPlayer player, int action, int sameCount, List<Mj> majiangs, boolean robGang) {
...
//                }
    ...
    private void calcXiaoHuPoint(YyMjPlayer player, int xiaoIndex) {
...
//		}
    ...
    private void recordDisMajiang(List<Mj> majiangs, YyMjPlayer player) {
    ...
    protected void robotDealAction() {
        if (isTest()) {
            ...
            YyMjPlayer next = seatMap.get(nextseat);
...
//            leftMjs = leftMjs.subList(0, gameModel.getDiscardHoleCards());
    ...
    private void createKingCardAndFloorCard(Mj assign, List<Mj> leftMajiangs) {
        ...
        int floorCardIndex = leftMajiangs.size() - dealDice;
        LogUtil.printDebug("地牌INDEX:{}->{}", floorCardIndex, leftMajiangs.get(floorCardIndex));
        this.floorCard = leftMajiangs.get((floorCardIndex > leftMajiangs.size() ? leftMajiangs.size() : floorCardIndex) - 1);
//        this.floorCard = getLeftMajiangs().get((floorCardIndex > getLeftMajiangCount() ? getLeftMajiangCount() : floorCardIndex) - 1);
        if (assign != null) {
            floorCard = assign;
        }
...
//        }
     ...
     * @return
     */
    public Mj getLeftMajiang(YyMjPlayer player) {
...
//
    ...
    public CreateTableRes buildCreateTableRes(long userId, boolean isrecover, boolean isLastReady) {
        ...
        for (YyMjPlayer player : playerMap.values()) {
            ...
            if (/*!isHasGangAction(player.getSeat()) && */actionSeatMap.containsKey(player.getSeat())
            	...
            	//全部报听完毕才发送信息
                if (!tempActionMap.containsKey(player.getSeat()) && gameModel.signTingAllOver()) {// 如果已做临时操作
                    // 则不发送前端可做的操作
                    playerRes.addAllRecover(actionSeatMap.get(player.getSeat()));
                }else{
                	LogUtil.printDebug("玩家有玩家没有报听,隐藏操作...");
				}
            }
     ...
     * @return
     */
    private boolean canChi(YyMjPlayer player, List<Mj> handMajiang, List<Mj> majiangs, Mj disMajiang) {
        if (!actionSeatMap.containsKey(player.getSeat())) {
            return false;
        }
        ...
        List<Mj> chi = MjTool.checkChi(majiangs, disMajiang, player.getPlayingTable(YyMjTable.class));
        return !chi.isEmpty();
    }
     ...
     * @return
     */
    private boolean canPeng(YyMjPlayer player, List<Mj> majiangs, int sameCount, Mj disMajiang) {
        if (!actionSeatMap.containsKey(player.getSeat())) {
            return false;
        }
        if (player.noNeedMoCard()) {
     ...
     * @return
     */
    private boolean canAnGang(YyMjPlayer player, List<Mj> majiangs, int sameCount, int action) {
        if (sameCount != 4) {
            return false;
        }
     ...
     */
    private boolean checkAction(YyMjPlayer player, List<Mj> cardList, List<Integer> hucards, int action) {
        ...
        if (canAction == false) {// 不能操作时 存入临时操作
            ...
            if (tempActionMap.size() == actionSeatMap.size()) {
                ...
                YyMjPlayer tempPlayer = seatMap.get(maxSeat);
     ...
     * @param player
     */
    private void refreshTempAction(YyMjPlayer player) {
        ...
        while (iterator.hasNext()) {
            ...
            if (tempAction.getSeat() == maxPrioritySeat) {
                ...
                YyMjPlayer tempPlayer = seatMap.get(tempAction.getSeat());
     ...
     * @return
     */
    public boolean checkCanAction(YyMjPlayer player, int action) {
        // 优先度为胡杠补碰吃
     ...
     * @return
     */
    private boolean canMingGang(YyMjPlayer player, List<Mj> handMajiang, List<Mj> majiangs, int sameCount,
                                Mj disMajiang) {
     ...
     * @date 2019/9/16
     */
    private int calcFinalRatio(YyMjPlayer v) {
        //负分不做翻倍
    ...
    public ClosingMjInfoRes.Builder sendAccountsMsg(boolean over, boolean selfMo, List<Integer> winList,
        ...
        if (isCreditTable()) {
            ...
            for (ClosingMjPlayerInfoRes.Builder builder : builderList) {
                YyMjPlayer player = seatMap.get(builder.getSeat());
        ...
        if (seatBirdBigMap != null) {
            ...
            while (iterable.hasNext()) {
				...
				if (next.getE() != 0) {
					((YyMjPlayer) getSeatMap().get(next.getE())).changeActionTotal(MjAction.ACTION_COUNT_ZHONGNIAOSHU, 1);
				}
            }
...
//            sendDealInfo((TjMjPlayer) player);
            sendTingInfo((YyMjPlayer) player);
        }
...
//            YyMjPlayer bankPlayer = seatMap.get(lastWinSeat);
...
//        }
    ...
    public void checkAutoPlay() {
		...
		if(gameModel.signTingAllOver()) {
			for (YyMjPlayer player : seatMap.values()) {
				if ((gameModel.isGangAfterAutoPlay() && !player.getGang().isEmpty()) || (gameModel.getSpecialPlay().isSignTing() && player.isSignCanAutoDisCard())) {

					if ((player.isSignTing()) && player.noNeedMoCard() && getMoMajiangSeat() == player.getSeat()) {
						// 能胡牌就不自动打出去
						List<Integer> actionList = actionSeatMap.get(player.getSeat());
        ...
        //最后一圈
//        if (isFinalOneCircle() && getLeftMajiangCount() >= 0) {
...
//        }
     ...
     * @date 2019/10/26
     */
    private void systemAutoSignTingInfo(YyMjPlayer player) {
        try {
    ...
    public synchronized void autoPlay() {
        ...
        if (!actionSeatMap.isEmpty()) {
            ...
            YyMjPlayer player = seatMap.get(nowDisCardSeat);
    ...
    public void autoChuPai(YyMjPlayer player) {
    ...
    public void autoPiao(YyMjPlayer player) {
        int piaoPoint = 0;
        if (gameModel.getSpecialPlay().getTableStatus() != MjConstants.TABLE_STATUS_PIAO) {
            return;
        }
    ...
    public void createTable(Player player, int playType, int bureauCount, List<Integer> params, List<String> strParams,
        ...
        GameModel.SpecialPlay specialPlay = GameModel.SpecialPlay.builder()
                ...
                //上家：4、8、北
                //一炮多响时从点炮玩家计算
//                .buyBirdNum(StringUtil.getIntValue(params, 39, 0))
                //报听
                .signTing(true)
                //7小对
//                .sevenPairs(StringUtil.getIntValue(params, 6, 0) > 0)
                .sevenPairs(true)
                //豪华7小对
//                .superSevenPairs(StringUtil.getIntValue(params, 6, 0) > 0)
                .superSevenPairs(true)
                //超豪华7小对
//                .specialSuperSevenPairs(StringUtil.getIntValue(params, 8, 0) > 0)
                .specialSuperSevenPairs(true)
                //超超豪华7小对
//                .specialSSuperSevenPairs(StringUtil.getIntValue(params, 6, 0) > 0)
                .specialSSuperSevenPairs(true)
                //留底牌,默认6张
                .stayLastCard(0)
                //最后一圈
                .finalOneCircleCard(0)
                //自摸胡
...
//                .buChi(StringUtil.getIntValue(params, 15, 0) == 0)
                .buChi(true)
                //杠摸3张
//                .gangMoNum(StringUtil.getIntValue(params, 16, 0) > 0 ? 1 : 1)
//                .gangMoNum(1)
                //鸟不落空
//                .birdOption(StringUtil.getIntValue(params, 17, 0) > 0 ? 2 : 0)
				...
				.jjHu(true)
				//有喜
				.oneCharPryAdditonScore(StringUtil.getIntValue(params, 8, 0) == 1 ? 2 : 0)
				//门清
				.menqing(StringUtil.getIntValue(params, 16, 0) == 1)
				//一条龙
				.dragon(true)
                .build();
...
//                .calcBanker(StringUtil.getIntValue(params, 18, 0) > 0)
                ...
                //点炮设置
                .ignite(StringUtil.getIntValue(params, 25, 0) == 1)
                //需要听牌才能杠
                .needTingGang(false)
				//门清将将胡可接炮
				.mQJJHuInvite(StringUtil.getIntValue(params, 14, 0) == 1)
				//封顶
				.topFen(StringUtil.getIntValue(params, 13, 0))
				//门清7小对
//				.mQSevenDui(StringUtil.getIntValue(params, 15, 0) == 1)
				.huangZhuangMisFen(StringUtil.getIntValue(params, 12, 0))
				//0随机 1先进房坐庄
				.changeBankerWay(1)
				.noneChar(true)
				.noneWind(true)
        ...
        //最后一圈等于底牌+1张
		gameModel.getSpecialPlay().setFinalOneCircleCard(gameModel.getSpecialPlay().getStayLastCard() + 1);
...
////            gameModel.setIgnite(false);
//            gameModel.setIgniteScoreArrive(20);
//        }
...
//            gameModel.setIgniteScoreArrive(gameModel.getNMulNotCalcScore());
     ...
     * @date 2019/10/16
     */
    public void sendTingInfo(YyMjPlayer player) {
    ...
    public boolean checkSignTingInfo() {
        ...
        if (gameModel.getSpecialPlay().isSignTing() && !gameModel.signTingAllOver()) {
            for (Iterator<YyMjPlayer> iterator = seatMap.values().iterator(); iterator.hasNext(); ) {
                YyMjPlayer player = iterator.next();
                List<Mj> cards = new ArrayList<>(player.getHandMajiang());
                List<Mj> huCards = MjTool.getTingMjs(cards, player.getGang(), player.getPeng(), player.getChi(),
                        player.getBuzhang(), true, gameModel.getSpecialPlay().isOnlyDaHu(), gameModel.getSpecialPlay().isQuanQiuRenJiang() ? 1 : 0, this, player);
    ...
    public void logFaPaiPlayer(YyMjPlayer player, List<Integer> actList) {
        StringBuilder sb = new StringBuilder();
        sb.append("265YyMj");
        sb.append("|").append(getId());
        sb.append("|").append(getPlayBureau());
    ...
    public void logMoMj(YyMjPlayer player, Mj mj, List<Integer> actList) {
        StringBuilder sb = new StringBuilder();
        sb.append("265YyMj");
        sb.append("|").append(getId());
        sb.append("|").append(getPlayBureau());
    ...
    public void logChuPaiActList(YyMjPlayer player, Mj mj, List<Integer> actList) {
        StringBuilder sb = new StringBuilder();
        sb.append("265YyMj");
        sb.append("|").append(getId());
        sb.append("|").append(getPlayBureau());
    ...
    public void logAction(YyMjPlayer player, int action, int xiaoHuType, List<Mj> mjs, List<Integer> actList) {
        StringBuilder sb = new StringBuilder();
        sb.append("265YyMj");
        sb.append("|").append(getId());
        sb.append("|").append(getPlayBureau());
     ...
     * @param player
     */
    public void processHideMj(YyMjPlayer player) {
...
//        if ((isPlayerAllReady() && getState() != SharedConstants.table_state.play) || (!isBegin && getState() == SharedConstants.table_state.play)) {
        if (super.isAllReady()) {
            if (gameModel.getSpecialPlay().getKePiao() == 1) {
                ...
                // 机器人默认处理
                if (this.isTest()) {
                    for (YyMjPlayer robotPlayer : seatMap.values()) {
    ...
    public void checkClearGangDisMajiang() {
        ...
        if (moList != null && moList.size() > 0 && actionSeatMap.isEmpty()) {
            YyMjPlayer player = seatMap.get(getMoMajiangSeat());
            for (YyMjPlayer seatPlayer : seatMap.values()) {
                GangMoMajiangRes.Builder gangbuilder = GangMoMajiangRes.newBuilder();
                gangbuilder.setRemain(leftMajiangs.size());
                gangbuilder.setGangId(gangMajiang.getId());
     ...
     */
    public void sendMoLast(YyMjPlayer player, int state) {
