...
import com.sy599.game.qipai.yymj.command.com.MjComCommand;
import com.sy599.game.qipai.yymj.command.play.MjPlayCommand;
...
public class MjCommandProcessor extends AbstractBaseCommandProcessor {
    ...
    public void process(Player player, MessageUnit message) {
        int code = 0;
        try {
            if (player == null || !(player instanceof YyMjPlayer)) {
                return;
            }
            YyMjPlayer csMajiangPlayer = player.getPlayer(processor);
