...
//    private int autoOutCard;
    ...
    */
    ...
    //true全名堂 false土炮胡
    private int mingTangPlayT;
    //1大六八番 2小六八番
    private int bigSexEight;

    //分数封顶
    private int topScore;
    
    private int kepiao;//固定飘分
    private int guchou;//钴臭
    private int xuanXiang;//选项
    private int chongfeng;//冲分
    ...
    @JSONField(serialize = false)
    public int bigSexEightMinSexEight(){
//		return getBigSexEight() == 1 ? 0 : getBigSexEight() == 2 ? 1 : 0;
    	return 0;
	}
    ...
    public int limitUpper(HuType huType, int huxi) {
        if (maxHuXiLimit != null && huType != null && huType.ordinal() <= maxHuXiLimit.length && maxHuXiLimit[huType.ordinal()]>0) {
    ...
    public static class SpecialPlay {
        ...
        //自摸翻倍
        private boolean sinceTouchFanBei;
        //一五十
        ...
        //3提5坎
        private boolean threeTiFiveKan;
        //十八小
        private boolean eighteenMin;
        //对对胡
        private boolean ddHu;
        //海胡
        private boolean seaHu;
        //听胡
        private boolean tingHu;
        //耍猴
        private boolean monkeyShowHu;
        //团胡
        private boolean groupHu;
        //偎牌 1明偎,0暗偎
        private boolean weiWay;
        //放招开关
        private boolean fangZhao;
		//爬坡,黄庄之后结算倍率*n,这部分倍率直到不黄庄结束
		//爬坡倍率
		private boolean uphill;
		//红乌
		private boolean redWu;
		//行行息
		private boolean hanghangX;
		//假行行
		private boolean jiaHangHang;
		//背靠背
		private boolean kappa;
		//四七红
		private boolean siQiHong;
		
		//亮张
		private boolean liangZhang;
