package com.sy599.game.qipai.xpepaohuzi.command;
...
import com.sy599.game.qipai.xpepaohuzi.command.com.ComCommand;
import com.sy599.game.qipai.xpepaohuzi.command.play.PlayCommand;
...
public class CommandProcessor extends AbstractBaseCommandProcessor {
    ...
    public void process(XPPaohuziPlayer player, MessageUnit message) {
        int code = 0;
        try {
    ...
    public void process(Player player, MessageUnit message) {
        ...
        int code = 0;
        try {
            XPPaohuziPlayer pdkPlayer = player.getPlayer(processor);
            BaseCommand action = ObjectUtil.newInstance(commandMap.get(message.getMsgType()));
            action.setPlayer(player);
