package com.sy599.game.qipai.xxpaohuzi.command;

import com.sy599.game.character.Player;
import com.sy599.game.gcommand.BaseCommand;
import com.sy599.game.qipai.xxpaohuzi.command.com.ComCommand;
import com.sy599.game.qipai.xxpaohuzi.command.play.PlayCommand;
...
public class CommandProcessor extends AbstractBaseCommandProcessor {
    ...
    public void process(XxPaohuziPlayer player, MessageUnit message) {
        int code = 0;
        try {
    ...
    public void process(Player player, MessageUnit message) {
        ...
        int code = 0;
        try {
            XxPaohuziPlayer pdkPlayer = player.getPlayer(processor);
            BaseCommand action = ObjectUtil.newInstance(commandMap.get(message.getMsgType()));
            action.setPlayer(player);
