...
import com.sy599.game.qipai.xxpaohuzi.command.CommandProcessor;
import com.sy599.game.qipai.xxpaohuzi.constant.EnumHelper;
 ...
 */
@Data
public class XxPaohuziPlayer extends Player {
    // 座位id
    private int seat;
    // 状态
    ...
    */
    ...
    public void addCardType(int action, List<PaohzCard> disCardList) {
        int huxi = PaohuziTool.getOutCardHuxi(action, disCardList, getPlayingTable(XxPaohuziTable.class).getGameModel());
        if (action != 0) {
...
//            this.isSendChui = StringUtil.getIntValue(values, i++);
    ...
    public void clearTableInfo() {
        XxPaohuziTable table = getPlayingTable();
    ...
    public PaohuziHuLack checkPaoHu(PaohzCard card, boolean isSelfMo, boolean canDiHu) {
        ...
        //胡牌计算
        PaohuziHuLack lack = PaohuziTool.isHuNew(PaohuziTool.getPaohuziHandCardBean(handCardsCopy), null, isSelfMo, totalHuxi, true, true, getPlayingTable(XxPaohuziTable.class).getGameModel());
        ...
        if (lack.isHu()) {
            ...
            XxPaohuziTable table=getPlayingTable(XxPaohuziTable.class);
    ...
    public PaohuziHuLack checkHu(PaohzCard card, boolean isSelfMo) {
        ...
        PaohuziHuLack lack = PaohuziTool.isHuNew(PaohuziTool.getPaohuziHandCardBean(handCopy), card, isSelfMo, outHuXi, isNeedDui(), isPaoHu, getPlayingTable(XxPaohuziTable.class).getGameModel());
        ...
        if (lack.isHu()) {
            if (lack.getHuxi() + outHuXi + zaiHuxi < ((XxPaohuziTable) this.getPlayingTable()).getGameModel().getRoundFinishLowestHuXi()) {
                // 10胡息能胡
                lack.setHu(false);
            } else {
                setHuxi(lack.getHuxi());
    ...
    public PaohuziCheckCardBean checkCard(PaohzCard card, boolean isSelfMo, boolean isPassHu, boolean isBegin, boolean isFirstCard, boolean isPass) {
        ...
        if (card != null) {
            ...
            if (((XxPaohuziTable) getPlayingTable()).getGameModel().getSpecialPlay().isIgnite() && !currentCardIsMoFlag && !isPass) {
     ...
     * @date 2019/9/4
     */
    private void checkChiOrPassChi(PaohzCard card, boolean isSelfMo, boolean isPass, PaohuziCheckCardBean check, boolean isCanChiPeng, XxPaohuziTable table, boolean currentCardIsMoFlag, List<Integer> fixdOutVals) {
        ...
        if (isCanChiPeng && !check.isTi() && !check.isPao() && isCheckChi && !isPass) {
            //上家检测
            XxPaohuziPlayer frontPlayer = (XxPaohuziPlayer) table.getPlayerBySeat(table.calcFrontSeat(seat));
    ...
    public List<PaohzCard> getChiList(PaohzCard card, List<PaohzCard> cardList) {
        ...
        if (cardList != null) {
            ...
            List<PaohzCard> chiList = PaohuziTool.checkChi(copy, card, getPlayingTable(XxPaohuziTable.class).getGameModel());
            ...
            if (sameList == null || sameList.isEmpty()) {
                ...
                for (PaohzCard sameCard : sameList) {
                    ...
                    // 相同的牌还能不能继续吃
                    List<PaohzCard> samechiList = PaohuziTool.checkChi(copy, sameCard, getPlayingTable(XxPaohuziTable.class).getGameModel());
            ...
            cardList = PaohuziTool.checkChi(handCard.getOperateCards(), card, getPlayingTable(XxPaohuziTable.class).getGameModel());
        ...
        if (sameList == null || sameList.isEmpty()) {
            ...
            for (PaohzCard sameCard : sameList) {
                ...
                // 相同的牌还能不能继续吃
                List<PaohzCard> samechiList = PaohuziTool.checkChi(handCard.getOperateCards(), sameCard, getPlayingTable(XxPaohuziTable.class).getGameModel());
                if (samechiList == null || samechiList.isEmpty()) {
                    // 如果不能吃 则这个牌不能吃
                    return null;
                }
...
//        sb.append(winLossPoint).append(",");
    ...
    public PlayerInTableRes.Builder buildPlayInTableInfo(long lookUid, boolean isrecover) {
        ...
        if (seat == table.getShuXingSeat()) {
            XxPaohuziPlayer paohuziPlayer = (XxPaohuziPlayer) table.getSeatMap().get(table.getLastWinSeat());
...
//        res.addExt(chui);//8
    ...
    public List<PhzHuCards> buildPhzCards(XxPaohuziTable table, long lookUid) {
        List<PhzHuCards> list = new ArrayList<>();
        for (CardTypeHuxi type : cardTypes) {
            if (type.getAction() == PaohzDisAction.action_zai) {
                // 不是本人并且是栽
