...
public class ZjMjCardDisType {
	...
	public boolean isHasCard(ZjMj card) {
	...
	public boolean isHasCardVal(int val) {
		if (cardIds != null) {
			List<ZjMj> majiangs = ZjMjQipaiTool.toMajiang(cardIds);
			List<Integer> vals = ZjMjQipaiTool.toMajiangVals(majiangs);
			return vals.contains(val);
		}
		return false;

	}
	public boolean isHasCardVal(ZjMj card) {
		if (cardIds != null) {
			List<ZjMj> majiangs = ZjMjQipaiTool.toMajiang(cardIds);
			List<Integer> vals = ZjMjQipaiTool.toMajiangVals(majiangs);
	...
	public int removeCardVal(int val) {
		if (cardIds != null) {
			...
			while (iterator.hasNext()) {
				...
				ZjMj majiang = ZjMj.getMajang(id);
