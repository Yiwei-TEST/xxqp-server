package com.sy599.game.qipai.zjmj.tool.hulib.util;
...
public class HuUtil {
    public static final String source_dir = "zjmjtbl/";
    public static final String dump_file_dir = "D:\\develop\\workspace\\2d\\qipai_all_2\\game-mj-zjmj\\src\\main\\resources\\zjmjtbl\\";
    ...
    public static boolean isCanHu(List<ZjMj> cards, int gui_num) {
    ...
    //将的下标
    private static final List<Integer> jiangIndex = Arrays.asList(1,4,7,10,13,16,19,22,25);
    public static boolean isCanHuJiangJiang(int[] cardsArr, int gui_num) {
    	boolean isAllJiang = true;
        for (int i = 0; i < cardsArr.length; i++) {
        	if(!jiangIndex.contains(i) && cardsArr[i] > 0){
        		return false;
        	}
        }
        return isAllJiang;
    ...
    public static List<ZjMj> getLackPaiList(List<ZjMj> cards, int gui_num) {
        int[] cardsArr = toCardArray(cards);
        boolean any = false;
        if (gui_num > 0) {
    ...
    public static int[] toCardArray(List<ZjMj> cards) {
        int[] cardsArr = new int[34];
        Arrays.fill(cardsArr, 0);

        for (int i = 0; i < cards.size(); i++) {
            ZjMj card = cards.get(i);
    ...
    private static ZjMj index2Majiang(int index) {
        int type = index / 9 + 1;
        ...
        return ZjMj.getMajiangByVal(val);
    }

    private static List<ZjMj> toRawCards(List<Integer> list) {
        List<ZjMj> retList = new ArrayList<>();
        for (int index : list) {
    ...
    public static int getMjIndex(ZjMj card) {
