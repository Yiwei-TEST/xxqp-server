...
public class CxMjResTool {
    public static void buildPlayRes(PlayMajiangRes.Builder builder, Player player, int action, List<CxMj> majiangs) {
        List<Integer> list = CxMjHelper.toMajiangIds(majiangs);
     ...
     * @return
     */
    public static PlayMajiangRes.Builder buildDisMajiangRes(Player player, CxMj majiang) {
        PlayMajiangRes.Builder builder = PlayMajiangRes.newBuilder();
        builder.setAction(0);
        builder.setUserId(player.getUserId() + "");
        builder.setSeat(player.getSeat());
