 ...
 * @author upstream
 */
public class CxMjIndexArr {
    CxMjIndex[] a = new CxMjIndex[5];

    public void addMajiangIndex(int count, List<CxMj> majiangList, int val) {
        if (a[count] == null) {
            a[count] = new CxMjIndex();
        }
     ...
     * @return
     */
    public Map<Integer, List<CxMj>> getMajiangMap(int size) {
        Map<Integer, List<CxMj>> map = new HashMap<>();
        for (int i = 0; i < a.length; i++) {
            if (size <= i + 1) {
                CxMjIndex majiangIndex = a[i];
    ...
    public Map<Integer, List<CxMj>> getJiang(boolean need258) {
        Map<Integer, List<CxMj>> map = new HashMap<>();
        for (int i = 0; i < a.length; i++) {
            if (2 <= i + 1) {
                CxMjIndex majiangIndex = a[i];
                if (majiangIndex != null) {
                    if (need258) {
                        for (Entry<Integer, List<CxMj>> entry : majiangIndex.getMajiangValMap().entrySet()) {
     ...
     * @return
     */
    public CxMjIndex getMajiangIndex(int index) {
        CxMjIndex majiangIndex = a[index];
        if (majiangIndex == null) {
