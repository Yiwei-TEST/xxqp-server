package com.sy599.game.qipai.wcphz.tool;

import com.sy599.game.GameServerConfig;
import com.sy599.game.qipai.wcphz.bean.CardTypeHuxi;
import com.sy599.game.qipai.wcphz.bean.GameModel.SpecialPlay;
...
import com.sy599.game.qipai.wcphz.bean.GameModel;
import com.sy599.game.qipai.wcphz.bean.PaohuziHandCard;
import com.sy599.game.qipai.wcphz.bean.PaohuziHuBean;
import com.sy599.game.qipai.wcphz.bean.PaohzDisAction;
import com.sy599.game.qipai.wcphz.rule.PaohuziMingTangRule.FindAnyCombo.FindAny;
...
public class PaohuziTool {
    ...
    public static List<List<PaohzCard>> getAllSeatAllCardNew(List<Integer> initAllCardsCombo, List<List<Integer>> gmOptCards, GameModel gameModel) {
        LogUtil.msg("StartSkyFloorHuBarrier...");
...
//        }
    ...
    public static List<Integer> listenCardBarrier(List<PaohzCard> cards, LinkedList<Integer> leftCards, GameModel gameModel) {
        ...
        //匹配所有顺子组合
        List<FindAny> findCombos = new ArrayList<>();
...
//            List<List<PaohzCard>> allSeatAllCardNew = getAllSeatAllCardNew(PaohuziConstant.cardList, new ArrayList<>(), GameModel.builder().specialPlay(GameModel.SpecialPlay.builder().build()).build());
//
//            if (CollectionUtils.isEmpty(allSeatAllCardNew) || allSeatAllCardNew.size() <= 3 ||
//                    allSeatAllCardNew.get(0).size()!=21||
//                    allSeatAllCardNew.get(1).size()!=20||
//                    allSeatAllCardNew.get(2).size()!=20||
//                    allSeatAllCardNew.get(3).size()!=19) {
//                System.out.println("System Exception....");
//                System.exit(-1);
//            }
//
//            HashMap<Integer, Integer> all = new HashMap<>();
//            Iterator<List<PaohzCard>> iterator = allSeatAllCardNew.iterator();
//            while (iterator.hasNext()) {
//                List<PaohzCard> next =  iterator.next();
//
//                for (PaohzCard paohzCard : next) {
//                    all.merge(paohzCard.getVal(), 1, (oV, nV) -> oV + nV);
//                    if (all.get(paohzCard.getVal()) > 4) {
//                        System.out.println("Found card number > 4");
//                        System.exit(-2);
//                    }
//                }
//            }
//
//            System.out.println(allSeatAllCardNew.size() + ":"+ (i+1));
//        }

		PaohuziHuLack lack = PaohuziTool.isHuNew(PaohuziTool.getPaohuziHandCardBean(new ArrayList<>(Arrays.asList(PaohzCard.phz1,PaohzCard.phz11,PaohzCard.phz2,PaohzCard.phz7,PaohzCard.phz25,PaohzCard.phz15,PaohzCard.phz25))), null, true, 21, true, false,
				GameModel.builder()
						.specialPlay(SpecialPlay.builder().build())
						.build(),
				true, new WcPaohuziPlayer());

		System.out.println(lack.isHu());
		for (CardTypeHuxi h : lack.getPhzHuCards()){
			System.out.println(h.getAction());
			System.out.println(h.getHux());
			System.out.println(h.getCardIds());
		}
...
//            List<PaohzCard> pcards2 = new ArrayList<>();
//            List<PaohzCard> pcards3 = new ArrayList<>();
//            List<Integer> lfs = new ArrayList<>(PaohuziConstant.cardList);
...
//
//            int k = 0 ;
//            Iterator<Integer> iterator2 = lfs.iterator();
//            while (iterator2.hasNext()) {
//                Integer next = iterator2.next();
//                pcards2.add(PaohzCard.getPaohzCard(next));
//                iterator2.remove();
//                if (++k >= 20) {
//                    break;
//                }
...
//
//            int l = 0 ;
//            Iterator<Integer> iterator3 = lfs.iterator();
//            while (iterator3.hasNext()) {
//                Integer next = iterator3.next();
//                pcards3.add(PaohzCard.getPaohzCard(next));
//                iterator3.remove();
//                if (++l >= 20) {
//                    break;
//                }
...
//                System.out.println("牌型111"+pcards);
//                lfs = listenCardBarrier(pcards, new LinkedList<>(lfs), null);
//                System.out.println("牌型222"+pcards2);
//                lfs = listenCardBarrier(pcards2, new LinkedList<>(lfs), null);
//                System.out.println("牌型333"+pcards3);
//                lfs = listenCardBarrier(pcards3, new LinkedList<>(lfs), null);
...
//            System.out.println();
//            System.out.println();
//            System.out.println("玩家1"+pcards);
//            System.out.println("玩家2"+pcards2);
//            System.out.println("玩家3"+pcards3);
//            System.out.println("鱼排数量:"+lfs.size());
//
//            HashMap<Integer, Integer> hashMap = new HashMap();
//
//            List[] sss = {pcards,pcards2,pcards3};
//
//            for (List<PaohzCard> ss : sss) {
//                ss.forEach(v -> hashMap.merge(v.getVal(), 1, (oV, nV) -> oV + nV));
//
//                if (hashMap.values().stream().anyMatch(v->v > 4)) {
//                    System.out.println(hashMap);
//                    System.out.println(lfs);
//                    System.out.println("异常牌型!!");
//                    System.exit(1);
//                }
//            }
//
...
//    }
     ...
     * 是否一样的牌
     */
    public static boolean isSameCard(List<PaohzCard> handCards, boolean kingCard) {
        int val = 0;
        for (PaohzCard card : handCards) {
            ...
            //癞子
			if(kingCard && card.getVal() == 999){
			}else if (val != card.getVal()) {
				return false;
            }
        }
        return true;
...
//    }
    ...
    private static boolean chaishun3(PaohuziHuLack lack, List<PaohzCard> hasPais, PaohzCard minCard, boolean isShun, boolean check2710, int pai1, int pai2, int pai3, GameModel gameModel) {
        ...
        if (lackNum > 0) {
            ...
            if (isShun) {
                ...
                if (isSameCard(hasMajiangList, false)) {
                    // 如果是三个一模一样的
    ...
    private static int getShunHuxi(List<PaohzCard> hasMajiangList, GameModel gameModel) {
		...
		if (hasMajiangList.get(0).getPai() == hasMajiangList.get(1).getPai() || hasMajiangList.get(0).getPai() == hasMajiangList.get(2).getPai() || hasMajiangList.get(1).getPai() == hasMajiangList.get(2).getPai()) {
            //不是顺子
            return 0;
        }

        PaohzCard minCard = hasMajiangList.get(0);
        boolean bigCard = (minCard.getVal() == 999 ? hasMajiangList.get(1): minCard).isBig();

        if (minCard.getPai() == 1 || new PaohuziMingTangRule.FindAnyCombo(
				new Integer[]{99, 2, 3},
				new Integer[]{1, 99, 3},
				new Integer[]{1, 2, 99}
		).anyComboMath(hasMajiangList.stream().map(v -> v.getPai())).isFind()) {
            if (bigCard) {
                return 6;
            } else {
                return 3;
            ...
            if (gameModel != null && gameModel.getSpecialPlay().isOneFiveTen()) {
                if (!new PaohuziMingTangRule.FindAnyCombo(
                        ...
                        new Integer[]{101, 105, 999},
                        new Integer[]{2, 7, 10},
                        new Integer[]{2, 7, 999},
                        new Integer[]{2, 999, 10},
                        new Integer[]{999, 7, 10},
                        new Integer[]{102, 107, 110},
						new Integer[]{102, 107, 999},
						new Integer[]{102, 999, 107},
						new Integer[]{999, 107, 110}
                        )
...
//    }
     ...
     * @date 2019/9/4
     */
    public static PaohuziHuLack isHuNew(PaohuziHandCard handCardBean, PaohzCard disCard, boolean isSelfMo, int outCardHuxi, boolean needDui, boolean isPaoHu, GameModel gameModel, boolean isTing, WcPaohuziPlayer player) {

		PaohuziHuLack lack = new PaohuziHuLack(1/*!CollectionUtils.isEmpty(handCardBean.getOperateCards()) && handCardBean.getOperateCards().size() % 3 != 2 ? 1 : 0*/);
        lack.setSelfMo(isSelfMo);
        lack.setCheckCard(disCard);
        lack.setNeedDui(needDui);
		lack.setPaoHu(isPaoHu);
		lack.setCardSrcSize(handCardBean.getHandCards().size());
        ...
        if (index2 != null) {
            ...
            for (int val : list) {
                ...
                //已经在手上的
				//底下没跑，手里有一坎+一对，可以按清的分数算。（如手里有三个大捌、两个小二，胡牌时这个牌可算12胡；或者手里有两个大捌，两个小二，胡大捌的情况下，闲家打的大捌，可算9胡息，自己抓的大捌可算12胡息）
				if(player.notFoundTiOrKanOrPao() && (handCardBean.getHandCards().size() == 4 || handCardBean.getHandCards().size() == 5)){
					//大字6分小字3分
					int huxi = val > 100 ? 12 : 9;
					lack.changeHuxi(huxi);
					List<PaohzCard> cards = index2.getPaohzValMap().get(val);
					lack.addPhzHuCards(PaohzDisAction.action_ti, toPhzCardIds(cards), huxi);
				}else{
					//大字6分小字3分
        ...
        if (fourNumberIndex != null) {
            ...
            for (int val : list) {
				...
				if ((disCard != null && val == disCard.getVal()) /*&& isTing*/) {
					// 抓到手上的牌可以不用强制组出4个
					handCardBean.getOperateCards().addAll(fourNumberIndex.getPaohzValMap().get(val));
					continue;
				}
        ...
        if (handPais.size() == 0) {
            ...
            sortMin(handPais);
            chaiPaiNew(huList, lack, handPais, disCard, gameModel, player.notFoundTiOrKanOrPao(), isTing);
            if (huList.size() > 0) {
    ...
    /**
     *@description
     *@param tiScore 按提(清)的分数算分
     *@return
     *@author Guang.OuYang
     *@date 2020/4/30
     */
    public static void chaiPaiNew(List<PaohuziHuLack> huList, PaohuziHuLack lack, List<PaohzCard> handPais, PaohzCard disCard, GameModel gameModel, boolean tiScore, boolean isTing) {
        if (lack.isNeedDui()) {
            //拆对
            ...
            if (duiZi != null && duiZi.getPaohzValMap().size() > 0) {
                for (Integer val : duiZi.getPaohzValMap().keySet()) {
					...
					createNewLackOrChaiPaiNew(huList, lack, disCard, gameModel, val, handPaisCopy, duiZiIdList, newLack, isTing);
				}
            }
			...
			PaohuziIndex single = indexArr.getPaohzCardIndex(0);
			if (single != null && single.getPaohzValMap().size() > 0 && lack.getHongzhongNum() > 0) {
				for (Integer v : single.getPaohzValMap().keySet()) {
					if(v == 999) continue;
					List<PaohzCard> handPaisCopy = new ArrayList<>(handPais);

					List<PaohzCard> duiZiCardList = single.getPaohzValMap().get(v);

					List<Integer> duiZiIdList = new ArrayList<>();

					for (PaohzCard card : duiZiCardList) {
						duiZiIdList.add(card.getId());
						duiZiIdList.add(PaohzCard.phz999.getId());
					}

					handPaisCopy.removeIf(v1 -> duiZiIdList.stream().anyMatch(v2 -> v2 == v1.getId()));

					PaohuziHuLack newLack = lack.clone();
					newLack.setNeedDui(false);
					newLack.changeHongzhong(-1);

					createNewLackOrChaiPaiNew(huList, lack, disCard, gameModel, v, handPaisCopy, duiZiIdList, newLack, isTing);
				}
			}

			//都没有 随便找单牌
//			if(!CollectionUtils.isEmpty(bean.getHandCards())){
//				for(int i = 0 ; i < handPais.size();i++){
//					List<PaohzCard> handPaisCopy = new ArrayList<>(bean.getHandCards());
//					int v = handPaisCopy.get(i).getVal();
//					List<PaohzCard> duiZiCardList = Arrays.asList(handPaisCopy.get(i));
//					handPaisCopy.removeAll(duiZiCardList);
//
//					List<Integer> duiZiIdList = new ArrayList<>();
//
...
//			}
            ...
            if (handPais.size() + lack.getHongzhongNum() < 3) {
                return;
            }
            //拆顺
            int val;
            ...
            boolean kingCard = false;
//            for(int i = 0 ; i <= 1; i++){
				for (int[] paiZu : paiZus) {
//					if (!gameModel.getSpecialPlay().isOneFiveTen() &&
//							new PaohuziMingTangRule.FindAnyCombo(
//									new Integer[]{1, 5, 10},
//									new Integer[]{101, 105, 110}).anyComboMath(IntStream.of(paiZu).mapToObj(v -> Integer.valueOf(v))).isFind()) {
//						continue;
//					}
					handPaisCopy = new ArrayList<>(handPais);

					rmList = removeVals(handPaisCopy, paiZu, lack, kingCard);
					...
					newLack = lack.clone();
					//最后摸起来的
					if (!lack.isSelfMo() && disCard != null && disCard.getVal() == val && isSameCard(rmList, true)) {
						// 三张一模一样的，碰牌分
						int huxi = disCard.isBig() ? 3 : 1;
						newLack.addPhzHuCards(PaohzDisAction.action_peng, toPhzCardIds(rmList), huxi);
						newLack.changeHuxi(huxi);
					}  else if(isSameCard(rmList, true)){
						// 癞子或3张一样的, 坎牌分
						int huxi = PaohzCard.getPaohzCardsByVal(val).get(0).isBig() ? 6 : 3;
						newLack.addPhzHuCards(PaohzDisAction.action_kan, toPhzCardIds(rmList), huxi);
						newLack.changeHuxi(huxi);
					} else {
					...
					if (handPaisCopy.size() == 0) {
						...
						chaiPaiNew(huList, newLack, handPaisCopy, null, gameModel, tiScore, isTing);
					}
				}
...
//			}
	...
	private static void createNewLackOrChaiPaiNew(List<PaohuziHuLack> huList, PaohuziHuLack lack, PaohzCard disCard, GameModel gameModel, Integer v, List<PaohzCard> handPaisCopy, List<Integer> duiZiIdList, PaohuziHuLack newLack, boolean isTing) {
		...
		if (handPaisCopy.size() == 0) {
			...
			if (disCard != null && disCard.getVal() == v) {
				chaiPaiNew(huList, newLack, handPaisCopy, null, gameModel, lack.isPaoHu(), isTing);
			}
			else {
				chaiPaiNew(huList, newLack, handPaisCopy, disCard, gameModel, lack.isPaoHu(), isTing);
			}
		}
	}

	public static List<PaohzCard> removeVals(List<PaohzCard> cards, int[] vals, PaohuziHuLack lack, boolean kingCard) {
    	int hongZhong = kingCard ? lack.getHongzhongNum() : 0;
        List<PaohzCard> rmList = new ArrayList<>(vals.length);
        int notFindNum = 0;
        for (Integer val : vals) {
            boolean hasVal = false;
            for (PaohzCard card : cards) {
				if ((card.getVal() == val || (card.getVal() == 999 && lack.getHongzhongNum() > 0)) && !rmList.contains(card)) {
					if(card.getVal() == 999){
						lack.changeHongzhong(-notFindNum);
					}
                    rmList.add(card);
                    hasVal = true;
                    break;
           	...
           	if (!hasVal && ++notFindNum > hongZhong) {
                return null;
            }
        }

		//癞子可以通过
		if (rmList.size() == vals.length - 1 && lack.getHongzhongNum() > 0) {
			rmList.add(PaohzCard.phz999);
			lack.changeHongzhong(-notFindNum);
			cards.removeAll(rmList);
			return rmList;
		}else if (rmList.size() == vals.length) {
            cards.removeAll(rmList);
            return rmList;
        }
        return null;
