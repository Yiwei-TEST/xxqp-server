package com.sy599.game.qipai.wcphz.rule;
...
public class PaohuziMingTangRule {
     ...
     * @return
     */
    public static List<Integer> calcMingTang(WcPaohuziPlayer player) {
        ...
        HuType huType = ((WcPaohuziTable) player.getPlayingTable()).calcHuType();
...
//        }
    ...
    private static boolean isPPHu(WcPaohuziPlayer player) {
        ...
        if (ppHu && player.getHandPais().size() >= 3) {
            ...
            //除非天胡否则把最后摸出来的牌算入
            if (!player.isFirstDisSingleCard() && !handCards.contains(player.getPlayingTable(WcPaohuziTable.class).getNowDisCardIds().get(0))) {
                //胡牌时把最后张底牌算进去,这张牌的组合不确定所以把玩家所有牌都算入
                handCards.add(player.getPlayingTable(WcPaohuziTable.class).getNowDisCardIds().get(0));
            }
...
//            ppHu = !new FindAnyCombo(allCombo).anyComboMath(handCards.stream().map(v -> v.getVal()));
     ...
     * @date 2019/9/2
     */
    public static int calcMingTangFen(int totalTun, WcPaohuziTable table, List<Integer> mt) {
        //总囤
