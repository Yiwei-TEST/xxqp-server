...
import com.sy599.game.qipai.xxgaohuzi.command.CommandProcessor;
import com.sy599.game.qipai.xxgaohuzi.constant.EnumHelper;
 ...
 */
@Data
public class XxGaohuziPlayer extends Player {
    // 座位id
    private int seat;
    // 状态
    ...
    */
    ...
    public void addCardType(int action, List<PaohzCard> disCardList) {
        int huxi = PaohuziTool.getOutCardHuxi(action, disCardList, getPlayingTable(XxGaohuziTable.class).getGameModel());
        if (action != 0) {
...
//            winLossPoint=StringUtil.getIntValue(values,i++);
            ...
            firstDisSingleCard=StringUtil.getIntValue(values,i++)==1;
...
//            this.isSendChui = StringUtil.getIntValue(values, i++);
    ...
    public void clearTableInfo() {
        XxGaohuziTable table = getPlayingTable();
    ...
    public PaohuziHuLack checkPaoHu(PaohzCard card, boolean isSelfMo, boolean canDiHu) {
        ...
        //胡牌计算
        PaohuziHuLack lack = PaohuziTool.isHuNew(PaohuziTool.getPaohuziHandCardBean(handCardsCopy), null, isSelfMo, totalHuxi, true, true, getPlayingTable(XxGaohuziTable.class).getGameModel());
        ...
        if (lack.isHu()) {
            ...
            XxGaohuziTable table=getPlayingTable(XxGaohuziTable.class);
    ...
    public PaohuziHuLack checkHu(PaohzCard card, boolean isSelfMo) {
        ...
        PaohuziHuLack lack = PaohuziTool.isHuNew(PaohuziTool.getPaohuziHandCardBean(handCopy), card, isSelfMo, outHuXi, isNeedDui(), isPaoHu, getPlayingTable(XxGaohuziTable.class).getGameModel());
        ...
        if (lack.isHu()) {
            if (lack.getHuxi() + outHuXi + zaiHuxi < ((XxGaohuziTable) this.getPlayingTable()).getGameModel().getRoundFinishLowestHuXi()) {
                // 10胡息能胡
                lack.setHu(false);
            } else {
                setHuxi(lack.getHuxi());
    ...
    public PaohuziCheckCardBean checkCard(PaohzCard card, boolean isSelfMo, boolean isPassHu, boolean isBegin, boolean isFirstCard, boolean isPass) {
        ...
        if (card != null) {
            ...
            if (((XxGaohuziTable) getPlayingTable()).getGameModel().getSpecialPlay().isIgnite() && !currentCardIsMoFlag && !isPass) {
    ...
    /**
     * @param
     * @return check.chi true能吃
     * @description 吃检测&过张操作检测,能吃的牌一定要自己摸到的或者上家摸到的
     * @author Guang.OuYang
     * @date 2019/9/4
     */
    private void checkChiOrPassChi(PaohzCard card, boolean isSelfMo, boolean isPass, PaohuziCheckCardBean check, boolean isCanChiPeng, XxGaohuziTable table, boolean currentCardIsMoFlag, List<Integer> fixdOutVals) {
        ...
        if (isCanChiPeng && !check.isTi() && !check.isPao() && isCheckChi && !isPass) {
            //上家检测
            XxGaohuziPlayer frontPlayer = (XxGaohuziPlayer) table.getPlayerBySeat(table.calcFrontSeat(seat));
    ...
    public List<PaohzCard> getChiList(PaohzCard card, List<PaohzCard> cardList) {
        ...
        if (cardList != null) {
            ...
            List<PaohzCard> chiList = PaohuziTool.checkChi(copy, card, getPlayingTable(XxGaohuziTable.class).getGameModel());
            ...
            if (sameList == null || sameList.isEmpty()) {
                ...
                for (PaohzCard sameCard : sameList) {
                    ...
                    // 相同的牌还能不能继续吃
                    List<PaohzCard> samechiList = PaohuziTool.checkChi(copy, sameCard, getPlayingTable(XxGaohuziTable.class).getGameModel());
            ...
            cardList = PaohuziTool.checkChi(handCard.getOperateCards(), card, getPlayingTable(XxGaohuziTable.class).getGameModel());
        ...
        if (sameList == null || sameList.isEmpty()) {
            ...
            for (PaohzCard sameCard : sameList) {
                ...
                // 相同的牌还能不能继续吃
                List<PaohzCard> samechiList = PaohuziTool.checkChi(handCard.getOperateCards(), sameCard, getPlayingTable(XxGaohuziTable.class).getGameModel());
                if (samechiList == null || samechiList.isEmpty()) {
                    // 如果不能吃 则这个牌不能吃
                    return null;
                }
...
//        sb.append(winLossPoint).append(",");
    ...
    public PlayerInTableRes.Builder buildPlayInTableInfo(long lookUid, boolean isrecover) {
        ...
        if (seat == table.getShuXingSeat()) {
            XxGaohuziPlayer paohuziPlayer = (XxGaohuziPlayer) table.getSeatMap().get(table.getLastWinSeat());
...
//        res.addExt(chui);//8
    ...
    public List<PhzHuCards> buildPhzCards(XxGaohuziTable table, long lookUid) {
        List<PhzHuCards> list = new ArrayList<>();
        for (CardTypeHuxi type : cardTypes) {
            if (type.getAction() == PaohzDisAction.action_zai) {
                // 不是本人并且是栽
    ...
    public int calcHuPoint(int total, int xiToTun) {
        ...
        //结算时胡息个位数四舍五入，两两玩家对应增减。（5为10，-5为-10）
//        int calc = (Math.abs(total) + 5) / 10 * 10;
//        return (total >= 0 ? calc : -calc) / xiToTun;
