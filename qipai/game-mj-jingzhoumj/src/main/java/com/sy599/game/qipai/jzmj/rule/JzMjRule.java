...
public class JzMjRule {
	 ...
	 *
	 */
	public static void checkXiaoHu3(JzMjiangHu hu, List<JzMj> majiangIds, boolean begin, JzMjTable table) {
		int playerNum = 4;
		if (table != null) {
			playerNum = table.getMaxPlayerCount();
		}
		JzMjAction actionData = new JzMjAction();
		JzMjIndexArr card_index = new JzMjIndexArr();
		JzMjQipaiTool.getMax(card_index, majiangIds);
		if (begin) {
			boolean findjiang = false;
			List<Integer> seList = new ArrayList<>(Arrays.asList(1, 2, 3));
			Map<Integer, List<JzMj>> huaseListMap = new HashMap<>();
			for (JzMj majiang : majiangIds) {
				...
				if (huaseListMap.containsKey(huase)) {
					List<JzMj> list = huaseListMap.get(huase);
					list.add(majiang);
...
//			}
			...
			List<JzMj> jiangMjs = new ArrayList<>();
			...
			if (jiangMjs.size() == 1 && jiangMjs.get(0).getVal() % 10 == 5) {
				...
				for (List<JzMj> list : huaseListMap.values()) {
			...
			JzMjIndex index3 = card_index.getMajiangIndex(3);
			...
			if (jinTongYuNuMjVals.size() == 2) {
				...
				hu.setShowMajiangs(JzMjQipaiTool.findMajiangByValsAndCount(majiangIds, vals, 2));
			}
			...
			if (!jiejieGaoMjVals.isEmpty()) {
				...
				hu.setShowMajiangs(JzMjQipaiTool.findMajiangByVals(majiangIds, vals));
			}
	 ...
	 *
	 */
	public static void checkXiaoHu2(JzMjiangHu hu, List<JzMj> majiangIds, boolean begin, JzMjTable table) {
		int playerNum = 4;
		if (table != null) {
			playerNum = table.getMaxPlayerCount();
		}
...
//			}
			...
			if (!seList.isEmpty()) {
				...
				hu.setQueyise(true);
				hu.setShowMajiangs(majiangIds);
				hu.addXiaoHu(JzMjAction.QUEYISE, new HashMap<Integer, List<JzMj>>());
			}
			// 板板胡 起牌后玩家手上没有一张258将牌
			if (!findjiang) {
				actionData.addBanBanHu();
				hu.setBanbanhu(true);
				hu.setShowMajiangs(majiangIds);
				hu.addXiaoHu(JzMjAction.BANBANHU, new HashMap<Integer, List<JzMj>>());
			}
			// 一枝花：满足以下任意一种情况：起牌后，玩家手牌中只有一张将牌，且这张将牌数字为“5”，即可胡牌（等同小胡自摸）。起牌后，玩家手牌中某花色只有一张，且这张将牌数字为“5”，即可胡牌（等同小胡自摸）。
			if (yizhihua) {
				actionData.addYiZhiHua();
				hu.setYizhihua(true);
				hu.addXiaoHu(JzMjAction.YIZHIHUA, new HashMap<Integer, List<JzMj>>());
				hu.setShowMajiangs(majiangIds);
			}
			...
			if (index3 != null) {
				actionData.addDaSiXi();
				hu.addXiaoHu(JzMjAction.DASIXI, index3.getMajiangValMap());
				hu.setDasixi(true);
				hu.setShowMajiangs(index3.getMajiangs());
			...
			if (count3 > 1) {
				...
				if (index2 != null) {
					...
					Map<Integer, List<JzMj>> map2 = index2.getMajiangValMap();
					for (Entry<Integer, List<JzMj>> entry : map2.entrySet()) {
//						if(map2.size()<2){
						if(table.checkXiaoHuCards(JzMjAction.ZHONGTULIULIUSHUN,entry.getKey())){
							continue;
						}
...
//						}
				...
				if(index3!=null){
					Map<Integer, List<JzMj>> map3 = index3.getMajiangValMap();
					for (Entry<Integer, List<JzMj>> entry : map3.entrySet()) {
//						if(map2.size()<2){
						if(table.checkXiaoHuCards(JzMjAction.ZHONGTULIULIUSHUN,entry.getKey())){
							continue;
						}
						List<JzMj> valus = new ArrayList<>(entry.getValue());
						valus.remove(0);
...
//						}
				...
				if(map.size()>1){
					int count = 0;
					Map<Integer, List<JzMj>> map2 = new HashMap<>();
					int key = 0;
					for (Entry<Integer, List<JzMj>> entry : map.entrySet()) {
						count++;
						if (count % 2 == 1) {
							key = entry.getKey();
							if (count3 == 3 && count == 3) {
							...
							List<JzMj> list = map2.get(key);
							if (list != null) {
								list.addAll(entry.getValue());
							}
						}
					}
					hu.addXiaoHu(JzMjAction.LIULIUSHUN, map2);
				}
			...
			if (jinTongYuNuMjVals.size() == 2) {
				...
				List<JzMj> list = JzMjQipaiTool.findMajiangByValsAndCount(majiangIds, vals, 2);
				Map<Integer, List<JzMj>> map2 = new HashMap<>();
				map2.put(list.get(0).getVal(), list);
				hu.addXiaoHu(JzMjAction.JINGTONGYUNU, map2);

				// 删除掉
				//majiangIds.removeAll(list);
				card_index = new JzMjIndexArr();
				JzMjQipaiTool.getMax(card_index, majiangIds);
				// hu.setShowMajiangs();
			}
			...
			if (!jiejieGaoMjVals.isEmpty()) {
				...
				Map<Integer, List<JzMj>> map2 = new HashMap<>();
				map2.put(list.get(0).getVal(), list);
				hu.addXiaoHu(JzMjAction.JIEJIEGAO, map2);
				...
				if (!jiejieGaoMjVals.isEmpty()) {
					...
					List<JzMj> list2 = JzMjQipaiTool.findMajiangByValsAndCount(majiangIds, vals2, 2);
				...
				// hu.setShowMajiangs(JzMjQipaiTool.findMajiangByVals(majiangIds,
				// vals));
				// JzMjQipaiTool.findMajiangByValsAndCount(handPais, vals,2);
			}
			...
			if (!sanTongMjVals.isEmpty()) {
				...
				Map<Integer, List<JzMj>> map2 = new HashMap<>();
				map2.put(list.get(0).getVal(), list);
				hu.addXiaoHu(JzMjAction.SANTONG, map2);
			...
			if (index3 != null) {
				...
				hu.setZhongtusixi(true);
				hu.setShowMajiangs(index3.getMajiangs());
				hu.addXiaoHu(JzMjAction.ZHONGTUSIXI, index3.getMajiangValMap());
				//card_index = removeMj(majiangIds, index3);
			}
...
//						}
				...
				if(map.size()>1){
					...
					hu.addXiaoHu(JzMjAction.ZHONGTULIULIUSHUN, map2);
				}
	...
	private static JzMjIndexArr removeMj(List<JzMj> majiangIds, JzMjIndex index3) {
		JzMjIndexArr card_index;
		List<JzMj> list = new ArrayList<JzMj>();
		// list.addAll(index3.getMajiangValMap().values());
		for (List<JzMj> mjs : index3.getMajiangValMap().values()) {
			list.addAll(mjs);
		}
	...
	public static Set<Integer> getJinTongYuNv(JzMjIndexArr card_index) {
		Set<Integer> jinTongYuNuMjVals = new HashSet<>();
		for (int i = 1; i <= 3; i++) {
			JzMjIndex jinTongYuNuIndex = card_index.getMajiangIndex(i);// 对子数量
	...
	public static Set<Integer> getJieJieGao(JzMjIndexArr card_index) {
		Set<Integer> jiejieGaoMjVals = new HashSet<>();
		Set<Integer> rset = new HashSet<>();
		Map<Integer, Set<Integer>> huaseMap = new HashMap<>();
		for (int i = 1; i <= 3; i++) {
			JzMjIndex jiejieGaoIndex = card_index.getMajiangIndex(i);// 对子数量
	...
	public static Set<Integer> getSantong(JzMjIndexArr card_index) {
		Set<Integer> sanTongMjVals = new HashSet<>();
		Map<Integer, Set<Integer>> valMap = new HashMap<>();
		for (int i = 1; i <= 3; i++) {
			JzMjIndex sanTongIndex = card_index.getMajiangIndex(i);// 对子数量
	...
	/**
	 * 0 碰碰胡 1将将胡 2清一色 3双豪华7小对 4豪华7小对 5:7小对 6全求人 15报听
	 *
	 * @param majiangIds
	 * @param gang
	 * @param peng
	 ...
	 * @return
	 */
	public static int[] checkDahu(JzMjiangHu hu, List<JzMj> majiangIds, List<JzMj> gang, List<JzMj> peng,
								  List<JzMj> chi, List<JzMj> buzhang) {
		List<JzMj> allMajiangs = new ArrayList<>();
		...
		JzMjIndexArr all_card_index = new JzMjIndexArr();
		JzMjQipaiTool.getMax(all_card_index, allMajiangs);
		...
		int hongzhongNum = JzMjTool.dropHongzhong(copy).size();
		...
		if (card_index.getDuiziNum() == 6 && hongzhongNum > 0) {
			...
			// 是否有豪华7小对
			JzMjIndex index = card_index.getMajiangIndex(3);
			if (index != null) {
				// 有4个一样的牌
		...
		if (majiangIds.size() == 2) {
			...
			// 两张牌只要有一个红中算可以胡了 必须为将
			if (haveHz || majiangIds.get(0).getVal() == majiangIds.get(1).getVal() && majiangIds.get(0).isJiang()) {
				arr[6] = 1;
	...
	private static boolean isqingyiseHu(List<JzMj> allMajiangs, List<JzMj> majiangIds) {
		boolean qingyise = checkqingyise(allMajiangs);
		if (qingyise) {
			boolean hu = JzMjTool.isPingHu(majiangIds, false);
			return hu;
		}
	...
	public static boolean checkqingyise(List<JzMj> allMajiangs) {
		boolean qingyise = false;
		int se = 0;
		for (JzMj mjiang : allMajiangs) {
	...
	private static boolean isJiangJiangHu(List<JzMj> majiangIds) {
		boolean jiangjianghu = true;
		for (JzMj mjiang : majiangIds) {
			if (mjiang.isHongzhong()) {
				continue;
			}
			if (!mjiang.isJiang()) {
	...
	private static boolean isPengPengHu(List<JzMj> majiangIds, JzMjIndexArr card_index) {
		JzMjIndex index4 = card_index.getMajiangIndex(3);
		JzMjIndex index3 = card_index.getMajiangIndex(2);
		JzMjIndex index2 = card_index.getMajiangIndex(1);
		JzMjIndex index1 = card_index.getMajiangIndex(0);
		...
		if (sameCount == 4 && index2 != null && index2.getLength() == 1) {
			return true;
		} else if (majiangIds.contains(JzMj.getMajang(201)) && index4 != null && index4.getLength() == 2
				&& index1 != null && index1.getLength() == 1 && index2 == null) {
			return true;
		} else if (majiangIds.contains(JzMj.getMajang(201)) && sameCount >= 3 && index2 == null && index1 != null
				&& index1.getLength() == 2) {
			return true;
		} else if (majiangIds.contains(JzMj.getMajang(201)) && sameCount >= 2 && index2 != null
