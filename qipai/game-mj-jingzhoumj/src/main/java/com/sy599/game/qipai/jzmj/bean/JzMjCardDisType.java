...
//	}
	...
	public boolean isHasCard(JzMj card) {
		return cardIds != null && cardIds.contains(card.getId());
	}

	public boolean isHasCardVal(JzMj card) {
		if (cardIds != null) {
			List<JzMj> majiangs = JzMjHelper.toMajiang(cardIds);
			List<Integer> vals = JzMjHelper.toMajiangVals(majiangs);
	...
	public int removeCardVal(int val) {
		if (cardIds != null) {
			...
			while (iterator.hasNext()) {
				...
				JzMj majiang = JzMj.getMajang(id);
