...
public class JzMjiangHu {
	...
	private HashMap<Integer,Map<Integer,List<JzMj>>> xiaohuMap = new HashMap<>();
	 ...
	 *
	 */
	private List<JzMj> showMajiangs;
	/**
	 * 胡的分数
	 */
	...
	public void setShowMajiangs(List<JzMj> showMajiangs) {
		this.showMajiangs = showMajiangs;
	}

	public List<JzMj> getShowMajiangs() {
		return showMajiangs;
	}
...
//			}
	...
	public static int getDaHuPointCount(List<Integer> daHuList){
		...
		int count = 0;
		for (int dahu : daHuList) {
			if (dahu == 11) {//双豪 双龙7对 算3个大胡
				count += 3;
			} else if (dahu == 6) { //豪 龙7对 算2个大胡
				count += 2;
			} else {
				//天胡地胡不算大胡20180803
				count += 1;
	...
	public void addXiaoHu(int xiaohu,Map<Integer,List<JzMj>> map){
		xiaohuMap.put(xiaohu, map);
	}

	public HashMap<Integer, Map<Integer, List<JzMj>>> getXiaohuMap() {
		return xiaohuMap;
	}
