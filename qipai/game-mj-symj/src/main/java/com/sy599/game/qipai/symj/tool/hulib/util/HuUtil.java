package com.sy599.game.qipai.symj.tool.hulib.util;
...
public class HuUtil {
    public static final String source_dir = "symjtbl/";
    public static final String dump_file_dir = "D:\\develop\\workspace\\2d\\qipai_all_2\\game-symj\\src\\main\\resources\\symjtbl\\";
    ...
    public static boolean isCanHu(List<SyMj> cards, int gui_num) {
    ...
    public static List<SyMj> getLackPaiList(List<SyMj> cards, int gui_num) {
        int[] cardsArr = toCardArray(cards);
        boolean any = false;
        if (gui_num > 0) {
    ...
    public static int[] toCardArray(List<SyMj> cards) {
        int[] cardsArr = new int[34];
        Arrays.fill(cardsArr, 0);

        for (int i = 0; i < cards.size(); i++) {
            SyMj card = cards.get(i);
    ...
    private static SyMj index2Majiang(int index) {
        int type = index / 9 + 1;
        ...
        return SyMj.getMajiangByVal(val);
    }

    private static List<SyMj> toRawCards(List<Integer> list) {
        List<SyMj> retList = new ArrayList<>();
        for (int index : list) {
    ...
    public static int getMjIndex(SyMj card) {
