package com.sy599.game.qipai.hylhq.command;
...
import com.sy599.game.qipai.hylhq.command.com.PaohuziComCommand;
import com.sy599.game.qipai.hylhq.command.play.PlayCommand;
...
public class PaohuziCommandProcessor extends AbstractBaseCommandProcessor {
	...
	public void process(HylhqPlayer player, MessageUnit message) {
		int code = 0;
		try {
	...
	public void process(Player player, MessageUnit message) {
		...
		int code = 0;
		try {
			HylhqPlayer pdkPlayer = player.getPlayer(processor);
			BaseCommand action = ObjectUtil.newInstance(commandMap.get(message.getMsgType()));
			action.setPlayer(player);
