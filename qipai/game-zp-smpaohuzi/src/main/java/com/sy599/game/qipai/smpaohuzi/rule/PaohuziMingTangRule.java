package com.sy599.game.qipai.smpaohuzi.rule;
...
import com.sy599.game.qipai.smpaohuzi.bean.CardTypeHuxi;
...
//    public static final int LOUDI_MINGTANG_EIGHTEEN_MIN = 16; //十八小: 即胡牌时小字牌数≥18张(6番)。
    ...
    public static final int LOUDI_MINGTANG_DUIZIHU = 23;//对子胡：胡牌时全部七方门子中的每一方子都是同一个字，即七方门子或都是对子、碰牌、偎牌、跑牌或提牌，大六八番8番、小六八番6番；
    ...
    public static final int LOUDI_MINGTANG_YIDIANZHU_TUPAO = 27;//土炮点胡：胡牌时有且只有一个红字，3番；
...
//        }
     ...
     */
    public static Map<Integer,Integer> calcMingTang(SmPaohuziPlayer player) {
        ...
        SmPaohuziTable table = player.getPlayingTable(SmPaohuziTable.class);

        HuType huType = ((SmPaohuziTable) player.getPlayingTable()).calcHuType();
...
//        }
            ...
            //黑胡
            mtList.put(table.getGameModel().isAllMingTang() ? LOUDI_MINGTANG_WUHU : LOUDI_MINGTANG_WUHU_TUPAO , 0);
            LogUtil.printDebug("黑胡:即胡牌时红牌数=0张(5番)。");
        }
		else if (table.getGameModel().getSpecialPlay().isRedWu() && redCardCount >= 13) {
			//红乌
		...
		//互斥名堂
            ...
            //红胡
			mtList.put(table.getGameModel().isAllMingTang() ? LOUDI_MINGTANG_DAHONGHU : LOUDI_MINGTANG_HONGHU_TUPAO, table.getGameModel().isAllMingTang() ? redCardCount - 10 : 0);
            LogUtil.printDebug("红胡：胡牌时大于或等于十张红字，每多一个红字多一番，大六八番起始3番、小六八番起始2番；");
        }else if (table.getGameModel().getSpecialPlay().isASmallRed() && redCardCount == 1) {
            //一点红
            mtList.put(table.getGameModel().isAllMingTang() ? LOUDI_MINGTANG_YIDIANZHU : LOUDI_MINGTANG_YIDIANZHU_TUPAO, 0);
		...
		if (table.getGameModel().getSpecialPlay().isTingHu() && player.isFristDisCard()) {
...
//        }
    ...
    private static boolean isPPHu(SmPaohuziPlayer player) {
        ...
        if (ppHu && player.getHandPais().size() >= 3) {
            ...
            //除非天胡否则把最后摸出来的牌算入
            if (!player.isFirstDisSingleCard() && !handCards.contains(player.getPlayingTable(SmPaohuziTable.class).getNowDisCardIds().get(0))) {
                //胡牌时把最后张底牌算进去,这张牌的组合不确定所以把玩家所有牌都算入
                handCards.add(player.getPlayingTable(SmPaohuziTable.class).getNowDisCardIds().get(0));
            }
...
//            ppHu = !new FindAnyCombo(allCombo).anyComboMath(handCards.stream().map(v -> v.getVal()));
     ...
     * @date 2019/9/2
     */
    public static int calcMingTangFen(int totalTun, SmPaohuziTable table, Map<Integer,Integer> mt) {
        //总囤
