    ...
    public static final BigMap<Integer, String, int[]> SCORE_CALC = new BigMap<Integer, String, int[]>(30) {
        {
			...
			//全名堂,,
			//大六八,小六八
        	put(LOUDI_MINGTANG_ZIMO, "*",  new int[] { 2 });
			put(LOUDI_MINGTANG_ZIMO_ADD, "+", new int[] { 1 });        //加分直接都计入囤
			put(LOUDI_MINGTANG_TIANHU, "*", new int[] { 4, 5 });
            put(LOUDI_MINGTANG_DIHU, "*", new int[] { 3, 4 });
            put(LOUDI_MINGTANG_HAIHU, "*", new int[] { 2, 4 });
            put(LOUDI_MINGTANG_TINGHU, "*", new int[] { 6, 4 });
            put(LOUDI_MINGTANG_DAHONGHU, "*", new int[] { 2, 2 });
            put(LOUDI_MINGTANG_WUHU, "*", new int[] { 5, 6 });
            put(LOUDI_MINGTANG_YIDIANZHU, "*", new int[] { 3, 4 });
            put(LOUDI_MINGTANG_DAZIHU, "*", new int[] { 4, 5 });
            put(LOUDI_MINGTANG_XIAOZIHU, "*", new int[] { 4, 5 });
            put(LOUDI_MINGTANG_PENGPENGHU, "*", new int[] { 4, 5 });
        ...
        //    put(LOUDI_MINGTANG_HONGHD_DUIZIHU, "*", new int[] { 4 });
            ...
            put(LOUDI_MINGTANG_HONGHU_TUPAO, "*", new int[] { 4 });
            put(LOUDI_MINGTANG_YIDIANZHU_TUPAO, "*", new int[] { 6 });
            put(LOUDI_MINGTANG_HONGWU, "*", new int[] { 4 });
            put(LOUDI_MINGTANG_WUHU_TUPAO, "*", new int[] { 8 });
            ...
            put(LOUDI_MINGTANG_HHD_PENGPENGHU, "*", new int[] { 8 });
...
//        }
     ...
     */
    public static Map<Integer,Integer> calcMingTang(HGWPaohuziPlayer player) {
        ...
        return getHGWMingt(player,null);
    }

	public static Map<Integer, Integer> getHGWMingt(HGWPaohuziPlayer player,PaohzCard checkCard) {
		Map<Integer,Integer> mtList = new HashMap<>();
        List<PaohzCard> allCards = new ArrayList<>();
        
       
        HGWPaohuziTable table = player.getPlayingTable(HGWPaohuziTable.class);
        PaohzCard huCard = null;
        if(player.getHu() != null&&table!=null){
        	huCard = player.getHu().getCheckCard();
        	if(checkCard!=null){
        		huCard =checkCard;
        	}
        	if(huCard==null&&table.getNowDisCardIds()!=null&&table.getNowDisCardIds().size()>0){
        ...
        if(allCards.isEmpty()){
        	allCards.addAll(player.getHandPhzs());
        }
        
        
        if(checkCard!=null&&!allCards.contains(checkCard)){
        	allCards.add(checkCard);
        }
        

        HuType huType = ((HGWPaohuziTable) player.getPlayingTable()).calcHuType();

        //仅自摸名堂叠加
        //自摸
        if (table.getGameModel().getZimoLiangF()>0 && huType == HuType.ZI_MO) {
            mtList.put(LOUDI_MINGTANG_ZIMO,0);
            LogUtil.printDebug("自摸: 即胡牌的牌为自己亲手在墩上所摸(2番)。选项");
        }

        if (table.getGameModel().getZimoJiaTun()>0 && huType == HuType.ZI_MO) {
...
//        }
         if (table.getGameModel().getSpecialPlay().getWuHu()>0 && redCardCount == 0) {
            //黑胡
            mtList.put(table.getGameModel().getMingTangPlayT()==1 ? LOUDI_MINGTANG_WUHU : LOUDI_MINGTANG_WUHU_TUPAO , 0);
            LogUtil.printDebug("黑胡:即胡牌时红牌数=0张(5番)。");
        }

		//互斥名堂
        else if (table.getGameModel().getSpecialPlay().getRedHu()>0 && redCardCount >= 10) {
            //红胡
			mtList.put(table.getGameModel().getMingTangPlayT()==1 ? LOUDI_MINGTANG_DAHONGHU :LOUDI_MINGTANG_HONGHU_TUPAO , redCardCount - 10 );
            LogUtil.printDebug("红胡：胡牌时大于或等于十张红字，紅黑点玩法2番。其他每多一个红字多一番，");
        }else if (table.getGameModel().getSpecialPlay().getASmallRed()>0 && redCardCount == 1) {
            //一点红
            mtList.put(table.getGameModel().getMingTangPlayT()==1 ? LOUDI_MINGTANG_YIDIANZHU : LOUDI_MINGTANG_YIDIANZHU_TUPAO, 0);
        ...
        //天胡&& player.isFirstDisSingleCard()
        if (table.getGameModel().getSpecialPlay().getTianhu()>0 && table.isFirstCard() && table.getLastWinSeat() == player.getSeat()) {
                mtList.put(LOUDI_MINGTANG_TIANHU , 0);
                mtList.remove(LOUDI_MINGTANG_ZIMO);
                mtList.remove(LOUDI_MINGTANG_ZIMO_ADD);
                LogUtil.printDebug("4.2 天胡: 庄家起手发牌结束后直接胡牌;计分翻倍。");
        } else if (table.getGameModel().getSpecialPlay().getDiHu()>0 && table.getLastWinSeat() != player.getSeat() && player.isFristDisCard()) {
...
//      //听胡：没有进字就胡牌（天胡和地胡都是听胡），大六八番6番、小六八番4番；
        ...
        //碰碰胡: 胡牌时7门子全部都是跑、提、偎、坎、碰、将，玩家手上和桌子上不能有一句话，桌子上不能有吃的牌;计分翻倍。
      	if (table.getGameModel().getSpecialPlay().getPpHu()>0 && isPPHu(player,huCard)) {
        ...
        //互斥名堂, 大小字
            if (table.getGameModel().getSpecialPlay().getXiaoziHu()>0&&smallCardsSize >= 16) {
                //小字胡
                mtList.put(LOUDI_MINGTANG_XIAOZIHU , smallCardsSize - 16);
                LogUtil.printDebug("小胡：胡牌时小字大于等于十六张，每多一个多一番，大六八番10番、小六八番8番；");
            } else if (table.getGameModel().getSpecialPlay().getDaziHu()>0&&(allCards.size() - smallCardsSize >= 18)) {
                //大字胡
		...
		if(table.getGameModel().getSpecialPlay().getSeaHu()>0 && table.getLeftCards().size()==0){
...
//        }
    ...
    private static boolean isPPHu(HGWPaohuziPlayer player,PaohzCard huCard) {
    	
    	if(player.getCardTypes().isEmpty()&&player.getHu()==null){
    		return false;
    	}
...
//                	}
     ...
     * @date 2019/9/2
     */
    public static int calcMingTangFen(int totalTun, HGWPaohuziTable table, Map<Integer,Integer> mt) {
        //总囤
...
//					totalTunNumber *= (currentCalc.getV()[currentCalc.getV().length > bigSexEightIndex ? bigSexEightIndex : 0] + kv.getValue());
					...
					if(kv.getKey()==LOUDI_MINGTANG_ZIMO){
						totalFan += (currentCalc.getV()[0] + kv.getValue());
					}else{
						totalFan += (getMingtangIndex(table, kv.getKey()) + kv.getValue());
					}
					
					LogUtil.printDebug("名堂分翻倍 : {}, {} ,{}", currentCalc.getV(), currentCalc.getK(), totalFan);
				}else if (currentCalc.getE().equals("+")) {    //+代表直接增加胡息
					addHuXi += (currentCalc.getV()[0] + kv.getValue());
...
//		}
    ...
    public static int getMingtangIndex(HGWPaohuziTable table,int mingtang){
    		int totalFan = 0;
    	
    	if(mingtang==LOUDI_MINGTANG_WUHU||mingtang==LOUDI_MINGTANG_WUHU_TUPAO){
    		
    		if(table.getGameModel().getMingTangPlayT()==1){
    			totalFan+=5;
    		}else{
    			totalFan+=8;
    		}
    		
    	}else if(mingtang==LOUDI_MINGTANG_DAHONGHU||mingtang==LOUDI_MINGTANG_HONGHU_TUPAO){
    	...
    	}else if(mingtang==LOUDI_MINGTANG_YIDIANZHU||mingtang==LOUDI_MINGTANG_YIDIANZHU_TUPAO){
    		if(table.getGameModel().getMingTangPlayT()==1){
    			totalFan+=3;
    		}else{
    			totalFan+=6;
    	 ...
    	 if(mingtang==LOUDI_MINGTANG_TIANHU){
    		 if(table.getGameModel().getMingTangPlayT()==1){
    			 if(table.getGameModel().getSpecialPlay().getTianhu()==1){
    	    			totalFan+=4;
    	    		}else{
    	    			 totalFan+=5;
    	    		}
    	...
    	}else if(mingtang==LOUDI_MINGTANG_DIHU){
    		 if(table.getGameModel().getMingTangPlayT()==1){
    			 if(table.getGameModel().getSpecialPlay().getDiHu()==1){
    	    			totalFan+=3;
    	    		}else{
    	    			 totalFan+=4;
    	    		}
    	 ...
    	 if(mingtang==LOUDI_MINGTANG_PENGPENGHU||mingtang==LOUDI_MINGTANG_HHD_PENGPENGHU){
    		 if(table.getGameModel().getMingTangPlayT()==1){
    			 if(table.getGameModel().getSpecialPlay().getDiHu()==1){
    	    			totalFan+=4;
    	    		}else{
    	 ...
    	 if(mingtang==LOUDI_MINGTANG_XIAOZIHU||mingtang==LOUDI_MINGTANG_DAZIHU){
    		 
    		 if(table.getGameModel().getMingTangPlayT()==1){
    			 if(table.getGameModel().getSpecialPlay().getXiaoziHu()==1||table.getGameModel().getSpecialPlay().getDaziHu()==1){
    	    			totalFan+=4;
    	    		}else{
    	    			 totalFan+=5;
    	    		}
    	 ...
    	 if(mingtang==LOUDI_MINGTANG_HAIHU){
    		 if(table.getGameModel().getMingTangPlayT()==1){
     			totalFan+=2;
     		}else{
     			totalFan+=6;
