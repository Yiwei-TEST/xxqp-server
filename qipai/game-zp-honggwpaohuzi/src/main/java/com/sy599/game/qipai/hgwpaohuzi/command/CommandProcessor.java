package com.sy599.game.qipai.hgwpaohuzi.command;
...
import com.sy599.game.qipai.hgwpaohuzi.command.com.ComCommand;
import com.sy599.game.qipai.hgwpaohuzi.command.play.PlayCommand;
...
public class CommandProcessor extends AbstractBaseCommandProcessor {
    ...
    public void process(HGWPaohuziPlayer player, MessageUnit message) {
        int code = 0;
        try {
    ...
    public void process(Player player, MessageUnit message) {
        ...
        int code = 0;
        try {
            HGWPaohuziPlayer pdkPlayer = player.getPlayer(processor);
            BaseCommand action = ObjectUtil.newInstance(commandMap.get(message.getMsgType()));
            action.setPlayer(player);
