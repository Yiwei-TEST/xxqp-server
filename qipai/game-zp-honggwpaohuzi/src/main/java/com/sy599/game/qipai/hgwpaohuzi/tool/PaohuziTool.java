...
import com.sy599.game.qipai.hgwpaohuzi.bean.PaohuziHandCard;
import com.sy599.game.qipai.hgwpaohuzi.bean.PaohuziHuBean;
...
public static boolean  getTingZps(HGWPaohuziPlayer player) {
	...
	for(Integer id: copy){
		...
		if(lack.isHu()){
			...
			PaohuziHuLack paoHu = player.checkPaoHu(tingC, true,false);
			if(paoHu.isHu()){
				return true;
			}
		}
...
//    }
    ...
    public static PaohuziHuLack isHuNew(PaohuziHandCard handCardBean, PaohzCard disCard, boolean isSelfMo, int outCardHuxi, boolean needDui, boolean isPaoHu, GameModel gameModel) {
        ...
        // 手上有3个的
        PaohuziIndex index2 = arr.getPaohzCardIndex(2);
        PaohzCard chaiCard = disCard;
        if (index2 != null) {
            List<Integer> list = index2.getValList();
            for (int val : list) {
                if (disCard != null && val == disCard.getVal()&&!isSelfMo) {
                    // 抓到手上的牌可以不用强制组出3个
                    handCardBean.getOperateCards().addAll(index2.getPaohzValMap().get(val));
                    continue;
                }
        ...
        if (handPais.size() == 0) {
            ...
            sortMin(handPais);
            chaiPaiNew(huList, lack, handPais, chaiCard, gameModel);
            if (huList.size() > 0) {
	...
	public static List<PaohzCard>  getTingZps(List<PaohzCard> handPais,HGWPaohuziPlayer player) {
		...
		for(Integer id : copy) {
			...
			if(!checkHuCard(huCards, card)||handPais.contains(card)) {
			 ...
			 // 检查胡
            PaohuziHuLack lack = player.checkHuAddCards(card, true, handPais,true);
            handPais.remove(card);
            if(!lack.isHu()){
            	lack = player.checkHuAddCards(card, false, handPais,true);
            	 handPais.remove(card);
            	 if(!lack.isHu()){
            		 lack = player.getPaoHuInfo(card, false, true,false,handPais);
            		 handPais.remove(card);
            	 }
            }
