...
public class HuTool {
     ...
     * @return
     */
    public static boolean isHu(List<Integer> ids,List<Integer> chunWang,int zhengWang,List<MjCardDisType> disTypes,boolean baoTing,boolean selfMo){
        if(isXiaoHu(ids,chunWang))
            return true;
        if(isDaHu(ids,chunWang,zhengWang,disTypes,baoTing,selfMo))
            return true;
        return false;
    }
    ...
    /**
     * 只计算是否能胡，胡哪些大胡另算
     * 部分大胡需要258做将，已在{@link HuTool#isXiaoHu(List, List)}该方法中计算。
     * @param ids
     * @param chunWang
     * @param zhengWang
     * @return
     */
    public static boolean isDaHu(List<Integer> ids,List<Integer> chunWang,int zhengWang,List<MjCardDisType> disTypes,boolean baoTing,boolean selfMo){
        //不需要成牌型的大胡
    ...
    public static boolean check7dui(List<Integer> ids,List<Integer> bossVals,List<Integer> huType){
        ...
        if(bossNum>=0){
            if(longQ)
                huType.add(KwMingTang.MINGTANG_DH_LQXD);
            else
                huType.add(KwMingTang.MINGTANG_DH_QXD);
            return true;
        }
        return false;
    }
    ...
    public static List<Integer> isQiShouHu(List<KwMj> mjs,KwMjTable table) {
        ...
        if(isYZH(mjs,zhengWang,table.getChunWang()))
     ...
     * @return
     */
    private static boolean isYZH(List<KwMj> mjs, int kingVal, List<Integer> chunWang){
        int count=0;
        for (KwMj mj:mjs){
            if(mj.getVal()==kingVal||chunWang.contains(mj.getVal())){
                count++;
                continue;
            }
    ...
    public static List<Integer> getHuType(List<Integer> ids, List<Integer> chunWang, KwMjPlayer player, KwMjTable table,boolean selfMo){
        //先判断起手胡
        List<Integer> qsType=new ArrayList<>();
        if(table.getDisNum()==0&&table.getBuIds().size()==0)
        ...
        if(huType.size()>qsType.size()&&table.getDisNum()==0){
...
//    }
    ...
    private static void checkBoss1(List<Integer> huType,List<KwMj> mjs,int zhengWang,List<Integer> chunWang,boolean selfMo,boolean isHu){
        ...
        if(zhengNum>=3&&!huType.contains(KwMingTang.MINGTANG_HTH)&&selfMo)
        ...
        else if(isHu&&chunNum1+chunNum2==0)
            huType.add(KwMingTang.MINGTANG_0W);
    }

    private static void checkBoss2(List<Integer> huType,List<KwMj> mjs,List<Integer> chunWang){
        int chunNum1=getMjNum(mjs,chunWang.get(0));
        int chunNum2=0;
        if(chunWang.size()==2){
            chunNum2=getMjNum(mjs,chunWang.get(1));
        }
        if(huType.size()>0&&chunNum1+chunNum2==0&&!huType.contains(KwMingTang.MINGTANG_0W))
            huType.add(KwMingTang.MINGTANG_0W);
    }
...
//    }
     ...
     * @return
     */
    public static List<Integer> getDaHuNew(List<Integer> ids, List<Integer> chunWang, KwMjPlayer player, KwMjTable table ,boolean selfMo){
        List<Integer> huType=new ArrayList<>();
        if(isJJHNoZu(player.getCardTypes(), player.getHandMajiang(), chunWang))
            huType.add(KwMingTang.MINGTANG_DH_JJH);
        boolean hu7dui = check7dui(ids, chunWang, huType);
        boolean pingHu = isHu(ids, chunWang, false);
        if(pingHu){
            if(isPPH(ids,player.getCardTypes(),chunWang))
                huType.add(KwMingTang.MINGTANG_DH_PPH);
            if(isQingYiSe(player.getCardTypes(), ids, chunWang))
                huType.add(KwMingTang.MINGTANG_DH_QYS);
        }
        //王相关胡
        checkBoss1(huType,MjHelper.toMajiang(ids),table.getZhengWang(),chunWang,selfMo,pingHu||hu7dui);
        return huType;
    }
