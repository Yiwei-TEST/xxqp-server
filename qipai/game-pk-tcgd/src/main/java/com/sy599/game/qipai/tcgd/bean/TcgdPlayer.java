...
import com.sy599.game.qipai.tcgd.command.TcgdCommandProcessor;
...
public class TcgdPlayer extends Player {
	...
	private int paitype_=0;
	private int istuoguanJieSan =0;
	private int istuoguanTimeJianBan=0;//托管减半标志
	private long myTuoGuanTime =0l;
	private long tuoguanJianBanCheckTime;
	public int getIsOutWhenAuto() {
		return isOutWhenAuto;
	}
	public void setIsOutWhenAuto(int isOutWhenAuto) {
		this.isOutWhenAuto = isOutWhenAuto;
	}

	/**
	 *  在新增5S自动过时间 出过牌=1
	 */
	private volatile int isOutWhenAuto=0;
...
//	全局总拿分
	...
	protected void loadFromDB0(RegInfo info) {
		if (UserDatasDao.getInstance().exists()){
			String val = UserDatasDao.getInstance().selectUserDataValue(String.valueOf(userId), TcgdTable.GAME_CODE,"all","currentLs");
			if (CommonUtil.isPureNumber(val)){
				currentLs = Integer.parseInt(val);
			}else{
				UserDatasDao.getInstance().saveUserDatas(String.valueOf(userId), TcgdTable.GAME_CODE,"all","currentLs","0");
			}

			val = UserDatasDao.getInstance().selectUserDataValue(String.valueOf(userId), TcgdTable.GAME_CODE,"all","maxLs");
			if (CommonUtil.isPureNumber(val)){
				maxLs = Integer.parseInt(val);
			}else{
				UserDatasDao.getInstance().saveUserDatas(String.valueOf(userId), TcgdTable.GAME_CODE,"all","maxLs","0");
			}
		}
	}
	...
	public void setAutoPlay(boolean autoPlay,BaseTable table) {
		if (this.autoPlay != autoPlay && !isRobot()){
			...
			TcgdTable table2 = getPlayingTable(TcgdTable.class);
        	...
        	table2.addPlayLog(table2.addSandhPlayLog(getSeat(), TcgdConstants.action_tuoguan, val, false, 0, null, 0));
		}
		...
		if(addLog) {
			StringBuilder sb = new StringBuilder("Tcgd");
			if (table != null) {
	...
	public void dealHandPais(List<Integer> pais, TcgdTable table) {
		this.table = table;
		this.handPais = pais;
		...
		Collections.sort(pais, new Comparator<Integer>() {
			...
			public int compare(Integer o1, Integer o2) {
				return TcgdUtil.loadCardValue(o2) - TcgdUtil.loadCardValue(o1);
			}
		});
	...
	public void addOutPais(List<Integer> cards , TcgdTable table) {
		this.table = table;
	...
	public String toInfoStr() {
		...
		sb.append(paitype_).append(",");
		sb.append(istuoguanJieSan).append(",");
		sb.append(myTuoGuanTime).append(",");
		return sb.toString();
	}
	...
	public void initPlayInfo(String data) {
		if (!StringUtils.isBlank(data)) {
			...
			paitype_=StringUtil.getIntValue(values, i++);
			istuoguanJieSan=StringUtil.getIntValue(values, i++);
			myTuoGuanTime=StringUtil.getIntValue(values, i++);
		}
	}
	...
	public PlayerInTableRes.Builder buildPlayInTableInfo(long recUserId, boolean isrecover) {
		...
		TcgdTable table = getPlayingTable(TcgdTable.class);
		if (table == null) {
		...
		int totalScore = TcgdUtil.getCpfen(TcgdSfNew.intCardToStringCard(chiFenCards));
		res.addAllScoreCard(chiFenCards);
		extList.add(getPaitype_());//1 上手牌型
...
//		}
	...
	public static final List<Integer> wanfaList = Arrays.asList(GameUtil.play_type_pk_tcgd);
	...
	public static void loadWanfaPlayers(Class<? extends Player> cls){
		for (Integer integer:wanfaList){
			PlayerManager.wanfaPlayerTypesPut(integer,cls, TcgdCommandProcessor.getInstance());
		}
	}
	...
	public int getPaitype_() {
		return paitype_;
	}

	public void setPaitype_(int paitype_) {
		this.paitype_ = paitype_;
	}
	...
	public int getIstuoguanJieSan() {
		return istuoguanJieSan;
	}

	public void setIstuoguanJieSan(int istuoguanJieSan) {
		this.istuoguanJieSan = istuoguanJieSan;
	}

	public int getIstuoguanTimeJianBan() {
		return istuoguanTimeJianBan;
	}

	public void setIstuoguanTimeJianBan(int istuoguanTimeJianBan) {
		this.istuoguanTimeJianBan = istuoguanTimeJianBan;
	}

	public long getMyTuoGuanTime() {
		return myTuoGuanTime;
	}

	public void setMyTuoGuanTime(long myTuoGuanTime) {
		this.myTuoGuanTime = myTuoGuanTime;
	}
