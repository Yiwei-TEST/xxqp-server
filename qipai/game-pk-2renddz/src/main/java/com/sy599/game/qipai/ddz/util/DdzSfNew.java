...
public class DdzSfNew {
	...
	public static boolean isSiDaiEr(List<String> paistr) {
		...
		pai = sortAry(pai, "asc");
		boolean bool = false;
		if(pai.length != 6) {//必须出4带2，不能4带3，4带1
			bool = false;
			return bool;
		}
...
//			 }
	 ...
	 * @return
	 */
	public static String getCpType2(List<String> pai,int is3dai2,int is4dai2,int is3zhang) {
		String type = "";
		if(isDanzhang(pai)) {
			type = "danz";
		} else if(isDuizi(pai)) {
			...
			type = "ld";
		} else if(is3zhang(pai) && is3zhang==1) {
			type = "3z";
		} else if(is3dai1(pai)) {
			type = "3d1";
		} else if(is3dai1dui(pai) && is3dai2==1) {
			type = "3d2";
		} else if(isSiDaiEr(pai) && is4dai2==1) {
			type = "4d2";
		}else if(is4zhang(pai)) {
			type = "boom";
		} else if(is1dw1xw(pai)) {
		...
		if(!shunzitype.equals("false")){
		 ...
		 String fjtype = isFeiJi2(pai);
		 if("3n".equals(fjtype) && is3zhang==1){
			 	type = "fjd0";//飞机不带 连三张
		 }else if("3n2n".equals(fjtype) && is3dai2==1 ){
			 type = "fjddui";//飞机带对
		 }else if("3n1".equals(fjtype) && !isBoom(p2) ){
			 type = "fjddan";//飞机带单
		 }
		return type;
	}
...
//				 System.out.println("3n");
	...
	public static String isFeiJi2(List<String> pai ) {
		...
		for (String p:pai ) {
			...
			for (String p2:pai ) {
				if(getNumbers(p2)==num && num<15){
					temp.add(p2);
				}
			}
			if(temp.size()==3 && !planeList.contains(temp)){
				planeList.add(temp);
				continue;
			}
		}
