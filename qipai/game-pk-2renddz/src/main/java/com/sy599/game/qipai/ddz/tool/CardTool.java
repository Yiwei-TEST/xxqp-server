package com.sy599.game.qipai.ddz.tool;

import com.sy599.game.GameServerConfig;
import com.sy599.game.qipai.ddz.constant.DdzConstants;
import com.sy599.game.qipai.ddz.util.CardType;
...
public final class CardTool {
	...
	public static List<List<Integer>> fapai(int playerCount, List<List<Integer>> zps) {
		...
		List<Integer> copy = new ArrayList<>(DdzConstants.cardList);
		...
		if (GameServerConfig.isDebug()) {
			if (zps != null && !zps.isEmpty()) {
				...
				//	list.add(findCardIIds(copy, zp, 0));
				 ...
				 copy = new ArrayList<>(DdzConstants.cardList);
				for(Integer id: pai2){
					copy.remove(id);
				}
				//copy.removeAll(pai2);
