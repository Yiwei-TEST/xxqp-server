...
import com.sy599.game.db.dao.SysPartitionDao;
...
public class SysPartitionUtil {
    ...
    public static void checkGroupPartition(int seq) throws Exception {
        if (!TableCheckDao.getInstance().checkTableExists(DbEnum.LOGIN, "t_table_user_" + seq)) {
            throw new RuntimeException("initGroupPartition|error|t_table_user_" + seq + "|notExist");
        }
        if (!TableCheckDao.getInstance().checkTableExists(DbEnum.LOGIN, "t_group_credit_log_" + seq)) {
