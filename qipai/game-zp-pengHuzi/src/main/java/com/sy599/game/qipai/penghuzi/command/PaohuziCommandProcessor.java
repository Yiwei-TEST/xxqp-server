package com.sy599.game.qipai.penghuzi.command;
...
import com.sy599.game.qipai.penghuzi.command.com.PenghuziComCommand;
import com.sy599.game.qipai.penghuzi.command.play.PlayCommand;
...
public class PaohuziCommandProcessor extends AbstractBaseCommandProcessor {
	...
	static {
		commandMap.put(WebSocketMsgType.cs_com, PenghuziComCommand.class);
	...
	public void process(PenghuziPlayer player, MessageUnit message) {
		int code = 0;
		try {
	...
	public void process(Player player, MessageUnit message) {
		...
		int code = 0;
		try {
			PenghuziPlayer pdkPlayer = player.getPlayer(processor);
			BaseCommand action = ObjectUtil.newInstance(commandMap.get(message.getMsgType()));
			action.setPlayer(player);
