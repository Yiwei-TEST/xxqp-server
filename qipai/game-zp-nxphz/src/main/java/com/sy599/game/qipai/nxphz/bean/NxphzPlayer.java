...
import com.sy599.game.qipai.nxphz.command.CommandProcessor;
import com.sy599.game.qipai.nxphz.constant.EnumHelper;
 ...
 */
@Data
public class NxphzPlayer extends Player {
    ...
    void disCard(int action, List<PaohzCard> disCardList) {
        ...
        if (disCardList.size() > 3 && disCardList.size() % 3 == 0) {
            for (int i = 0; i < disCardList.size() / 3; i++) {
                ...
                addCardType(action, list, getPlayingTable(NxphzTable.class).getGameModel());
            }

        } else {
            addCardType(action, disCardList, getPlayingTable(NxphzTable.class).getGameModel());
        }
        changeSeat(seat);
        changeTableInfo();
    }
...
//            winLossPoint=StringUtil.getIntValue(values,i++);
            ...
            sishou = StringUtil.getIntValue(values, i++);
...
//            this.isSendChui = StringUtil.getIntValue(values, i++);
    ...
    public void clearTableInfo() {
        NxphzTable table = getPlayingTable();
    ...
    public PaohuziHuLack checkPaoHu(PaohzCard card, boolean isSelfMo, boolean canDiHu) {
        ...
        //胡牌计算
        PaohuziHuLack lack = PaohuziTool.isHuNew(PaohuziTool.getPaohuziHandCardBean(handCardsCopy), null, isSelfMo, totalHuxi, true, true, getPlayingTable(NxphzTable.class).getGameModel());
        ...
        if (lack.isHu()) {
            ...
            if (lack.getHuxi() + totalHuxi < ((NxphzTable) this.getPlayingTable()).getGameModel().getRoundFinishLowestHuXi()) {
                // 10胡息能胡
                lack.setHu(false);
    ...
    private List<PaohzCard> checkTiOrPaoAndAddHuXi(PaohzCard card, boolean isSelfMo, boolean canDiHu, AtomicInteger addHuXi, AtomicInteger action) {
        ...
        if (card != null) {
            NxphzTable table = getPlayingTable(NxphzTable.class);
    ...
    public PaohuziHuLack checkHu(PaohzCard card, boolean isSelfMo) {
        ...
        PaohuziHuLack lack = PaohuziTool.isHuNew(PaohuziTool.getPaohuziHandCardBean(handCopy), card, isSelfMo, outHuXi, isNeedDui(), isPaoHu, getPlayingTable(NxphzTable.class).getGameModel() );
        ...
        if (lack.isHu()) {
            if (lack.getHuxi() + outHuXi + zaiHuxi < ((NxphzTable) this.getPlayingTable()).getGameModel().getRoundFinishLowestHuXi()) {
                // 10胡息能胡
                lack.setHu(false);
            } else {
                setHuxi(lack.getHuxi());
...
//        if (lack.isHu()) {
        ...
        if(isSishou()){
        	NxphzTable table = getPlayingTable(NxphzTable.class);
        	long tableId = table!=null?table.getId():0l;
        	LogUtil.msgLog.info("----tableId:" +tableId  +"---userId:"+getUserId()+ "---userName:" +getName()
        	+"---死守状态不能胡牌----剩余手牌：" + getHandPais());
        }
    ...
    public PaohuziCheckCardBean checkCard(PaohzCard card, boolean isSelfMo, boolean isPassHu, boolean isBegin, boolean isFirstCard, boolean isPass) {
        ...
        if (card != null) {
            ...
            //牌是摸出来的,检测平胡
            if (!isSishou() && currentCardIsMoFlag && !isPass && !check.isChouZai() && !check.isZai() && !check.isTi()) {
                // 检查胡
            ...
            if (((NxphzTable) getPlayingTable()).getGameModel().getSpecialPlay().isIgnite() && !currentCardIsMoFlag && !isPass && !isSishou()) {
            ...
            if(isSishou()){
            	LogUtil.msgLog.info("----tableId:" + table.getId() +"---userId:"+getUserId()+ "---userName:" +getName()
            	+"---死守状态不能胡牌----剩余手牌：" + getHandPais()+"--胡息："+(getHuxi() + outHuXi + zaiHuxi));
            }
...
//        }
     ...
     * @date 2019/9/4
     */
    private void checkChiOrPassChi(PaohzCard card, boolean isSelfMo, boolean isPass, PaohuziCheckCardBean check, boolean isCanChiPeng, NxphzTable table, boolean currentCardIsMoFlag, List<Integer> fixdOutVals) {
        ...
        if (isCanChiPeng && !check.isTi() && !check.isPao() && isCheckChi && !isPass) {
            //上家检测
            NxphzPlayer frontPlayer = (NxphzPlayer) table.getPlayerBySeat(table.calcFrontSeat(seat));
    ...
    public List<PaohzCard> getChiList(PaohzCard card, List<PaohzCard> cardList) {
        ...
        if (cardList != null) {
            ...
            List<PaohzCard> chiList = PaohuziTool.checkChi(copy, card, getPlayingTable(NxphzTable.class).getGameModel());
            ...
            if (sameList == null || sameList.isEmpty()) {
                ...
                for (PaohzCard sameCard : sameList) {
                    ...
                    // 相同的牌还能不能继续吃
                    List<PaohzCard> samechiList = PaohuziTool.checkChi(copy, sameCard, getPlayingTable(NxphzTable.class).getGameModel());
            ...
            cardList = PaohuziTool.checkChi(handCard.getOperateCards(), card, getPlayingTable(NxphzTable.class).getGameModel());
        ...
        if (sameList == null || sameList.isEmpty()) {
            ...
            for (PaohzCard sameCard : sameList) {
                ...
                // 相同的牌还能不能继续吃
                List<PaohzCard> samechiList = PaohuziTool.checkChi(handCard.getOperateCards(), sameCard, getPlayingTable(NxphzTable.class).getGameModel());
                if (samechiList == null || samechiList.isEmpty()) {
                    // 如果不能吃 则这个牌不能吃
                    return null;
                }

//                if (!CollectionUtils.isEmpty(samechiList)) {
//                    // 添加相同的牌 的吃
...
//        sb.append(winLossPoint).append(",");
    ...
    public PlayerInTableRes.Builder buildPlayInTableInfo(long lookUid, boolean isrecover) {
        ...
        if (seat == table.getShuXingSeat()) {
            NxphzPlayer paohuziPlayer = (NxphzPlayer) table.getSeatMap().get(table.getLastWinSeat());
...
//        res.addExt(chui);//8
    ...
    public List<CardTypeHuxi> getHuCards(){
    	List<CardTypeHuxi> list = new ArrayList<>();
        for (CardTypeHuxi type : cardTypes) {
            list.add(type);
        }
        if (hu != null && hu.getPhzHuCards() != null) {
    ...
    public List<PhzHuCards> buildPhzCards(NxphzTable table, long lookUid) {
        List<PhzHuCards> list = new ArrayList<>();
        for (CardTypeHuxi type : cardTypes) {
            if (type.getAction() == PaohzDisAction.action_zai) {
                // 不是本人并且是栽
