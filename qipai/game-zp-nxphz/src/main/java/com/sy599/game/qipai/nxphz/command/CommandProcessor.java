package com.sy599.game.qipai.nxphz.command;
...
import com.sy599.game.qipai.nxphz.command.com.ComCommand;
import com.sy599.game.qipai.nxphz.command.play.PlayCommand;
...
public class CommandProcessor extends AbstractBaseCommandProcessor {
    ...
    public void process(NxphzPlayer player, MessageUnit message) {
        int code = 0;
        try {
    ...
    public void process(Player player, MessageUnit message) {
        ...
        int code = 0;
        try {
            NxphzPlayer pdkPlayer = player.getPlayer(processor);
            BaseCommand action = ObjectUtil.newInstance(commandMap.get(message.getMsgType()));
            action.setPlayer(player);
