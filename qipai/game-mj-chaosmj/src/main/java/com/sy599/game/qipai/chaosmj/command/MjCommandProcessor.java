package com.sy599.game.qipai.chaosmj.command;
...
import com.sy599.game.qipai.chaosmj.command.com.MjComCommand;
import com.sy599.game.qipai.chaosmj.command.play.MjPlayCommand;
...
public class MjCommandProcessor extends AbstractBaseCommandProcessor {
    ...
    public void process(Player player, MessageUnit message) {
        int code = 0;
        try {
            if (player == null || !(player instanceof ChaosMjPlayer)) {
                return;
            }
            ChaosMjPlayer csMajiangPlayer = player.getPlayer(processor);
