...
import com.sy599.game.qipai.chaosmj.command.MjCommandProcessor;
...
import com.sy599.game.qipai.chaosmj.tool.MjEnumHelper;
...
@Data
public class ChaosMjPlayer extends Player {
...
//    //麻将id->明杠>1000|暗杠>1000|seat
    ...
    public List<Mj> showXiaoHuMajiangs(int xiaohu, boolean connect) {
        ChaosMjTable table = getPlayingTable(ChaosMjTable.class);
    ...
    private void addCloseingExt(ClosingMjPlayerInfoRes.Builder res) {
        ...
        res.addExt(getPlayingTable(ChaosMjTable.class).getReappointment()); //连庄 0~9 - 1~10
...
//		}
    ...
    public List<Mj> getCanChiMajiangs(Mj disMajiang) {
        return MjTool.checkChi(handPais, disMajiang, getPlayingTable(ChaosMjTable.class));
    }
    ...
    public boolean isTingPai(int val, boolean onlySevenDuiNoHu, boolean needRemoveThisVal) {
        ...
        if (copy.size() % 3 != 2 ) {
            if (getPlayingTable(ChaosMjTable.class).getKingCard() != null) {
                copy.add(Mj.getMajang(getPlayingTable(ChaosMjTable.class).getKingCard().getId()));
            }else {
    ...
    public List<Integer> checkDisCard(Mj majiang) {
        return checkDisCard(majiang, true, false, getPlayingTable(ChaosMjTable.class).getKingCard() != null && majiang != null && getPlayingTable(ChaosMjTable.class).getKingCard().getVal() == majiang.getVal());
    }
            ...
            if (gangList.isEmpty() || !table.getGameModel().getSpecialPlay().isGangMoNum()) {
                ...
                if (table.calcNextSeat(table.getDisCardSeat()) == seat) {
                    if (!(/*table.getMaxPlayerCount() == 2 &&*/ table.getGameModel().getSpecialPlay().isBuChi())) {
                        List<Mj> chi = MjTool.checkChi(handPais, majiang, getPlayingTable(ChaosMjTable.class));
                        if (!chi.isEmpty()) {
                            actData.addChi();
                        }
                    }
...
//            }
    ...
    public int getDahuScore() {
        return MjiangHu.calcMenZiScore(getPlayingTable(ChaosMjTable.class), getDahu());
    }
...
//            canGang = hu.isHu() && !((!table.getGameModel().getSpecialPlay().isXiaoDuiGang() && (hu.isXiaodui() || hu.isHao7xiaodui() || hu.isShuang7xiaodui())) && hu.getDahuCount() == 1);
            ...
            //自摸控制
            if (hu.isHu() && getPlayingTable(ChaosMjTable.class).getGameModel().getSpecialPlay().isSelfMoHu()) {
                //二人麻将
                if (table.getMaxPlayerCount() == 2) {
                    if (table.getGameModel().getSpecialPlay().isOnlyDaHu()) {
                        if (table.getGameModel().getSpecialPlay().isMenqing() && !isChiPengGang()) {
    ...
    private void checkGang(MjAction actData, ChaosMjTable table, boolean canXiaohu, boolean canGang) {
