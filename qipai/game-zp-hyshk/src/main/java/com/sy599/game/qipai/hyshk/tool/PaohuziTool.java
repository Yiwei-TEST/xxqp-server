package com.sy599.game.qipai.hyshk.tool;

import com.sy599.game.GameServerConfig;

import com.sy599.game.qipai.hyshk.been.*;
...
//            list.add(l);
    ...
    public static synchronized List<List<PaohzCard>> fapai(List<Integer> copy, List<List<Integer>> t,int cardNum,int playerCount,boolean have1510) {
        List<List<PaohzCard>> list = new ArrayList<>();
        ...
        if (GameServerConfig.isDebug() && t != null && !t.isEmpty()) {
            list = zuopai(new ArrayList<>(copy), t, playerCount);
        }else {
            int handCardNum=20;
            if(cardNum!=0){
                handCardNum=cardNum;
        ...
        if(allReFaPai(list,playerCount,have1510)){
            return list;
        }else {
            return fapai(copy,null,cardNum,playerCount,have1510);
        }
    }

    public static boolean allReFaPai(List<List<PaohzCard>> list,int playerCount,boolean have1510){
        if(!checkShuangLong(list,playerCount))
            return false;
        if(!checkWuKan(list,playerCount))
            return false;
        if(!checkTianHu(list,have1510))
            return false;
        return true;
    }
    ...
    public static synchronized boolean checkTianHu(List<List<PaohzCard>> list,boolean have1510){
        List<PaohzCard> banker = list.get(0);
        List<PaohuziHuLack> hulist = isHuNew1(PaohuziTool.getPaohuziHandCardBean(banker), null, false, false, false,have1510);
    ...
    public static synchronized List<List<PaohzCard>> fapaiControl(List<Integer> copy, List<List<Integer>> t,int fifteenCards,int playerCount,int fapaiNum,boolean have1510) {
        List<Integer> copy1=new ArrayList<>(copy);
        List<List<PaohzCard>> list = fapai(copy1, t, fifteenCards, playerCount,have1510);
        fapaiNum++;
        if(fapaiNum>=10)
            return list;
        for (int j = 0; j < playerCount; j++) {
            ...
            if(num>=8||num<=4){
                ...
                if(rand.nextInt(10)<5)
                    return fapaiControl(copy1,t,fifteenCards,playerCount,fifteenCards,have1510);
            }
        }
        return list;
    }
...
//            lack.setNeedDui(true);
        ...
        if (handPais.size() == 0) {
            ...
            sortMin(handPais);
            chaiPaiNew(huList, lack, handPais, disCard,have1510);
            if (huList.size() > 0) {
                return huList;
            }
        }
    ...
    public static void chaiPaiNew(List<PaohuziHuLack> huList, PaohuziHuLack lack, List<PaohzCard> handPais, PaohzCard disCard,boolean have1510) {
        if (lack.isNeedDui()) {
            //拆对
            ...
            if (duiZi != null && duiZi.getPaohzValMap().size() > 0) {
                for (Integer val : duiZi.getPaohzValMap().keySet()) {
                    ...
                    if (handPaisCopy.size() == 0) {
                        ...
                        chaiPaiNew(huList, newLack, handPaisCopy, disCard,have1510);
                    }
                }
            }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 100000; i++) {
            List<List<PaohzCard>> list = fapai(copy, null, 0,2,false);
            for (int j = 0; j < 2; j++) {
                List<PaohzCard> cards = list.get(j);
                int count1=0;
                int nums[]=new int[20];
