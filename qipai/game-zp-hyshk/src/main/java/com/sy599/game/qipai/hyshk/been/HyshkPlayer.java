...
import com.sy599.game.qipai.hyshk.command.PaohuziCommandProcessor;
import com.sy599.game.qipai.hyshk.constant.EnumHelper;
...
public class HyshkPlayer extends Player {
    // 座位id
    private int seat;
    // 状态
...
//        if(!isNeedDui)
//            return null;
        HyshkTable table = getPlayingTable(HyshkTable.class);
        //这个起手提需要关注一下，改错了
    ...
    public List<PaohuziHuLack> checkPaoHuShiZhong(PaohzCard card, boolean isSelfMo) {
        ...
        List<PaohuziHuLack> list = PaohuziTool.isHuNew1(PaohuziTool.getPaohuziHandCardBean(handCopy), card, isSelfMo,  isNeedDui, true,table.getVal1_5_10()==1);
        Iterator<PaohuziHuLack> it = list.iterator();
        while(it.hasNext()){
            PaohuziHuLack lack = it.next();
            if (action != 0) {
    ...
    public PaohuziHuLack checkHuNew(PaohzCard card, boolean isSelfMo) {
        ...
        if (list!=null&&!list.isEmpty()){
            for(PaohuziHuLack hu:list){
                ...
                if (allHuxi >= table.getFloorValue()||(table.getPiaoHu()==1&&allHuxi==0)){
    ...
    public PaohuziCheckCardBean  checkCard(PaohzCard card, boolean isSelfMo, boolean isPassHu, boolean isBegin, boolean isFirstCard, boolean isPass) {
        ...
        if (card != null) {
            ...
            if (isCanChiPeng && !check.isTi() && !check.isPao() && isCheckChi && !isPass) {
                ...
                HyshkPlayer frontPlayer = (HyshkPlayer) table.getPlayerBySeat(table.calcFrontSeat(seat));
    ...
    public PaohuziCheckCardBean  checkShiZhongCard(PaohzCard card) {
        ...
        if(check.isTi()&&seat==getPlayingTable(HyshkTable.class).getLastWinSeat()){
...
//        res.setFinalPoint(getFinalPoint());
    ...
    public List<PhzHuCards> buildPhzCards(HyshkTable table,long someOne) {
        List<PhzHuCards> list = new ArrayList<>();
        for (CardTypeHuxi type : cardTypes) {
