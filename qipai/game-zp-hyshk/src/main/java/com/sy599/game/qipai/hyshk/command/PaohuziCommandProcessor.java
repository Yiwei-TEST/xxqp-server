package com.sy599.game.qipai.hyshk.command;
...
import com.sy599.game.qipai.hyshk.command.com.PaohuziComCommand;
import com.sy599.game.qipai.hyshk.command.play.PlayCommand;
...
public class PaohuziCommandProcessor extends AbstractBaseCommandProcessor {
	...
	public void process(HyshkPlayer player, MessageUnit message) {
		int code = 0;
		try {
	...
	public void process(Player player, MessageUnit message) {
		...
		int code = 0;
		try {
			HyshkPlayer pdkPlayer = player.getPlayer(processor);
			BaseCommand action = ObjectUtil.newInstance(commandMap.get(message.getMsgType()));
			action.setPlayer(player);
