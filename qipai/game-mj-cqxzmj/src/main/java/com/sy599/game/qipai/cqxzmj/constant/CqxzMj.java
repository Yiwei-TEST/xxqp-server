package com.sy599.game.qipai.cqxzmj.constant;
...
public enum CqxzMj {
    ...
    static CqxzMj[] mjArray;
    public static CqxzMj[] fullMj = new CqxzMj[27];
    static {
        init();
    }
    ...
    private static void init() {
        int maxVal = 0;
        for(CqxzMj mj : CqxzMj.values()) {
            if (mj.getVal() > maxVal && mj.getId() < 300) {
                maxVal = mj.getVal();
            }
        }
        mjArray = new CqxzMj[maxVal];
    ...
    CqxzMj(int id, int val) {
        this.id = id;
        this.val = val;
    }

    public static CqxzMj getMajang(int id) {
        return CqxzMj.valueOf(CqxzMj.class, "mj" + id);
    }
    ...
    public static CqxzMj getMajiangByValue(int value) {
        for (CqxzMj mj : CqxzMj.values()) {
            if (mj.getVal() == value) {
                return mj;
            }
    ...
    public static final List<Integer> code19= new ArrayList<>();
    ...
    public static List<Integer> getCode19() {
        return code19;
    }
     ...
     * 筛选掉单张超过4张的，剩下为19牌型。
     */
    public static void findAll19(){
        List<int[]> val19=new ArrayList<>();
        val19.addAll(new ArrayList<>(paiZuMap.get(1)));
        val19.addAll(new ArrayList<>(paiZuMap.get(9)));
        createCombo(new ArrayList<>(),val19,0,0);
    }

    /**
     *
     */
    private static void createCombo(List<int[]> combo,List<int[]> val19,int cur,int useNum) {
        if(combo.size()>0){
            //验证该组合
            verifyCombo(combo);
        }
        ...
        if(cur+1<val19.size())
            createCombo(combo,val19,cur+1,useNum);

        if(useNum<4){
            List<int[]> copy=new ArrayList<>(combo);
            copy.add(val19.get(cur));
            createCombo(copy,val19,cur,useNum+1);
        }
    }

    private static void verifyCombo(List<int[]> combo){
        int [] vals=new int[10];
        for (int[] zu:combo) {
