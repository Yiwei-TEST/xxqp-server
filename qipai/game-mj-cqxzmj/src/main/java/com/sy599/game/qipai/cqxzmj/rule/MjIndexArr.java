...
public class MjIndexArr {
    ...
    public void addMajiangIndex(int count, List<CqxzMj> majiangList, int val) {
        if (a[count] == null) {
            a[count] = new MjIndex();
        }
     ...
     * @return
     */
    public Map<Integer, List<CqxzMj>> getMajiangMap(int size) {
        Map<Integer, List<CqxzMj>> map = new HashMap<>();
    ...
    public Map<Integer, List<CqxzMj>> getJiang(boolean need258) {
        Map<Integer, List<CqxzMj>> map = new HashMap<>();
        for (int i = 0; i < a.length; i++) {
            if (2 <= i + 1) {
                MjIndex majiangIndex = a[i];
                if (majiangIndex != null) {
                    if (need258) {
                        for (Entry<Integer, List<CqxzMj>> entry : majiangIndex.getMajiangValMap().entrySet()) {
    ...
    public List<CqxzMj> getKeziList() {
