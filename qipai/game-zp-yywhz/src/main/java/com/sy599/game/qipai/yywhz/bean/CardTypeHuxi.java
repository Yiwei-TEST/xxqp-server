...
public class CardTypeHuxi {
	...
	public CardTypeHuxi(int action, List<Integer> cardIds, int hux, boolean selfMo) {
		this.action = action;
	...
	public boolean isHasCard(GuihzCard card) {
	...
	public Builder buildMsg(boolean hideCards) {
		...
		if (hideCards) {
			msg.addAllCards(GuihuziTool.toGhzCardZeroIds(cardIds));
		} else {
			if(cardIds == null)
