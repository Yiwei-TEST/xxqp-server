...
public class YzwdmjResTool {
    public static void buildPlayRes(PlayMajiangRes.Builder builder, Player player, int action, List<Yzwdmj> majiangs) {
        List<Integer> list = YzwdmjHelper.toMajiangIds(majiangs);
     ...
     * @return
     */
    public static PlayMajiangRes.Builder buildDisMajiangRes(Player player, Yzwdmj majiang) {
        PlayMajiangRes.Builder builder = PlayMajiangRes.newBuilder();
        builder.setAction(0);
        builder.setUserId(player.getUserId() + "");
        builder.setSeat(player.getSeat());
