...
public class ZpHuBean {
    ...
    // 打牌听牌信息
    private Map<Integer, List<Integer>> daTingMap;
    // 听牌信息
    private List<Integer> tingList;
    ...
    public Map<Integer, List<Integer>> getDaTingMap() {
        return daTingMap;
    }

    public void setDaTingMap(Map<Integer, List<Integer>> daTingMap) {
        this.daTingMap = daTingMap;
    }
     ...
     * @return
     */
    public Map<Integer, List<Integer>> calcDaTingMap() {
        if (daTingMap == null) {
            daTingMap = new HashMap<>();
        }
        for (int val = 1; val <= 20; val++) {
            if (cardArr[val] == 0 || cardArr[val] == 3 || cardArr[val] == 4) {
                continue;
            }
     ...
     * @return
     */
    public List<Integer> calcTingList() {
        List<Integer> tingList = new ArrayList<>();
        for (int val = 1; val <= 20; val++) {
            if (cardArr[val] == 0 || cardArr[val] == 3 || cardArr[val] == 4) {
                continue;
            ...
            List<ZpHuLack> huList = new ZpHuBean(cardArr, laiZiNum + 1).calcHu();
            if (huList.size() > 0) {
                int[] ting = new int[21];
                for (ZpHuLack hu : huList) {
                    int[] lzVal = hu.getLzVal();
