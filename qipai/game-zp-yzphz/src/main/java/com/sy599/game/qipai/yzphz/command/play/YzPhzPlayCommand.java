package com.sy599.game.qipai.yzphz.command.play;
...
public class YzPhzPlayCommand extends BaseCommand {
	@Override
	public void execute(Player player0, MessageUnit message) throws Exception {
		YzPhzTable table = player0.getPlayingTable(YzPhzTable.class);
		if (table == null) {
			return;
		}
		YzPhzPlayer player=(YzPhzPlayer)player0;
		// 该牌局是否能打牌
		...
		if (action == YzPhzConstants.action_pao||action == YzPhzConstants.action_mo){
			return;
		}
			...
			synchronized (table){
				...
				if (actionMap != null && (list=actionMap.get(player.getSeat()))!=null) {
					...
					if (list.get(7).intValue() == 1 || list.get(8).intValue() == 1 || list.get(9).intValue() == 1) {
						...
						LogUtil.monitorLog.info("action pass wang diao or chuang or zha:userId="+player.getUserId()+",tableId="+table.getId()+",card="+nextCard+",cardVal="+ YzPhzCardUtils.loadCardVal(nextCard)+",wang action="+list.get(7)+"_"+list.get(8)+"_"+list.get(9));
					}else{
					...
					if (list.get(0).intValue() == 1) {
						LogUtil.monitorLog.info("action pass hu:userId="+player.getUserId()+",tableId="+table.getId()+",card="+table.getHuCard()+",cardVal="+ YzPhzCardUtils.loadCardVal(table.getHuCard()));
					}
				}
			}
			if (!cards.isEmpty()) {
