...
public final class YzPhzCardUtils {
     ...
     *
     * @param playerCount
     * @param cardsCount 发牌数量
     * @return 下标为count代表桌子上的牌
     */
    public static List<List<Integer>> loadCards(List<Integer> allCards, int playerCount, int cardsCount) {
        List<Integer> cards=new ArrayList<>(allCards);
        List<List<Integer>> result = new ArrayList<>();

        int size = cards.size();
        if (playerCount <= 4 && size >= 80) {
            int index = 0;
        ...
        if(checkShuangLong(result,playerCount)){
            ...
            return loadCards(allCards,playerCount,cardsCount);
        }
...
//                }
    ...
    public static boolean canTi(YzPhzHandCards handCards, int id) {
        int val = loadCardVal(id);
        return handCards.KAN.containsKey(val);
    }

    public static boolean canPao(YzPhzHandCards handCards, int id) {
        int val = loadCardVal(id);
    ...
    public static YzPhzHuXiEnums loadHuXiEnum(List<Integer> list){
        sort(list);
        if (list.size()==3){
                ...
                if (val2>=WANGPAI_VAL){
                    ...
                    if (val1==val2){
                        ...
                        if (sameType(list.get(0),list.get(1))) {
                            if (val1 == 1 || val1 == 101) {
                                return YzPhzHuXiEnums.CHI123;
                            } else if (redCard(list.get(0))&&redCard(list.get(1))) {
                                return YzPhzHuXiEnums.CHI2710;
                            } else if ((val1==2&&val2==3)||(val1==102&&val2==103)){
                                return YzPhzHuXiEnums.CHI123;
                            }else {
                                return YzPhzHuXiEnums.CHI;
                            }
                        }else{
                            return YzPhzHuXiEnums.JIAO;
                        }
                    }
                }
        ...
        if (handCards.CHI_COMMON.size()>0){
            for(List<Integer> temp:handCards.CHI_COMMON){
                ...
                if (map.containsKey(1)){
                    list.add(new YzPhzCardMessage(YzPhzHuXiEnums.CHI123,temp));
                }else if (map.containsKey(101)){
                    list.add(new YzPhzCardMessage(YzPhzHuXiEnums.CHI123,temp));
                }else if (map.containsKey(2)&&map.containsKey(7)&&map.containsKey(10)){
                    list.add(new YzPhzCardMessage(YzPhzHuXiEnums.CHI2710,temp));
                }else if (map.containsKey(102)&&map.containsKey(107)&&map.containsKey(110)){
                    list.add(new YzPhzCardMessage(YzPhzHuXiEnums.CHI2710,temp));
                }else{
                    list.add(new YzPhzCardMessage(YzPhzHuXiEnums.CHI,temp));
                }
            }
        }
        return list;
    ...
    public static List<YzPhzChiCard> canChi(List<Integer> srcList, int id) {
        List<YzPhzChiCard> result = new ArrayList<>();
        int val = loadCardVal(id);
        sort(srcList);
        //绞牌
        List<Integer> similarVals = loadIdsByVal(srcList, val <= 10 ? val + 100 : val - 100);
        ...
        switch (sameVals.size()) {
            case 0: {
                if (similarVals.size() == 2) {
                    result.add(new YzPhzChiCard().buildChi(similarVals));
                }
                ...
                if (id1 > 0 && id2 > 0) {
                    result.add(new YzPhzChiCard().buildChi(asList(id1, id2)));
                }
                id1 = id2;
            ...
            case 1:
                ...
                if (yjhList.size() > 0) {
                    for (List<Integer> temp : yjhList) {
                        ...
                        List<YzPhzChiCard> list1 = canChi(list0, id);
                        for (YzPhzChiCard chiCard : list1) {
                ...
                if (similarVals.size() > 0) {
                    result.add(new YzPhzChiCard().buildChi(asList(sameVals.get(0), similarVals.get(0))));
                }
        ...
        if (result.size()>0&&sameVals.size()>0){
            List<YzPhzChiCard> result0=new ArrayList<>();
            for (YzPhzChiCard chiCard : result){
                YzPhzChiCard pre=null;
                List<Integer> chi=chiCard.getChi();
                sort(chi);
                int val1=loadCardVal(chi.get(0));
                int val2=loadCardVal(chi.get(1));
                for (YzPhzChiCard chiCard1 : result0){
     ...
     * @return
     */
    public static List<List<Integer>> loadYijuhua(List<Integer> srcList, int containVal,int wangId,boolean canJiao,boolean sort) {
     ...
     * @return
     */
    public static List<List<Integer>> loadYijuhua(List<Integer> srcList, int containVal,int wangId,boolean canJiao,boolean sort,boolean checkMultiDui) {
    ...
    public static int countHuxi0(List<Integer> list) {
        ...
        for (Integer tmp : list) {
            int val = YzPhzCardUtils.loadCardVal(tmp);
            Integer count = map.get(val);
        ...
        switch (list.size()){
            case 3:
                ...
                if (wangCount == null) {
                    ...
                    if (size == 3) {
                        if (map.containsKey(1)) {
                            ...
                            total = (YzPhzCardUtils.smallCard(list.get(0)) ? YzPhzHuXiEnums.CHI.getSmall() : YzPhzHuXiEnums.CHI.getBig());
                        }
                    } else if (size == 2) {
                        total = (YzPhzCardUtils.smallCard(list.get(1)) ? YzPhzHuXiEnums.JIAO.getSmall() : YzPhzHuXiEnums.JIAO.getBig());
                    } else {
                        total = (YzPhzCardUtils.smallCard(list.get(0)) ? YzPhzHuXiEnums.KAN.getSmall() : YzPhzHuXiEnums.KAN.getBig());
                    }
                } else {
                    ...
                    if (val1 == 201) {
                        total = YzPhzHuXiEnums.KAN.getBig();
                    } else if (val2 == 201) {
                        total = (YzPhzCardUtils.smallCard(list.get(0)) ? YzPhzHuXiEnums.KAN.getSmall() : YzPhzHuXiEnums.KAN.getBig());
                    } else if (val3 == 201) {
                        if (val1 == val2) {
                            ...
                            if (val1 == 1 || val1 == 101 || val1 == 2 || val1 == 102) {
                                total = YzPhzCardUtils.smallCard(list.get(0)) ? YzPhzHuXiEnums.CHI123.getSmall() : YzPhzHuXiEnums.CHI123.getBig();
                            } else {
                                total = YzPhzCardUtils.smallCard(list.get(0)) ? YzPhzHuXiEnums.CHI.getSmall() : YzPhzHuXiEnums.CHI.getBig();
                            }
                        } else if (redCard(list.get(0)) && redCard(list.get(1)) && sameType(list.get(0), list.get(1))) {
                            total = YzPhzCardUtils.smallCard(list.get(0)) ? YzPhzHuXiEnums.CHI2710.getSmall() : YzPhzHuXiEnums.CHI2710.getBig();
                        } else {
                            total = YzPhzHuXiEnums.JIAO.getBig();
                        }
                    }
                ...
                break;
            case 2:
                total= YzPhzCardUtils.smallCard(list.get(0)) ? YzPhzHuXiEnums.DUI.getSmall() : YzPhzHuXiEnums.DUI.getBig();
                break;
            case 4:
                total= YzPhzCardUtils.smallCard(list.get(0)) ? YzPhzHuXiEnums.TI.getSmall() : YzPhzHuXiEnums.TI.getBig();
                break;
        }
...
//            }
    ...
    public static synchronized List<List<Integer>> fapaiControl(List<Integer> copy, int playerCount, int cardsCount,int fapaiNum) {
        Collections.shuffle(copy);
        List<List<Integer>> list = loadCards(new ArrayList<>(copy), playerCount, cardsCount);
        fapaiNum++;
        if(fapaiNum>=2)
            return list;
        for (int j = 0; j < playerCount; j++) {
            ...
            int randNum =  new SecureRandom().nextInt(100);
            if(num>=3&&randNum<=50){
    ...
    public static void main1(String[] args) {
        int[] wangNum=new int[5];
        int count=0;
        for (int i = 0; i < 100000; i++) {
            int count1=0;
            List<List<Integer>> list = fapaiControl(YzPhzCardUtils.loadCards(3), 2, 14,0);
            for (int j = 0; j < 2; j++) {
                List<Integer> cards = list.get(j);
                int num=0;
                for (Integer id:cards) {
     ...
     * @return 手牌的红牌set
     */
    public static Set<Integer> getRedCards(List<Integer> ins) {
        Set<Integer> returnlist = new HashSet<>();
	    if(ins.size()>0){

            for (Integer i : ins) {
                    if(i%10==2 || i%10==7 || i%10==10 ){
                        returnlist.add(i);
                    }
            }
        }
     ...
     *
     * @param ins 手中普通牌
     * @param wangsList 手中王牌
     * @param val val 癞子要变的牌val
     * @param i 需要变的个数
     * @return
     */
    public static  HashMap<String,List<Integer>> turnLaizi(List<Integer> ins, List<Integer> wangsList, Integer val, int i) {
            List<Integer> turnlist = new ArrayList<>();
            //手中的牌为val的list
            List<Integer> shouka = new ArrayList<>();
            List<Integer> laizi2=  null;
            if(val<=40&& val!=201){
                laizi2 = new ArrayList<>(Arrays.asList(10+val,20+val,30+val,val));
            }else if(val>40&& val!=201){
                //大牌的val 是100+val
                int va = val-100;
                laizi2 = new ArrayList<>(Arrays.asList(50+va,60+va,70+va,40+va));
            }

            for (Integer s:ins) {
                if(loadCardVal(s)==val){
                    shouka.add(s);
                }
            }
            laizi2.removeAll(shouka);
            if(i==1){
                ins.add(laizi2.get(0));
                turnlist.add(laizi2.get(0));
            }else  if(i==2){
                ins.add(laizi2.get(0));
                ins.add(laizi2.get(1));

                turnlist.add(laizi2.get(0));
                turnlist.add(laizi2.get(1));
            }else  if(i==3){
                ins.add(laizi2.get(0));
                ins.add(laizi2.get(1));
                ins.add(laizi2.get(2));

                turnlist.add(laizi2.get(0));
                turnlist.add(laizi2.get(1));
                turnlist.add(laizi2.get(2));
            }
            wangsList = wangsList.subList(i,wangsList.size());

        HashMap<String,List<Integer>> map = new HashMap();
        map.put("handcards",ins);//癞子填充后手牌
        map.put("wangcards",wangsList);//剩余王牌
        map.put("turncards",turnlist);//癞子转换的牌
        return map;
    }

    public static void main(String[] args) {
//        List<Integer> a = new ArrayList<>(Arrays.asList(18,28, 22,32));
//        List<Integer> w = new ArrayList<>(Arrays.asList(81,82));
//        int val = 2;
//        int i= 2;
//        turnLaizi(a,w,val,i);
       ...
       a.clear();
        ...
        System.out.println("==>");
        System.out.println(a);
        List<Integer> a3 = new ArrayList<>(Arrays.asList(18,28, 22,32));
        a.addAll(a3);
        System.out.println("==>");
        System.out.println(a);
