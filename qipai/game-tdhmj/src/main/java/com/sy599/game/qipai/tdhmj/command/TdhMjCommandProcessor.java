package com.sy599.game.qipai.tdhmj.command;
...
import com.sy599.game.qipai.tdhmj.command.com.TdhMjComCommand;
import com.sy599.game.qipai.tdhmj.command.play.TdhMjPlayCommand;
...
public class TdhMjCommandProcessor extends AbstractBaseCommandProcessor {
    private static TdhMjCommandProcessor processor = new TdhMjCommandProcessor();
    ...
    static {
        commandMap.put(WebSocketMsgType.cs_com, TdhMjComCommand.class);
        commandMap.put(WebSocketMsgType.cs_play, TdhMjPlayCommand.class);
    ...
    public static TdhMjCommandProcessor getInstance() {
        return processor;
    }
    ...
    public void process(Player player, MessageUnit message) {
        int code = 0;
        try {
            if (player == null || !(player instanceof TdhMjPlayer)) {
                return;
            }
            TdhMjPlayer csMajiangPlayer = player.getPlayer(processor);
