...
//	}
	...
	public boolean isHasCard(TdhMj card) {
		return cardIds != null && cardIds.contains(card.getId());
	}

	public boolean isHasCardVal(TdhMj card) {
		if (cardIds != null) {
			List<TdhMj> majiangs = TdhMjHelper.toMajiang(cardIds);
			List<Integer> vals = TdhMjHelper.toMajiangVals(majiangs);
	...
	public int removeCardVal(int val) {
		if (cardIds != null) {
			...
			while (iterator.hasNext()) {
				...
				TdhMj majiang = TdhMj.getMajang(id);
