...
import com.sy599.game.qipai.yybs.command.YybsCommandProcessor;
...
public class YybsPlayer extends Player {
	...
	protected void loadFromDB0(RegInfo info) {
		if (UserDatasDao.getInstance().exists()){
			String val = UserDatasDao.getInstance().selectUserDataValue(String.valueOf(userId),YybsTable.GAME_CODE,"all","currentLs");
			if (CommonUtil.isPureNumber(val)){
				currentLs = Integer.parseInt(val);
			}else{
				UserDatasDao.getInstance().saveUserDatas(String.valueOf(userId),YybsTable.GAME_CODE,"all","currentLs","0");
			}

			val = UserDatasDao.getInstance().selectUserDataValue(String.valueOf(userId),YybsTable.GAME_CODE,"all","maxLs");
			if (CommonUtil.isPureNumber(val)){
				maxLs = Integer.parseInt(val);
			}else{
				UserDatasDao.getInstance().saveUserDatas(String.valueOf(userId),YybsTable.GAME_CODE,"all","maxLs","0");
			}
		}
	}
	...
	public void setAutoPlay(boolean autoPlay,BaseTable table) {
		if (this.autoPlay != autoPlay && !isRobot()){
			...
			YybsTable table2 = getPlayingTable(YybsTable.class);
			...
			if(null == table2){
				...
				table2.addPlayLog(table2.addSandhPlayLog(getSeat(), YybsConstants.action_tuoguan, val, false, 0, null, 0,""));
			}
        	// table.addPlayLog(getSeat(), YybsConstants.action_tuoguan + "",(autoPlay?1:0) + "");
		}
	...
	public void dealHandPais(List<Integer> pais,YybsTable table) {
		this.table = table;
		this.handPais = pais;
	...
	public void addOutPais(List<Integer> cards ,YybsTable table) {
		this.table = table;
	...
	public PlayerInTableRes.Builder buildPlayInTableInfo(long recUserId, boolean isrecover) {
		...
		YybsTable table = getPlayingTable(YybsTable.class);
...
//		extList.add(0);//7
	...
	public static void loadWanfaPlayers(Class<? extends Player> cls){
		for (Integer integer:wanfaList){
			PlayerManager.wanfaPlayerTypesPut(integer,cls, YybsCommandProcessor.getInstance());
		}
	}
