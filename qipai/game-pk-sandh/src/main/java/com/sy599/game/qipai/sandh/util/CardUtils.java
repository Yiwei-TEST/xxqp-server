package com.sy599.game.qipai.sandh.util;
...
public final class CardUtils {
     ...
     * @param cards
     */
    public static CardType  isTuoLaji(List<Integer> cards,int zhuColor,boolean chouLiu) {
		...
		boolean tlj = true;
    	//是否全是对子len / 2 - 1
		if(hasDuiCount(newCardIds)==100&&(newCardIds.get(newCardIds.size()-1) -newCardIds.get(0)==newCardIds.size()/2-1)&&(isSameColor(cards)&&!isContainsYZ(cards))){//连对 拖拉机(没有硬主2 7 王)
			...
			if(dui == 100) {//全是对子
				...
				for(int i=0;i<newCardIds.size()-1;i++) {
					...
					int val1 = getCardPro(a, zhuColor,chouLiu);
					int val2 = getCardPro(b, zhuColor,chouLiu);
					//相邻的两对不是连着的 不是拖拉机
...
//						break;
		...
		//拖拉机
		if(ct.getType()==0&&(CardTool.allZhu(cards, zhuColor)||isSpcialTLJ(cards))){
			ct.setCardIds(newCardIds);
			ct.setType(CardType.TUOLAJI);
		}
    ...
    private static int getCardPro(int card,int zhuColor,boolean chouLiu){
    ...
    private static boolean isSpcialTLJ(List<Integer> hands){
    	boolean flag = false;
    	if(!isSameColor(hands)){
    		return false;
    	}
    ...
    public static void main(String[] args) {
		...
		for(int i=0;i<newCardIds.size()-1;i++) {
			int a = newCardIds.get(i);
			int b = newCardIds.get(i+1);
			int val1 = getCardPro(a, 3,false);
			int val2 = getCardPro(b, 3,false);
			//相邻的两对不是连着的 不是拖拉机
			if(a!=b){//&&(Math.abs(val2-val1)!=1)
				if(!proList.contains(val1)){
					proList.add(val1);
