...
import com.sy599.game.qipai.sandh.command.SandhCommandProcessor;
...
public class SandhPlayer extends Player {
	...
	protected void loadFromDB0(RegInfo info) {
		if (UserDatasDao.getInstance().exists()){
			String val = UserDatasDao.getInstance().selectUserDataValue(String.valueOf(userId),SandhTable.GAME_CODE,"all","currentLs");
			if (CommonUtil.isPureNumber(val)){
				currentLs = Integer.parseInt(val);
			}else{
				UserDatasDao.getInstance().saveUserDatas(String.valueOf(userId),SandhTable.GAME_CODE,"all","currentLs","0");
			}

			val = UserDatasDao.getInstance().selectUserDataValue(String.valueOf(userId),SandhTable.GAME_CODE,"all","maxLs");
			if (CommonUtil.isPureNumber(val)){
				maxLs = Integer.parseInt(val);
			}else{
				UserDatasDao.getInstance().saveUserDatas(String.valueOf(userId),SandhTable.GAME_CODE,"all","maxLs","0");
			}
		}
	}
	...
	public void setAutoPlay(boolean autoPlay,BaseTable table) {
		if (this.autoPlay != autoPlay && !isRobot()){
			...
			SandhTable table2 = getPlayingTable(SandhTable.class);
			...
			if(table2!=null){
				table2.addPlayLog(table2.addSandhPlayLog(getSeat(), SandhConstants.action_tuoguan, val, false, 0, null, 0));
			}
			// table.addPlayLog(getSeat(), SandhConstants.action_tuoguan + "",(autoPlay?1:0) + "");
		}
	...
	public void dealHandPais(List<Integer> pais,SandhTable table) {
		this.table = table;
		this.handPais = pais;
	...
	public void addOutPais(List<Integer> cards ,SandhTable table) {
		this.table = table;
	...
	public PlayerInTableRes.Builder buildPlayInTableInfo(long recUserId, boolean isrecover) {
		...
		SandhTable table = getPlayingTable(SandhTable.class);
		if (table == null) {
	...
	public static void loadWanfaPlayers(Class<? extends Player> cls){
		for (Integer integer:wanfaList){
			PlayerManager.wanfaPlayerTypesPut(integer,cls, SandhCommandProcessor.getInstance());
		}
	}
