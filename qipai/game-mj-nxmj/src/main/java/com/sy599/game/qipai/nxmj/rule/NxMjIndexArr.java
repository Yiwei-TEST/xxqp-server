 ...
 * 
 */
public class NxMjIndexArr {
	NxMjIndex[] a = new NxMjIndex[4];

	public void addMajiangIndex(int count, List<NxMj> majiangList, int val) {
		if (a[count] == null) {
			a[count] = new NxMjIndex();
		}
	 ...
	 * @return
	 */
	public Map<Integer, List<NxMj>> getMajiangMap(int size) {
		Map<Integer, List<NxMj>> map = new HashMap<>();
		for (int i = 0; i < a.length; i++) {
			if (size <= i + 1) {
				NxMjIndex majiangIndex = a[i];
	...
	public Map<Integer, List<NxMj>> getJiang(boolean need258) {
		Map<Integer, List<NxMj>> map = new HashMap<>();
		for (int i = 0; i < a.length; i++) {
			if (2 <= i + 1) {
				NxMjIndex majiangIndex = a[i];
				if (majiangIndex != null) {
					if (need258) {
						for (Entry<Integer, List<NxMj>> entry : majiangIndex.getMajiangValMap().entrySet()) {
	 ...
	 * @return
	 */
	public NxMjIndex getMajiangIndex(int index) {
		NxMjIndex majiangIndex = a[index];
		if (majiangIndex == null) {
