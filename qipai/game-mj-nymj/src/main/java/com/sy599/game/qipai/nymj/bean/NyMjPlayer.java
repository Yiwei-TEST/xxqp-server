...
import com.sy599.game.qipai.nymj.command.MjCommandProcessor;
import com.sy599.game.qipai.nymj.rule.Mj;
import com.sy599.game.qipai.nymj.rule.MjHelper;
import com.sy599.game.qipai.nymj.tool.MjEnumHelper;
...
@Data
public class NyMjPlayer extends Player {
...
//    //麻将id->明杠>1000|暗杠>1000|seat
    ...
    //明杠得分
    private int mGangScoreAdd;
    //暗杠得分
    private int aGangScoreAdd;
    //放杠得分
    private int fGangScoreAdd;
    //大胡分数 展示用
    private int daHuScore;
    ...
    public int getFlutterVal() {
        int f = (flutter > 10 ? flutter / 10 : flutter);
        return f==-1?0:f;
    }
    ...
    public List<Mj> showXiaoHuMajiangs(int xiaohu, boolean connect) {
        NyMjTable table = getPlayingTable(NyMjTable.class);
    ...
    public String toInfoStr() {
		...
		sb.append(mGangScoreAdd).append(",");
		sb.append(aGangScoreAdd).append(",");
		sb.append(fGangScoreAdd).append(",");
		sb.append(daHuScore).append(",");
        return sb.toString();
    }
    ...
    public void initPlayInfo(String data) {
        if (!StringUtils.isBlank(data)) {
            ...
            this.flutter = StringUtil.getIntValue(values, i++);
			...
			this.mGangScoreAdd = StringUtil.getIntValue(values, i++);
			this.aGangScoreAdd = StringUtil.getIntValue(values, i++);
			this.fGangScoreAdd = StringUtil.getIntValue(values, i++);
			this.daHuScore = StringUtil.getIntValue(values, i++);
        }
    }
    ...
    private void addCloseingExt(ClosingMjPlayerInfoRes.Builder res) {
		...
		res.addExt(getPlayingTable(NyMjTable.class).getReappointment()); //连庄 0~9 - 1~10
		res.addExt(mGangScoreAdd); //明杠 3
		res.addExt(aGangScoreAdd); //暗杠 4
		res.addExt(fGangScoreAdd); //放杠 5
//        res.addExt(horseAndGangScore); //马和杠  0~6
		...
		res.addExt(-Math.abs(mGangScoreAdd - mGangScore)); //明杠 10
		res.addExt(-Math.abs(aGangScoreAdd - aGangScore)); //暗杠 11
		res.addExt(-Math.abs(fGangScoreAdd - fGangScore)); //放杠 12
		res.addExt(daHuScore);    //大胡门子分数
...
//		}
    ...
    public List<Mj> getCanChiMajiangs(Mj disMajiang) {
        return MjTool.checkChi(handPais, disMajiang, getPlayingTable(NyMjTable.class));
    }
    ...
    public boolean isTingPai(int val, boolean onlySevenDuiNoHu, boolean needRemoveThisVal) {
        ...
        if (copy.size() % 3 != 2 ) {
            if (getPlayingTable(NyMjTable.class).getKingCard() != null) {
                copy.add(Mj.getMajang(getPlayingTable(NyMjTable.class).getKingCard().getId()));
            }else {
        ...
        boolean jiang258 = getPlayingTable(NyMjTable.class).getGameModel().isNeed258();
        NyMjTable table = getPlayingTable(NyMjTable.class);
        //开杠
        if (table.getGameModel().getSpecialPlay().isJiajianghu()) {
            jiang258 = false;
        }
        MjiangHu huBean = MjTool.isHu(copy, gangList, copyPeng, chi, bzCopy, false, jiang258, table, handPais.get(0), this, false, 0);
    ...
    public List<Integer> checkDisCard(Mj majiang) {
        return checkDisCard(majiang, true, false, getPlayingTable(NyMjTable.class).getKingCard() != null && majiang != null && getPlayingTable(NyMjTable.class).getKingCard().getVal() == majiang.getVal());
    }
     ...
     * @param haveKingIce 有的王牌不能变了
     * @return
     * @description
     * @author Guang.OuYang
     * @date 2019/10/28
     */
    public List<Integer> checkDisCard(Mj majiang, boolean need258, boolean gangHouMo, boolean haveKingIce) {
        List<Integer> list = new ArrayList<>();
        ...
        if ((!table.moHu() || table.getDisCardSeat() == seat) && passMajiangVal != majiang.getVal()) {
            ...
            MjiangHu hu = MjTool.isHu(copy, getGang(), peng, chi, bzCopy, false, jiang258, table, majiang, this, haveKingIce, -1);
            ...
            if (gangList.isEmpty() || !table.getGameModel().getSpecialPlay().isGangMoNum()) {
                ...
                if (table.calcNextSeat(table.getDisCardSeat()) == seat) {
                    if (!(/*table.getMaxPlayerCount() == 2 &&*/ table.getGameModel().getSpecialPlay().isBuChi())) {
                        List<Mj> chi = MjTool.checkChi(handPais, majiang, getPlayingTable(NyMjTable.class));
                        if (!chi.isEmpty()) {
                            actData.addChi();
                        }
                    }
...
//            }
    ...
    public int getDahuScore() {
        return MjiangHu.calcMenZiScore(getPlayingTable(NyMjTable.class), getDahu());
    }
    ...
    public MjiangHu checkHu(Mj disMajiang, boolean isbegin) {
		return checkHu(disMajiang, isbegin, true);
	}
    ...
    public MjiangHu checkHu(Mj disMajiang, boolean isbegin, boolean haveKingIce) {
        ...
        MjiangHu hu = MjTool.isHu(copy, getGang(), peng, chi, bzCopy, isbegin, jiang258, table, disMajiang, this, haveKingIce, -1);
        if (hu.isHu()) {
            if (table.getGameModel().getSpecialPlay().isMenqing() && !isChiPengGang()) {
                hu.setMenqing(true);
            }
            hu.setFlutter(getFlutterTypeBool());
            //跟庄
//            hu.setGenZhuang(getPlayingTable(ChaosMjTable.class).isFollowMaster());
    ...
    public List<Integer> checkMoCard(Mj majiang, boolean isBegin) {
        ...
        if (noNeedMoCard() || majiang != null || isBegin) {
            ...
            List<Mj> copy = new ArrayList<>(handPais);
            MjiangHu hu = MjTool.isHu(copy, getGang(), peng, chi, bzCopy, isBegin, jiang258, table, majiang, this, false, -1);
...
//            canGang = hu.isHu() && !((!table.getGameModel().getSpecialPlay().isXiaoDuiGang() && (hu.isXiaodui() || hu.isHao7xiaodui() || hu.isShuang7xiaodui())) && hu.getDahuCount() == 1);
            ...
            //自摸控制
            if (hu.isHu() && getPlayingTable(NyMjTable.class).getGameModel().getSpecialPlay().isSelfMoHu()) {
                //二人麻将
                if (table.getMaxPlayerCount() == 2) {
                    if (table.getGameModel().getSpecialPlay().isOnlyDaHu()) {
                        if (table.getGameModel().getSpecialPlay().isMenqing() && !isChiPengGang()) {
    ...
    private void checkGang(MjAction actData, NyMjTable table, boolean canXiaohu, boolean canGang) {
        if ((noNeedMoCard() && !canXiaohu) || !table.getGameModel().isNeedTingGang()) {
            ...
            for (Mj handMajiang : handPais) {
                if (pengMap.containsKey(handMajiang.getVal())) {
                    // 有碰过
                    boolean isTing = table.getGameModel().isNeedTingGang() ? isTingPai(handMajiang.getVal(), true) : true;
            ...
            if (handMap.containsValue(4)) {
                for (Entry<Integer, Integer> entry : handMap.entrySet()) {
                    if (entry.getValue() == 4) {
                        boolean isTing = table.getGameModel().isNeedTingGang() ? isTingPai(entry.getKey(), true) : true;
                        if (isTing /*&& !MjRule.isKingCard(Mj.findAnyValByMj(entry.getKey()), table.getKingCard())*/) {
                            if (canGang)
