package com.sy599.game.qipai.dzbp.tool;

import com.alibaba.fastjson.JSON;
import com.sy599.game.GameServerConfig;
import com.sy599.game.qipai.dzbp.bean.PaohuziHandCard;
import com.sy599.game.qipai.dzbp.bean.PaohuziHuBean;
import com.sy599.game.qipai.dzbp.bean.PaohzDisAction;
...
public class PaohuziTool {
    ...
    public static synchronized List<List<PaohzCard>> fapai(List<Integer> copy, List<List<Integer>> t,int playerCount) {
        ...
        if (GameServerConfig.isDebug() && t != null && !t.isEmpty()) {
            ...
            int fapaiCount = 14 * 2 + 1 - testcount;
            if (pai.size() >= 15) {
                list.add(pai);
                pai = new ArrayList<>();
            }
...
//            return false;
    ...
    public static synchronized boolean checkTianHu(List<List<PaohzCard>> list){
        ...
        PaohuziHuLack hu = isHuNew(PaohuziTool.getPaohuziHandCardBean(banker), null, false, 0, false, false);
        if(hu.isHu())
            return false;
        return true;
    }
...
//				lack.addPhzHuCards(PaohzDisAction.action_zai, toPhzCardIds(cards), huxi);
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 100000; i++) {
            int count1=0;
            List<List<PaohzCard>> list = fapai(copy, null, 2);
            for (int j = 0; j < 2; j++) {
    ...
    public static PaohuziHuLack isHuNew(PaohuziHandCard handCardBean, PaohzCard disCard, boolean isSelfMo, int outCardHuxi, boolean needDui, boolean isPaoHu) {
