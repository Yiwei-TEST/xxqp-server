package com.sy599.game.qipai.dzbp.command;
...
import com.sy599.game.qipai.dzbp.command.com.PaohuziComCommand;
import com.sy599.game.qipai.dzbp.command.play.PlayCommand;
...
public class PaohuziCommandProcessor extends AbstractBaseCommandProcessor {
	...
	public void process(DzbpPlayer player, MessageUnit message) {
		int code = 0;
		try {
	...
	public void process(Player player, MessageUnit message) {
		...
		int code = 0;
		try {
			DzbpPlayer pdkPlayer = player.getPlayer(processor);
			BaseCommand action = ObjectUtil.newInstance(commandMap.get(message.getMsgType()));
			action.setPlayer(player);
