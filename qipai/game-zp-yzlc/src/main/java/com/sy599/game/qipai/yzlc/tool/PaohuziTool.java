package com.sy599.game.qipai.yzlc.tool;

import com.sy599.game.GameServerConfig;
import com.sy599.game.qipai.yzlc.bean.CardTypeHuxi;
import com.sy599.game.qipai.yzlc.bean.PaohuziHandCard;
import com.sy599.game.qipai.yzlc.bean.PaohuziHuBean;
...
import com.sy599.game.qipai.yzlc.rule.PaohuziIndex;
import com.sy599.game.qipai.yzlc.rule.PaohuziMingTangRule;
import com.sy599.game.qipai.yzlc.rule.PaohzCardIndexArr;
import com.sy599.game.qipai.yzlc.bean.GameModel;
import com.sy599.game.qipai.yzlc.rule.PaohuziMingTangRule.FindAnyCombo.FindAny;
...
public class PaohuziTool {
    ...
    public static List<List<PaohzCard>> getAllSeatAllCardNew(List<Integer> initAllCardsCombo, List<List<Integer>> gmOptCards, GameModel gameModel) {
        ...
        int totalSeat = 3;
        //庄家比闲家多的牌
        int mainAddCardCount = 0;
        //闲家牌数量
		int optSeatCardCount = gameModel.getGameMaxHuman() == 4 ? 20 : 26;
        //庄家牌数量
...
//            //检测顺子or对子
    ...
    public static synchronized List<List<PaohzCard>> fapai(List<Integer> copy, List<List<Integer>> t, GameModel gameModel) {
        ...
        int fapaiCount = gameModel.getGameMaxHuman() == 4 ? 20 : 26;
        if (pai.size() >= 21) {
            list.add(pai);
            pai = new ArrayList<>();
        }
...
//    }
    ...
    private static boolean chaishun3(PaohuziHuLack lack, List<PaohzCard> hasPais, PaohzCard minCard, boolean isShun, boolean check2710, int pai1, int pai2, int pai3, GameModel gameModel) {
        ...
        if (lackNum > 0) {
            ...
            int huxi = 0;
            int action = PaohzDisAction.action_small_face;
            if (isShun) {
                int minPai = minCard.getPai();
                if (minPai == 1 || minPai == 2) {
                    // 123 和 2710 加胡息
                ...
                if (isSameCard(hasMajiangList)) {
                    // 如果是三个一模一样的
                    action = PaohzDisAction.action_single_card;
                    if (lack.isSelfMo()) {
                        action = PaohzDisAction.action_double_card;
                    }
...
//				lack.addPhzHuCards(PaohzDisAction.action_double_card, toPhzCardIds(cards), huxi);
        ...
        if (index3 != null) {
            ...
            for (int val : list) {
                int huxi = 0;
                boolean paoHu = false;
                int action = PaohzDisAction.action_eat_box;
                if (disCard != null && val == disCard.getVal()) {
                    if (!isPaoHu) {
                        ...
                        action = PaohzDisAction.action_big_face;
                    } else {
                        action = PaohzDisAction.action_eat_box;
                        paoHu = true;
                        if (val > 100) {
                            // 碰大的6分
                            huxi = 9;
    ...
    public static PaohzCard autoDisCard(List<PaohzCard> handPais) {
        ...
        if (val == 0) {
        	...
        	if(handPais!=null&&!handPais.isEmpty()){
        		return Collections.min(handPais);
			}
...
//    }
    ...
    public static PaohuziHuLack isHuNew(PaohuziHandCard handCardBean, PaohzCard disCard, boolean isSelfMo, int outCardHuxi, boolean needDui, boolean isPaoHu, GameModel gameModel, YzLcPaohuziPlayer player) {
		PaohuziHuLack lack = new PaohuziHuLack(0);
		lack.setSelfMo(isSelfMo);
		lack.setCheckCard(disCard);
		lack.setHu(player.getHuxi() >= gameModel.getRoundFinishLowestHuXi());
		lack.setHuxi(player.getHuxi());
		if (lack.isHu()) {
			Iterator<CardTypeHuxi> iterator = player.getCardTypes().iterator();
			while (iterator.hasNext()) {
				CardTypeHuxi next = iterator.next();
				lack.addPhzHuCards(next.getAction(), next.getCardIds(), next.getHux());
			}
		}
		return lack;
	}
        ...
        if (lack.isNeedDui()) {
            ...
            for (int[] paiZu : paiZus) {
                ...
                if (disCard != null && disCard.getVal() == val && isSameCard(rmList)) {
                    // 三张一模一样的，碰牌分
                    newLack.addPhzHuCards(PaohzDisAction.action_single_card, toPhzCardIds(rmList), disCard.isBig() ? 3 : 1);
                } else {
    ...
    public static class Counter<T> {
		...
		public Counter addValue(T t) {
			if (tValues == null && t != null) {
				tValues = new ArrayList<T>();
				tValues.add(t);
			}
			return this;
		}

		public Counter id(int id) {
			this.id = id;
			return this;
		}

		public Counter count(int count) {
			this.count = count;
			return this;
		}
